...
# ---------------------------------------------------------------- W972 additions
# T811 left one place where the rules were mine: the float peer was defined FTZ, to
# match TNF's own zero handling. The honest alternative is a float with real
# subnormals, with the leading-zero normaliser they require PAID FOR on the side
# that has them -- TNF pays nothing because its grid genuinely has no subnormals,
# which is a property of the format and not a choice of mine.
class SUB:
    def __init__(self, name, exp_bits, mant_bits, bias):
        self.name, self.exp_bits, self.mant_bits, self.bias = name, exp_bits, mant_bits, bias
        self.mant = 1 << mant_bits
...
def sub_ref(f, raw):
    """Python reference: IEEE-shaped float WITH subnormals, as fp32 bits."""
    s = (raw >> f.sign_shift) & 1
    off = (raw >> f.exp_shift) & ((1 << f.exp_bits) - 1)
    m = raw & (f.mant - 1)
    if off == f.offset_max:
    ...
    if off == 0:
        if m == 0:
            return s << 31
        lz = f.mant_bits - m.bit_length()          # leading zeros inside the field
        e32 = (1 - f.bias - lz - 1) + 127
        mm = (m << (lz + 1)) & (f.mant - 1)
        if not (0 < e32 < 255):
            return None
        return (s << 31) | (e32 << 23) | (mm << (23 - f.mant_bits))
    e32 = (off - f.bias) + 127
    if not (0 < e32 < 255):
        return None
    return (s << 31) | (e32 << 23) | (m << (23 - f.mant_bits))
...
def sub_emit(name, f, bits):
    MB = f.mant_bits
    return f"""`default_nettype none
// GENERATED for an IEEE-shaped float e{f.exp_bits}m{MB}, bias {f.bias}, WITH subnormals.
// The leading-zero normaliser they require is synthesised here and therefore paid for.
// TNF pays nothing for it because its grid has no subnormals -- a property of the
// format, not a convention chosen by the comparison. Verified against the Python
// reference over all {1 << bits} codes.
module s_{name}_decode (input wire [{bits-1}:0] x, output wire [31:0] fp32_out);
  wire        s   = x[{f.sign_shift}];
  wire [{f.exp_bits-1}:0] off = x[{f.sign_shift-1}:{f.exp_shift}];
  wire [{MB-1}:0] m = x[{MB-1}:0];
  wire is_inf  = (off == {f.exp_bits}'d{f.offset_max});
  wire is_sub  = (off == {f.exp_bits}'d0) && (|m);
  wire is_zero = (off == {f.exp_bits}'d0) && (~|m);

  function [{(MB-1).bit_length()-1 if MB>1 else 0}:0] lzc(input [{MB-1}:0] v);
    integer i;
    begin
      lzc = {MB};
      for (i = 0; i < {MB}; i = i + 1)
  ...
  wire [{(MB-1).bit_length()-1 if MB>1 else 0}:0] lz = lzc(m);
  wire [{MB-1}:0] mnorm = m << (lz + 1);
  wire [7:0] e_norm = off + 8'd{127 - f.bias};
  wire [7:0] e_sub  = 8'd{127 + 1 - f.bias - 1} - {{{8 - ((MB-1).bit_length() if MB>1 else 1)}'b0, lz}};
  wire [7:0] e32 = is_sub ? e_sub : e_norm;
  wire [{MB-1}:0] mout = is_sub ? mnorm : m;
  assign fp32_out = is_inf  ? (|m ? 32'h7FC00000 : {{s, 31'h7F800000}})
                  : is_zero ? {{s, 31'b0}}
                            : {{s, e32, mout, {23 - MB}'b0}};
endmodule
...
SUB_UNITS = [
    ("fp19_e7m11_sub", SUB("fp19_e7m11_sub", 7, 11, 63), 19),
]
FTZ_UNITS = [
...
if __name__ == "__main__":
    ...
    for name, f, bits in SUB_UNITS:
        ...
        vf.write_text(sub_emit(name, f, bits))
        ...
        if bare and fu:
            res[name] = {"physical_bits": bits, "codes_checked": checked, "mismatches": 0,
                         ...
                         "r2_fused": fu["r2"], "kind": "ieee_float_with_subnormals"}
    ...
    for name, et, mb in RUNGS:
        ...
        (OUT / "struct972.json").write_text(json.dumps(res, indent=1))
    print("WROTE " + str(OUT / "struct972.json"), flush=True)
