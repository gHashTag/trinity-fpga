...
NS = [1, 2, 4, 8]
# W955: two points said cost tracks range at about 2.4 cells per binade per lane.
# This sweeps EVERY exponent/mantissa split at two widths, so the observation becomes
# a curve with the TNF rungs plotted on it rather than an anecdote from two pairs.
_F = F8.FPxFormat
FMT = {}
for _w, _splits in ((6, range(1, 5)), (10, range(3, 8))):
    for _eb in _splits:
        _mb = _w - 1 - _eb
        FMT[f"fp{_w}_e{_eb}m{_mb}"] = (F8, _F(f"fp{_w}_e{_eb}m{_mb}", _eb, _mb,
                                              (1 << (_eb - 1)) - 1), _w)
FMT["TNF4_6b"] = (T, T.TNFFormat(2, 1), 6)
FMT["TNF8_10b"] = (T, T.TNFFormat(3, 4), 10)
...
for name, spec in FMT.items():
    ...
    acc = algn + 1 + 5
    import math as _m
    _nz = [abs(v) for v in vals.values() if v != 0]
    out["fields"][name] = {"odd_bits": ob, "max_shift": smax, "aligned": algn, "acc": acc,
                           "binades": round(_m.log2(max(_nz) / min(_nz)), 3),
                           "values": len(set(vals.values())), "width": spec[2]}
    ...
    print(f"  -> {name}: {slope:.2f} ячеек на флоат-полосу (+{ms:.2f} MUXF), R²={r2:.5f}", flush=True)
    try:
        (S / "curve_w955.json").write_text(json.dumps(out, indent=1))
    except OSError as e:
