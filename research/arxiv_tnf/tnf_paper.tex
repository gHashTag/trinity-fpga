...
constants at synthesis in fifteen of twenty-eight clauses, so every area figure
...
$F_{\max}\approx 3174\cdot\mathrm{LUT}^{-0.648}$ ($R^2=0.92$) across a
$41\times$ span of area, which makes MHz per LUT a proxy for size rather than a
...
Section~\ref{sec:selfcheck} sets out the comparison failures this work had to
rule out, so the reasoning
...
codec passing a 35-of-35 functional testbench on an Artix-7. The $323$\,MHz
frequency once quoted for that codec is withdrawn and is not used here: the GF16
cores contain no registers, the synthesis top feeds them literal operands, so the
arithmetic is constant-folded away, and the reported figure was the toggle rate of
the ring oscillator left in the test top. No frequency for the GF16 codec has been
characterised. That preprint also carries an RTL-correctness erratum dated
...
Section~\ref{sec:matchedwidth}. A reference does not need to win those comparisons. It needs to be the
...
no-survivors corollary of Section~\ref{sec:catalogue} does not survive being
...
width: the TNF rungs store more bits than their names --- on the v2
specification ladder TNF8 stores 10, TNF16 \textbf{19}, TNF32 \textbf{36} ---
so each row hands TNF a margin of 2, 3 and 4 bits respectively, and the ratio
column is \emph{not} a matched-width result. The exact-width rungs are
TNF(2,3), TNF(4,8) and TNF(4,24), and Table~\ref{tab:fullthroughput} reports
these same measurements against the physical widths. Every
...
field unpacks by reading fields. Fixed and tapered formats separate on
$F_{\max}$, not on LUT area: every fixed field decodes faster than every taper,
while on area the two groups overlap (Table~\ref{tab:cleandecode},
Theorem~\ref{thm:taperdelay}). That separation belongs to Theorems~\ref{thm:scan}
...
\[
  E_t^{*} = \bigl\lceil \log_3 (b+2) \bigr\rceil,
  \qquad
  M^{*} = N - 1 - \bigl\lceil \log_3 (b+2) \bigr\rceil .
\]
\end{theorem}
\begin{proof}
...
$86$ of the $1038$ rows in each of the eighteen files published at
\texttt{tnf-vectors-2}, four and a half to eight and a third per cent, have an
expected result recorded as a negative code word, which no hardware can produce.
Of the $1{,}206$ affected rows, $1{,}170$ are multiplications and $36$ are
additions spread over fifteen of the eighteen files. Those rows were
unsatisfiable. Rather than overwrite files whose digests are cited, we bumped the
vector specification version: the affected files remain in place at
\texttt{tnf-vectors-2} and a repaired set is published beside them at
\texttt{tnf-vectors-3}, with its own manifest of SHA-256 digests and no negative
code word at any rung. Any transcript
