...
\paragraph{The accounting has one hole, and it is stated rather than closed.}
At the finest ladder the chosen absolute grid index over full tensors spans
$m\in[-176,-27]$ for qwen. That is $150$ codes, which fits in $255$ but
\emph{does not fit MXFP4's bias-$127$ origin}. A re-biased origin costs
essentially nothing per element, but it is a format-level constant tuned on two
sub-billion-parameter models, so the honest accounting is \textbf{$4.25$ bits per
element plus an origin constant MXFP4 does not have}.
