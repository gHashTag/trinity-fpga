...
Twenty retractions of our own claims were made during the work, and are
enumerated in Section~\ref{sec:selfcheck}. Eighteen retractions are marked in
place below; the remaining two were themselves withdrawn.
\end{abstract}
...
twenty retractions during the work. Eighteen retractions survive as marks in
this revision; two were themselves withdrawn. Applied to two external papers read in full it produced
...
\section{The hardware axis opened, and the die contradicted a simulation}
\label{sec:silicon}

Every previous revision of this paper reported hardware numbers from synthesis:
cell counts after \texttt{yosys}, with no place-and-route, no bitstream, and no
part. Twenty-three research waves recorded the hardware axis as blocked. It was
not blocked by any of the things we had named. Docker was never used, the
toolchain was native and present, and the cable worked. The two real obstacles
were a JTAG chain that read empty until it did not, and a default cable address
still pointing at a bench configuration that no longer existed (W973, W974).

This section reports what the die said once it answered. It summarises records
W973 through W992. Two of its results are unfavourable to us and are stated here
rather than left for a referee.

\subsection{Four designs measured through a full flow}
\label{sec:silicon-designs}

Table~\ref{tab:silicondesigns} gives the first end-to-end numbers: specification
to Verilog to bitstream through native \texttt{openxc7} (\texttt{yosys},
\texttt{nextpnr-xilinx}, \texttt{prjxray}), on an \texttt{xc7a200tfbg676-1},
IDCODE \texttt{0x3636093}. These are placed and routed, not estimated.

\begin{table}[t]\centering
\caption{Measured through place-and-route on \texttt{xc7a200tfbg676-1} (W974).
The classifier is constrained on \texttt{cfgmclk} and the \texttt{gft\_*} designs
on \texttt{slowclk}; the two groups are \emph{not} comparable across that
boundary, and the table is ruled to say so.}
\label{tab:silicondesigns}
\begin{tabular}{lrrrr}
\hline
design & LUT & $F_{\max}$ (MHz) & clock & MHz/kLUT \\
\hline
\texttt{mvp\_ternary\_classifier} & 123 & 80.35 & \texttt{cfgmclk} & 653.3 \\
\hline
\texttt{gft\_sadd} & 1{,}312 & 18.24 & \texttt{slowclk} & 13.9 \\
\texttt{gft\_signed\_mac} & 6{,}466 & 9.14 & \texttt{slowclk} & 1.41 \\
\texttt{gft\_signed\_dot4} & 12{,}872 & 5.50 & \texttt{slowclk} & 0.43 \\
\hline
\end{tabular}
\end{table}

The headline that survives is not the classifier's 80.35\,MHz. It is the fall
\emph{within} the comparable group: from a single signed add to a four-term dot
product, MHz/kLUT drops from 13.9 to 0.43, a factor of $32$. A cost figure for
this format that names no operator is not a cost figure. Our earlier
post-synthesis ratios were quoted for an adder; they do not carry to a datapath,
and this table is why.

\subsection{Silicon refuted a simulation that passed}
\label{sec:silicon-mac}

The signed MAC passed its bench 2/2 in simulation and failed on the die: clause
pattern \texttt{0011}, \texttt{ok=0} (W975). Reproduced off the die in
25\,microseconds once the right stimulus was known (W976). The cause was not in
the multiplier the specification defines but in a \emph{private copy} of it
inside the MAC, which had lost the guard that makes zero an annihilator (W977).
Restoring the guards fixed it, and the defect had been spending roughly 15\,\% of
the design's LUTs on logic it should never have used (W978). An audit of the
whole corpus then closed the class, and in doing so explained an earlier
hardware failure that had been recorded without a board to reproduce it (W979).

A cost model fitted in an earlier revision did not survive contact with these
builds either: the 21\,ms per LUT rule is 2.4$\times$ off and is not linear, so
the place-and-route cap was re-derived rather than re-quoted (W988).

We report this because it is the strongest methodological result in the paper and
it cuts against us: a simulation bench that passes twice is not evidence that the
generated hardware is correct. The two artefacts diverged at exactly the point
where a hand-written copy replaced the specified operator.

\subsection{Thirty-nine reads, and the coverage they do not have}
\label{sec:silicon-reads}

Across eight designs the project now holds 39 die reads, of which 28 pass
(W987, W989). Both spec fixes above were re-confirmed on silicon: the MAC across
four placements and three distinct BSCAN sites returned an identical word
(W990). That is a confirmation, not a proof of placement-independence, and the
recording tool says so in its own output rather than leaving the reader to
notice.

Two limits belong beside those numbers. First, 30 of 36 die clauses had no
same-named test (W980): the die was being asked questions the test suite did not
ask. Second, the reads were not machine-locatable until they were canonicalised
--- a referee re-deriving our table could reach only 10 of 38, now 39 of 39
(W989). A number that cannot be found is not reproducible, whatever its value.
An anomaly chased across six waves stopped reproducing after eight placements
passed in a row (W985). Nine die reads had already refuted the site hypothesis
and shown that a planned FASM diff could not decide it (W981); a 430-LUT
reproducer and a SAT proof on the mapped netlist narrowed it, while a third
method failed its own control and was discarded rather than reported (W982).
Half of the on-die clauses turned out to be folded constants, which also refuted
the operand-order framing (W983), and once the clauses were made real the
multiplier read correctly (W984). The frozen reproducer still splits 2/2, so the
anomaly belongs to the folded design and not to the bench (W986).

\subsection{At matched physical width the ladder is behind}
\label{sec:silicon-w991}

The comparison in Table~\ref{tab:physicalwidth} is the one we would rather not
publish, and it is computed from the committed oracles (W991).

TNF's named rungs do not occupy their named widths. The \emph{physical} widths
are 6, 10, 19, 36 and 69 bits for TNF4 through TNF64. Keyed on physical width,
against posit at $\mathrm{es}=1$:

\begin{table}[t]\centering
\caption{At every matched physical width the ladder occupies, TNF represents
fewer values than posit and steps more coarsely at $1.0$ (W991).}
\label{tab:physicalwidth}
\begin{tabular}{lrrrrr}
\hline
width & TNF values & posit values & TNF step & posit step & ratio \\
\hline
19 bits & 516{,}096 & 524{,}286 & 0.024\,\% & 0.002\,\% & $12\times$ \\
10 bits & 960 & 1{,}022 & 3.125\,\% & 0.781\,\% & $4\times$ \\
6 bits & 56 & 62 & 25\,\% & 12.5\,\% & $2\times$ \\
\hline
\end{tabular}
\end{table}

The cause is structural rather than an encoding accident: a ternary exponent does
not tile a binary field. Four trits need seven bits and use 81 of 128 codes, so
8{,}190 of the $2^{19}$ codes at the 16-rung are unreachable.

This does not overturn the paper's case, and we are careful not to claim that it
does in either direction. Nothing in this table is a cost figure, and TNF's
argument was never precision-per-bit; it is what the lattice costs in cells,
which lives in the area records. A format can lose every column here and still
win a design, and the reverse. But the earlier framing invited the reader to
compare at the \emph{named} width, where the ladder looks even, and at the width
the hardware actually pays for it does not.
