...
\begin{corollary}[Scale resolution pays until the ladder resolves one arc]
\label{cor:designrule}
For a block format with element codebook $\mathcal{C}$ and corner density $D$
per octave, refining the shared-scale ladder beyond roughly $D$ points per binade
buys the $O(h^{2})$ tail of Theorem~\ref{thm:latticeexp}(i) rather than the
$O(h)$ regime, so the return per doubling falls from about $50\%$ to about $25\%$
of the previous doubling. $D$ is computable from $\mathcal{C}$ and the weight
distribution alone. \emph{The operating point of the scale field is therefore set
by the element codebook, and can be read off before any quantisation is run.}
\end{corollary}
