...
Among the twenty-five carrying their own range, GFTernary leads throughput per area by $10.9\%$ --- at two bits, where width confounds the comparison. At \emph{equal} storage a guarded sixteen-bit rung beats IEEE half by $6.92\%$ ($p<10^{-20}$, fifty-four seeds).  On an XC7A200T a ternary neuron
costs $28$ LUT per weight and \emph{no DSP at any fan-in}.

Which rung follows from the budget --- $\varphi$ at four bits, $r^5{=}r^3{+}1$ at
five, $r^6{=}r{+}1$ at six ---  On the block axis the gain is scale \emph{resolution}: at $4.25$ bits per element a finer ladder beats MXFP4 by $17.1\%$ RMSE on two models, zero losing blocks in $14.9$M, and $9.0\%$ perplexity against a best-encoded baseline --- and a binary ladder realises it better than a ternary one.

 Thirteen retractions are marked in place below.
\end{abstract}
...
Thirteen of our own claims were falsified by the measurements meant to support them,
...
than twenty-eight. Thirteen claims of ours are retracted in place below, and the
