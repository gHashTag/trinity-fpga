...
\texttt{takum} reaches $\pm 599$ binades at both widths compared and never leaves
...
Twenty of our own claims were falsified or narrowed by the measurements meant to
support them, and each is retracted where it was made rather than quietly
dropped; Section~\ref{sec:selfcheck} lists all twenty with their locations so the
count can be checked against the text.
...
\caption{Canon plate. What the present paper inherits: the field-layout rule and its nine reproduced widths from the GoldenFloat preprint~\cite{goldenfloat}, the bit-exact conformance ledger from the catalog preprint~\cite{catalogue} --- whose title says 84 where the shipped count is 83 --- and the three-site node argument that is new here.}
...
$3^{E_t-1}$ positive and the same number of negative nonzero states, giving
$2\cdot3^{E_t-1}$ binades in the stated convention.  A tapered code instead
...
\end{corollary}
\begin{proof}
Theorem~\ref{thm:posit} changes the effective mantissa at equally spaced
thresholds, so its finite-difference rate is constant between the stated
thresholds.  Theorem~\ref{thm:takum} instead changes it when $|e|+1$ crosses
$1,2,4,\ldots$, whose gaps grow geometrically.  Enlarging a finite fitting
window therefore changes the fraction of the window occupied by each jump and
changes its fitted slope, even though the encoder is unchanged.  A smooth
logarithmic fit consequently averages over jumps rather than representing the
one-bit discontinuities themselves, which proves the warning.
\end{proof}
...
geometric  & $-1$ per doubling             & $7$  & takum $\times 4$, tekum $\times 3$ \\
\bottomrule
\end{tabular}
\end{table}
...
$b$, choose an integer $E_t\ge2$ with $3^{E_t}-1\ge b$ and set
...
range $(3^{E_t}-1)\log_{10}2$ in decades. Every rung is shown at the width
...
\caption{Canon plate. The regime field as a prefix code, as this section develops it: the unary and logarithmic staircases, and the Kraft floor no format of unbounded range can taper beneath.}
...
\caption{Canon plate. Converting fabric area into precision, as this section develops it: the marginal area of a mantissa bit, the exchange rate it defines, and the resulting bill for the regime field.}
...
\caption{Canon plate. The value law costs more than the field layout, as this section shows: an exponent read where it lies, the same law decoded through memory, and the published all-logic form given alongside it.}
...
\begin{table}[t]\centering
\caption{One ternary neuron, matched width against matched width, every
...
dynamic range $3^{E_t}-1$ binades, so the members are totally ordered by $E_t$ and
...
\begin{sloppypar}
\begin{proof}
Apply Theorem~\ref{thm:coverage} to each row of Table~\ref{tab:nosurvivors},
using the row's measured $(m,b)$ pair rather than a fitted summary.  The
finite enumeration has $7+7+9+4+1=28$ rows; its maximal required exponent
budgets are witnessed respectively by
$(N,E_t,M)=(8,4,3),(16,6,9),(32,5,26),(64,10,53),(128,10,117)$.
The width and range checks are exact:
$1+E_t+M=N$ and the corresponding ranges are
$80,728,242,59048,59048$ binades.  Every other row in its width class has
no larger requirement by the same enumeration, and the direct coordinate
comparison records a strict improvement in at least one coordinate for all
twenty-eight rows.  The coverage theorem therefore supplies a dominating
member for every listed candidate; the conclusion is only about this finite
position-budget table.
\end{proof}
\end{sloppypar}
...
\[
  E_t^{*} = \max\!\left\{2,\,\bigl\lceil \log_3 (b+1) \bigr\rceil\right\},
  \qquad
  M^{*} = N - 1 - E_t^{*} .
\]
\end{theorem}
...
constrains only how many binades it must hold, so it requires $3^{E_t}-1 \ge b$,
i.e.\ $E_t \ge \log_3(b+1)$. The family definition also imposes
$E_t\ge2$, hence every admissible member has $E_t\ge E_t^{*}$. By the width rule
$M = N-1-E_t$ is strictly decreasing in $E_t$, so the constraint binds at
$E_t^{*}$ (provided that member exists) and the minimiser is unique.
\end{proof}
...
exponent width is
\[
  E_t^{\dagger} = \max\!\left\{2,\,\bigl\lceil
  \log_3\bigl(2\max(|e_{\min}|,|e_{\max}|)+3\bigr) \bigr\rceil\right\}
  \;\ge\; E_t^{*},
\]
...
which rearranges to $3^{E_t} \ge 2\max(|e_{\min}|,|e_{\max}|)+3$.
Taking the ceiling and imposing the family constraint $E_t\ge2$ gives
$E_t^{\dagger}$. Since
$\max(|e_{\min}|,|e_{\max}|) \ge (b-1)/2$ we have $2\max+3 \ge b+2 > b+1$, so
$E_t^{\dagger} \ge E_t^{*}$. For a span symmetric about unity,
$e \in \{-k,\dots,k\}$, we have $b=2k+1$ and the two ceilings are
$\lceil\log_3(2k+2)\rceil$ and $\lceil\log_3(2k+3)\rceil$, which differ only if
$2k+2$ is a power of three; no power of three is even, so they agree. For any
...
binade, and let the workload visit $b$ binades. Then the smallest admissible
 exponent width is $\max\{2,\lceil \log_3(2b+1) \rceil\}$; it never falls below
$E_t^{*}$ and exceeds it for $136$ of the first $200$ values of $b$. The positive half of the
...
$\Delta = (3^{E_t}-1)/2$, that is $3^{E_t} \ge 2b+1$. Taking the ceiling and
imposing $E_t\ge2$ gives the stated width.
Since $2b+1 \ge b+1$ for $b \ge 0$ and the ceiling of $\log_3$ is non-decreasing,
the width never falls below $E_t^{*} =
\max\{2,\lceil\log_3(b+1)\rceil\}$; that it strictly exceeds it for $136$ of
$b = 1 \ldots 200$ is by enumeration. The indices
...
\[
  R =
  \begin{cases}
    E_t^{*}(b') - E_t^{*}(b) \ \text{mantissa bits}, & b' > b,\\[2pt]
...
$E_t^{*}(b) = \max\{2,\lceil\log_3(b+1)\rceil\}$, and the width rule $1 + E_t + M = N$ fixes
...
in it overflows. The ratio $\log_3(b'+1)/\log_3(b+1)$ is the share of the
\emph{trit budget}, not of the workload measure --- at $b = 80$, $b' = 26$ the two
read $0.25$ and $0.675$ --- and an earlier revision of this theorem stated the
...
A recent controlled pretraining study on native MXFP4 hardware adds a boundary
that our inference-only measurements do not cover: quantising weight gradients,
rather than forward propagation or activation gradients, was the main source of
convergence degradation in the reported Llama~3.1--8B experiments
~\cite{cim2026mxfp4}. Deterministic Hadamard rotations restored stable
optimisation in that study, whereas stochastic rounding alone did not. We
therefore do not transfer our inference results to low-precision training, and
we treat gradient-path behaviour as an open downstream axis rather than as
evidence for or against TNF.
...
\end{corollary}
\begin{proof}
Let $b_g$ be the binade span seen by one assigned group and $b$ the span of
the unpartitioned tensor.  Since the group range is a subrange, $b_g\leq b$,
and Theorem~\ref{thm:optimal} gives
$E_t^{*}(b_g)\leq E_t^{*}(b)$ because
$E_t^{*}(u)=\max\{2,\lceil\log_3(u+1)\rceil\}$ is non-decreasing.
At fixed word width, each trit saved in the exponent is one mantissa position
by the width rule.  Thus per-group assignment cannot increase the exponent
budget and can return the difference to the fixed field; a tapered code pays
the corresponding surplus as the over-sizing branch of
Theorem~\ref{thm:regret}.  This establishes the direction of the effect, but
not its magnitude, which remains workload-dependent.
\end{proof}
...
a depth $d^{*}$ at which APoT costs what it was introduced to avoid. In the
post-synthesis estimate of \texttt{research/block/SCALE\allowbreak\_FRONTIER\allowbreak\_2026-08-10.md}
(yosys, XC7A200T primitives, DSP inference off; no place-and-route was run) at
$W=32$, $n=2$: $d^{*}=2$, where APoT-8 costs 1217 LUTs against 1215.
\end{corollary}
\begin{proof}
For a runtime-variable scale, Theorem~\ref{thm:shifter} gives one
$\Theta(W\log W)$ barrel path per term, so the term count
$n^{d+1}$ makes the APoT cost increase with $d$ while the multiplier cost is
independent of $d$.  In the stated post-synthesis estimate the areas for
$d=0,1,2$ are $384$, $659$, and $1217$ LUTs, respectively, against $1215$ LUTs
for the multiplier.  The first two are below the multiplier and the third is
above it; hence the first crossing is $d^{*}=2$.  The crossing depth is a
property of that cost estimate, not of routed hardware.
\end{proof}
...
\end{theorem}
\begin{proof}
Read the three measured points in Table~\ref{tab:scalefrontier} with smaller
coordinates preferred.  Their areas satisfy
$173<384<1215$, while their excess errors satisfy
$0<0.1651<2.4420$ percent.  The multiplier and APoT-2 both take one cycle,
whereas $\varphi^k$ takes $k\geq1$ cycles.  Thus the multiplier cannot dominate
either alternative because its area is larger; $\varphi^k$ cannot dominate
either alternative because its error is larger; and APoT-2 cannot dominate
either neighbour because it is worse than the multiplier in error and worse
than $\varphi^k$ in area.  The three points are therefore pairwise
non-dominating.  When the scale is frozen, the stated dominance follows instead
from Theorem~\ref{thm:unrolled}.
\end{proof}
...
\end{corollary}
\begin{proof}
By Theorem~\ref{thm:reqmul}, the runtime multiplicative factors are restricted to
powers of the alphabet base, while the inner product still requires addition for
each accumulated term. LNS supplies general multiplication rather than only
those powers, so part of its operation set is unused under the stated datapath
model and its additions remain on the accumulation path. Fixed point supplies
addition but has no free operation for a runtime scale factor; that factor is
free only when compilation has frozen it into wiring or constants.
\end{proof}
...
from a single placement run and is withdrawn. A later sweep says the
withdrawal did not go far enough: the placer and router \emph{configuration}
moves the ordering, not only the magnitudes. Twenty-one decoder designs were
routed through three configurations --- \texttt{heap/router1},
\texttt{heap/router2}, \texttt{sa/router2} --- at five seeds each, and of the
$210$ design pairs, $25$ do not return the same verdict across the seeds of one
configuration, with median margins reaching $17.3\%$
(\texttt{measurements/verdict\allowbreak\_stability\allowbreak\_2026-08-19.json},
\texttt{measurements/placer\allowbreak\_router\allowbreak\_full\allowbreak\_2026-08-19.json}).
The flow keeps the first configuration that routes, so a published median is a
median over seeds \emph{within} whichever configuration happened to succeed. Area
behaves differently from frequency here: on one cost arm
(\texttt{tnf\allowbreak\_cost\allowbreak\_e2m11\allowbreak\_add\allowbreak\_top})
the LUT count is identical at all five seeds --- $467$ --- while $F_{\max}$ spans
$379.65$ to $422.65$\,MHz, a spread of $10.5\%$, with a nextpnr log per seed and
its SHA-256 recorded
(\texttt{measurements/pnr\allowbreak\_seed\allowbreak\_sweep\allowbreak\_2026-08-19.json},
\texttt{measurements/pnr\allowbreak\_logs/}). These runs target the openXC7
chipdb \texttt{xc7a200tfbg676-1}: an XC7A200T die in the FGG676 package, which is
a combination none of our boards carries --- the ALINX AX7203 is an XC7A200T in
FBG484 at speed grade $-2$ (IDCODE \texttt{0x13636093}) and the QMTech Wukong is
an XC7A100T in FGG676 (IDCODE \texttt{0x13631093}) --- and the CI sweep routes
\texttt{xc7a200tfbg484-2}, a different package \emph{and} a different speed
grade. So these are post-route figures for a fabric model, not measurements of a
board, and they are not comparable to the CI's. What they do establish is
methodological and it is enough for the claim made here: no throughput ordering
in this paper is offered as separating two formats. \texttt{int8} is excluded from that table even so, and the
...
\label{thm:packedcov}
A packed format with $E_t$ trits of scale and $m$ bits of significand, realisable
...
\[
  m + \max\!\left\{2,\,\lceil \log_3 (b+1) \rceil\right\}
  \;\le\; N-1 .
\]
\end{theorem}
\begin{proof}
A member $E_t$ has mantissa $N-1-E_t$ and range $3^{E_t}-1$. Domination requires
$N-1-E_t \ge m$ and $3^{E_t}-1 \ge b$. Together with the family constraint
$E_t\ge2$, this is
$\max\{2,\lceil\log_3(b+1)\rceil\}\le E_t\le N-1-m$.
Such an integer exists exactly when the stated inequality holds.
\end{proof}
...
\label{cor:slack}
Every uniform binary format of width $N$ with at least two exponent positions is
dominated by a member of $\mathcal{F}_N$. The slack has two readings and they are not equal. In real
...
domination holds for every $E \ge 2$, but with a strictly positive margin only from
$E = 3$; the linear growth in the competitor's exponent width is a property of
$\Delta$, not of $\Delta_{\mathbb{Z}}$.
\end{corollary}
\begin{proof}
By Theorem~\ref{thm:budget}, $b+1 \le 2^E$ and $m \le N-1-E$. For $E\ge2$,
$\max\{2,\lceil\log_3(b+1)\rceil\}\le \lceil E\log_3 2\rceil$. Hence
$m + \max\{2,\lceil\log_3(b+1)\rceil\} \le (N-1-E) + \lceil E\log_3 2\rceil
...
A nearby threat is a different kind of composition: signed-digit lookup-table
inference can carry the ternary representation into the attention K/V cache and
co-design the cache layout with the execution datapath
\cite{duan2026lut}. This is not evidence that a particular numeric format wins:
it is a downstream and hardware-mapping system whose result must be compared at
the task output and at a declared implementation cost. TNF's measurements in
this section therefore do not absorb its cache mechanism, and its existence
removes any broad claim that a ternary datapath alone defines the relevant
frontier.
...
\paragraph{And what it is not.} It is not a format claim for a datapath that
...
reaches $\pm 599$ binades at both widths we compared
...
\end{theorem}
\begin{proof}
Model the delivery chain as a directed graph whose vertices are artefacts and
whose edges are reads performed by a checker.  A consistency assertion can be
evaluated only for a pair connected through such reads; otherwise the checker
has no value against which to compare.  A document absent from the tool's input
graph therefore contributes no checked edge, even when it repeats a value from
code.  The assertion follows: a document--artefact pair is guarded only after
the document is made an explicit input or is checked by a separate audit.
\end{proof}
...
and synthesis~\cite{cheng2026hint}. A further recent pipeline combines formal
exploration with neuro-symbolic refinement to generate and repair RTL assertions
under coverage feedback, reporting roughly twofold gains in assertion count and
mutation coverage over traditional mining~\cite{yan2026neuroassertion}. This
further narrows the distinction between formal coverage and a conformance
workflow, but it remains a verification workflow rather than a semantic proof of
this numeric field. Against
...
layer a format governs is the binding one. A fresh cross-kernel study makes the
same boundary concrete: with the checkpoint, prompts, engine and quantisation
held fixed, replacing an INT8 GEMM kernel changed end-to-end outputs, while
identical operands with power-of-two scales matched and the remaining
differences were localised to scale application and output rounding
~\cite{chen2026integeralibi}. This is a threat to any claim that an exact
accumulator alone closes the reproducibility contract; it supports the narrower
claim made here, that the accumulator convention and the scale/rounding law
must travel with the format. The first layer is also acquiring load-bearing
...
either format.
Jokar's survey makes the same methodological point from the transform side:
the useful direction depends jointly on the allocation regime and the number
format, and optimality for one objective does not transfer to another
\cite{jokar2026greatinversion}.  We therefore treat the selector's workload,
format and objective as a single stated contract, not as a property of a format
in isolation.
The companion study by Chen sharpens the conformance boundary: a tolerance of
one bfloat16 spacing can remain blind to several epilogue faults, whereas
bitwise agreement becomes reproducible only after the scale and requantisation
contract is stated explicitly \cite{chen2026deterministic}.  We therefore do not
count tolerance agreement as bit-exact evidence; the TNF vectors specify the
rounding, reduction order and boundary cases that the check actually covers.
...
\end{observation}
\begin{proof}
A field with $t$ ternary positions has $3^t$ states and therefore requires
$\lceil\log_2(3^t)\rceil=\lceil t\log_2 3\rceil$ binary storage bits.  Charging
the same field as $t$ cells leaves exactly
$\lceil t\log_2 3\rceil-t$ uncharged cells.  Substitution gives
$\lceil4\log_2 3\rceil-4=7-4=3$ and
$\lceil11\log_2 3\rceil-11=18-11=7$.
\end{proof}
...
unscaled attention scores, and the widest spread of any row is under twelve
binades, so $D$ stays far below every threshold in Table~\ref{tab:rungthr}. The
...
$\pm 599$ --- $376$ of $400$ samples fall outside it at both rungs. This is
...
earlier. The smallest positive and largest finite magnitudes of \texttt{takum} are
$3.45 \times 10^{-77}$ and $2.07 \times 10^{180}$, a reach of about $\pm 599$
binades, and they are \emph{identical} for \texttt{takum16} and \texttt{takum32}:
the width buys precision, not reach. Against that, the $(4,8)$ rung reaches
$\pm 39$ binades and the $(5,23)$ rung $\pm 120$.
...
\texttt{takum} go out of range where TNF does not. And the earlier assumption we
worked from, that \texttt{takum} reach is about $\pm 255$ binades and scales with
width, was simply wrong --- it was inferred from a bias constant instead of
measured. We record it here because a wrong number that was never printed is
still a wrong number that shaped a design decision.
...
task-level probes and measured the tasks' results.
...
\subsection{A summation stress task: naive versus Kahan}
\label{sec:downstream-kahan}

We added a third task-level probe whose output is a finite sum rather than a
representation error or a solver residual.  The input sequence is
\[
  [2^{24}] + [1]^{100000} + [-2^{24}],
\]
whose exact sum is $100000$.  Each term is encoded and decoded by the candidate
format before it reaches the accumulator.  Both the naive and Kahan reductions
use a \texttt{float32} accumulator for every addition; only the exact reference
uses \texttt{float64}.  The order is fixed, and no timing, area or energy
measurement is inferred from the experiment.

\begin{table}[t]\centering\small
\caption{Final value of the cancellation sum and absolute error against the
exact result $\mathbf{100000}$ \textbf{[measured --- software]}.  The experiment
tests the numerical task and the reduction algorithm; it is not a ranking of
formats.}
\label{tab:downstream-kahan}
\begin{tabular}{llrr}
\toprule
format & reduction & final sum & absolute error \\
\midrule
\texttt{binary32} & naive & $0$ & $100000$ \\
\texttt{binary32} & Kahan & $100000$ & $0$ \\
\textbf{TNF$(4,8)$} & naive & $0$ & $100000$ \\
\textbf{TNF$(4,8)$} & Kahan & $100000$ & $0$ \\
\texttt{takum16} & naive & $0$ & $100000$ \\
\texttt{takum16} & Kahan & $100000$ & $0$ \\
\bottomrule
\end{tabular}
\end{table}

For this fixed sequence, naive \texttt{float32} addition loses every unit
increment while the partial sum is at $2^{24}$; Kahan's compensation retains
those increments and restores the exact task result.  The same pair of outcomes
appears for all three candidate encodings because $2^{24}$, $1$ and $-2^{24}$
are exactly representable by the tested formats.  This is therefore evidence
about the reduction algorithm under a stated accumulator contract, not evidence
that TNF is more accurate than either comparison format.  It also gives a
negative control for the representation--computation objection: a change in
the numerical procedure can dominate the format on a task output.

\begin{sloppypar}
The generator
\texttt{measurements/}\allowbreak\texttt{gen\_downstream\_kahan.py} and its
result record
\texttt{measurements/}\allowbreak\texttt{tnf\_downstream\_kahan\_}\allowbreak
\texttt{2026-09-01.json}
preserve the
sequence, accumulator rule and exact output.  This is one deterministic
stress task, so it does not establish performance across summation orders,
distributions or applications.
\end{sloppypar}

\paragraph{A third task attempted, and a metric we disqualified.} We also ran the
logistic map $x \mapsto 3.9\,x(1-x)$ for $400$ steps. The candidate state was
stored after every step; multiplication and subtraction used a \texttt{float32}
accumulator, while the \texttt{float64} trajectory was only the reference. All
trajectories stayed in $[0,1]$. We record the result because it is an independent
task-level control, not because its departure time is a valid format ranking.

\begin{table}[t]\centering\small
\caption{Chaotic-map control: first step at which the candidate differs from the
\texttt{float64} trajectory by more than $10^{-2}$, and mean absolute error over
the first 20 steps \textbf{[measured --- software]}. The departure-time column is
diagnostic only and is not used to rank precision.}
\label{tab:downstream-logistic}
\begin{tabular}{lrr}
\toprule
format & first step $>10^{-2}$ & mean abs. error, steps 1--20 \\
\midrule
\texttt{binary16} & 16 & $1.5003\times10^{-2}$ \\
\textbf{TNF$(4,8)$} & 5 & $1.0298\times10^{-1}$ \\
\texttt{takum16} & 11 & $8.8072\times10^{-2}$ \\
\bottomrule
\end{tabular}
\end{table}

The apparent ordering is not a precision ordering: TNF$(4,8)$ departs earliest
even though the departure time in a positive-Lyapunov map depends on the sign and
direction of the first rounding error, not only on its magnitude. We therefore
draw no superiority claim from this table. The run remains useful as a negative
control and as a check that a task metric can fail its own validation; the
reproducible record is
\texttt{measurements/tnf\_downstream\_logistic\_2026-08-14.json}.
...
TNF's own. What is visible in that source, and what we report as structure
...
\item \textbf{The range is bounded, and it is not a takum advantage.} The measured
TNF rungs used here reach about $\pm39$ and $\pm120$ binades for $(E_t,M)=(4,8)$
and $(5,23)$, respectively, whereas both \texttt{takum16} and \texttt{takum32}
reach about $\pm599$ binades. Fixed fields therefore buy a simple datapath and a
uniform mantissa at the cost of range; they do not provide range reserve over
\texttt{takum}. Workloads outside a rung's measured range require a different rung
or a tapered format.
...
\bibitem{cim2026mxfp4} M.~Cim, S.~Arora, P.~Palangappa, M.~Hodak,
R.~Dwivedula, M.~Arunachalam and M.~T.~Kandemir, ``Pretraining large language
models with MXFP4 on Native FP4 Hardware,'' arXiv:2605.09825, 2026.
...
\bibitem{jokar2026greatinversion} E.~Jokar, ``Transforms for LLM Quantization:
The Great Inversion and Format Co-Design,'' arXiv:2608.25188, 25~August 2026.
...
\bibitem{chen2026integeralibi} T.-R.~Chen, ``The Integer Alibi: Localizing Cross-Kernel Divergence in INT8-Quantized LLM Inference,'' arXiv:2608.13756, 13~August 2026. \url{https://arxiv.org/abs/2608.13756}

\bibitem{chen2026deterministic} T.-R.~Chen, ``Deterministic LLM Inference Across GPU Kernels: Power-of-Two INT8 Quantization Scales and the Limits of Tolerance-Based Conformance,'' arXiv:2609.00363, 25~August 2026. \url{https://arxiv.org/abs/2609.00363}
...
\bibitem{yan2026neuroassertion} Z.~Yan, Z.~Zheng and H.~Zhang, ``Coverage-Driven RTL Assertion Generation with Formal Exploration and Neuro-Symbolic Refinement,'' arXiv:2608.18482, 19~August 2026. \url{https://arxiv.org/abs/2608.18482}
...
\bibitem{duan2026lut} Z.~Duan, J.~Wu, Z.~Chen, H.~Song, Y.~Deng, Z.~Shen,
N.~Xie, S.~Wu, B.~Wang, P.~Zhou and C.~Wang, ``Unified Lookup-Table Inference
with Signed-Digit K/V Caches for Ternary LLMs,'' arXiv:2608.03229, 4~August
2026. \url{https://arxiv.org/abs/2608.03229}
