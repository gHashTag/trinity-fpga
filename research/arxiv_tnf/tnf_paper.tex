...
% A live utilisation claim carries the format it is about, so
% tools/check_codespace_claims.py can verify it against code_use.json
% instead of guessing which format a percentage in prose refers to.
\newcommand{\codeuse}[2]{#2\%}

% A statement containing "we did not search for" is an observation, not a
% theorem. The block-axis statement was inside \begin{theorem} while its own
% next paragraph showed the ranking inverts, which is the opposite of a bound.
...
The weight is a code: multiplying by it is a sign-select.  The accumulator alone has a range to spend, and Ternary Network
...
where the multiplier is decided. Removing a multiplier rather than
cheapening it requires the weight lattice to be \emph{closed}. $\mathbb{Z}[\varphi]$ is, because $\varphi^2=\varphi+1$:
...
has entries in $\{0,\pm1\}$: degree two admits $\varphi$ alone, and $r^d=r+1$
reaches any granularity at \emph{one addition} and $d$ registers --- fineness
costs registers, not adders.

Among the twenty-five carrying their own range, GFTernary leads throughput per area by $10.9\%$ --- at two bits, where width confounds it. At \emph{equal} storage a guarded sixteen-bit rung beats IEEE half by $6.92\%$ ($p<10^{-20}$, 54 seeds).  On an XC7A200T a ternary neuron
costs $28$ LUT per weight and \emph{no DSP at any fan-in}.

Which rung follows from the budget: $\varphi$ at four bits, $r^5{=}r^3{+}1$ at
five, $r^6{=}r{+}1$ at six. On the block axis the gain is scale \emph{resolution}: against MXFP4's \emph{own best encoder}, $14.1\%$ RMSE and $7.2$--$12.4\%$ perplexity on four models, zero losing blocks in $14.9$M --- and a binary ladder realises it better than a ternary one. Squared error transfers across models to $1.04\times$, perplexity only to $3.00\times$: the perturbation is constant to $1.02\times$, so what varies is sensitivity.

 Sixteen retractions are marked in place below.
\end{abstract}
...
Sixteen of our own claims were falsified by the measurements meant to support them,
and each is retracted where it was made rather than quietly dropped. The sixth,
Section~\ref{sec:wellposed}, accounts for four published disagreement figures at
once, because they turned out to share one cause: the comparison, not the thing
compared. The last four were found by an independent review rather than by us: a caveat
this paper computed rather than measured, a sentence defending our own first
place, a ranking never tested against its own instrument's resolution, and four
ladder rungs whose exponents alias.
...
\paragraph{And then the paper turns on itself.} Everything above was written
before the campaign that produced the second half, and the second half is not
more results of the same kind --- it is what happened when the instruments were
audited. Sections~\ref{sec:wellposed} to~\ref{sec:selfcheck} record it, and the
shape is worth stating in advance because it recurs.

\emph{The measurement was almost always right and the comparison around it was
almost always wrong.} A decoder emitting \texttt{fp32} was compared against a
real-valued reference by exact equality, which reports rounding as error
(Section~\ref{sec:wellposed}); two formats publishing two variants were each
compared against the other one's; two rows of the throughput table were measured
on a harness $64$ LUT more expensive than the nineteen they were ranked against
(Section~\ref{sec:bands}); and the ranking itself had never been tested against
the noise of the instrument that produced it, which resolves seven levels rather
than twenty-eight. Sixteen claims of ours are retracted in place below, and the
count is the point rather than an embarrassment: a table that has survived being
audited is worth more than one that has not been.

Three results came out of the audit that could not have come out of the
measurements alone. Section~\ref{sec:tiling} proves that a ternary exponent
never tiles a binary field --- $3^{k}\neq 2^{m}$ --- and turns the waste into a
closed form whose minimum picks the rung; Section~\ref{sec:window} shows the same
rung is the widest an \texttt{fp32} decoder can carry without its exponent
wrapping, so two unrelated constraints land on $E_t=5$; and
Section~\ref{sec:sixteen} builds that rung, guards every code outside its
specification, and measures it against IEEE half at equal storage on fifty-four
placement seeds.

\paragraph{The block axis, last, because it was the rule we set ourselves.}
This work would not publish on the block axis until it beat a deployed format
there by measurement. Section~\ref{sec:blockmeasured} reports that measurement
--- and reports, in the same breath, that the ternary reading of the scale field
is not why it wins, that a plain binary ladder wins by more, that about a fifth
of the improvement is an encoder change belonging to the baseline, and that once
the appliers are placed in silicon the cheapest arm is $3.6$ times more efficient
per LUT than the best ladder. \emph{The result stands and the story we expected
does not}, which is the same sentence the audit sections keep producing and the
reason they are in the paper rather than in a notebook.
...
\begin{tabular}{rrrrr}
\toprule
$E_t$ & values $3^{E_t}$ &b & capacity & efficiency \\
\midrule
$3$  & $27$    & $5$  & $32$   & $84.4\%$ \\
...
numbers. An earlier version reported $440$ against $895$ LUTs, a $5.1\times$
ratio, which handed TNF pre-widened fields while the competitors unpacked
...
\midrule
8b & \textbf{TNF8} & $\mathbf{609}$ & $63.67$\,MHz & $\mathbf{0.1045}$ & $\mathbf{1.42\times}$ \\
8b & posit8 & $614$ & $45.30$\,MHz & $0.0738$ & --- \\
\addlinespace
16b & \textbf{TNF16} & $\mathbf{629}$ & $65.17$\,MHz & $\mathbf{0.1036}$ & $\mathbf{2.11\times}$ \\
16b & posit16 & $731$ & $35.95$\,MHz & $0.0492$ & --- \\
\bottomrule
\end{tabular}
\end{table}

TNF16 beats posit16 by $1.26\times$ in area and $1.64\times$ in frequency, so $2.07\times$ in throughput per LUT --- measured end to end rather than assembled from separate decoder and adder figures. The decomposition says why: against a $1{,}477$-LUT multiplier and a $384$-LUT adder, posit's $480$-LUT decoder is $21\%$ of the datapath; with the multiplier gone it is $56\%$. For takum the same shift takes $85\%$ to $97\%$, and
...
each rung is measured at its specified parameters. Against \texttt{int8} the ladder rungs are not competitive on this datapath, and the row that comes closest is GFTernary, which is not a rung (Section~\ref{sec:blockrelated}). The two are consistent: the frontier asks what a word of
...
This is why the silicon tables carry TNF rows and no separate GF-T ones: at
...
\paragraph{The theorem checked against the silicon.} A theorem about a lattice
says nothing about whether the RTL implements the lattice, and six modules in
this work turned out not to implement the format they were labelled with. So the
synthesised node is compared against exact integer arithmetic in
$\mathbb{Z}[\varphi]$, with no float anywhere in the comparison: the datapath's
pair $(a,b)$ against the reference's, as integers.

Sixteen thousand vectors at fan-in $8$, $16$ and $32$, eight-bit samples,
weights drawn uniformly from the alphabet: \textbf{zero disagreements}. Depth is
checked the same way, since the claim is about depth and not only about a layer
--- one step fed back on itself is $k$ stacked layers, and after $k$ clocks the
pair must be $(F_{k-1},F_k)$. Thirty depths, zero disagreements; at $k=30$ the
gain is exactly $(514229,\,832040)$, two integers, with no multiplication
anywhere on the path.

The first run disagreed on $3{,}601$ of $4{,}000$, in the $b$ component only
with $a$ always agreeing --- the signature of a convention difference rather than
an arithmetic one, and it was the reference that had the encoding wrong. The
shape of a disagreement says which side to suspect: all-wrong-in-one-component
is a convention, wrong-at-the-extremes is a missing edge case, scattered is
arithmetic.
...
$0.9\%$, and every ladder that beats MXFP4 on the first loses on the second. \emph{The fifteenfold figure has not been reproduced by an independent instrument, which measures $1.18\times$ on the same windows; until the harness is published this column should be read as unconfirmed.}}
...
\begin{observation}[The block axis is not an element-format problem]\label{thm:block}
On a short block with a shared scale, the element codebook minimising squared quantisation error is not the one minimising perplexity: the squared-error optimum scores $22.2976$ against a deployed MXFP4's $22.4998$, a gap of $0.9\%$ on SmolLM2-135M. We did not search for a perplexity-optimal codebook and state no bound on one; what is measured is that on this axis squared error does not predict perplexity, so element-format design guided by squared error has no demonstrated headroom here.
\end{observation}
...
models. Section~\ref{sec:blockmeasured} measures that comparison again at \emph{equal} bits and finds the ordering is step size rather than algebra: $2^{k/8}$ beats $\varphi^{k}$ by more than $\varphi^{k}$ beats $2^{k}$. What survives here is that $\varphi^{k}$ is the finest grid available \emph{without a multiplier}, which is a different and narrower claim than the one this paragraph originally made. It is not a trade.
...
\paragraph{The same question on the larger sample, and the margin nearly vanishes.}
Table~\ref{tab:geoscale} rests on perplexity over $40$ windows. The squared-error
axis carries $14.9$ million blocks, and asking it the same question --- two scale
grids of \emph{identical} resolution, eight points per binade, differing only in
whether those points are placed geometrically ($2^{k/8}$) or linearly in a
mantissa (E4M3, which is NVFP4's grid) --- gives a very different effect size:
...
$2^{k/8}$, geometric & $\mathbf{+19.55\%}$ & $\mathbf{+19.53\%}$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent Both against each model's own best E8M0 encoder. \textbf{Geometric wins
on both models, as Theorem~\ref{thm:geoscale} requires --- by $0.14$ and $0.15$
points, which is $0.7\%$ of the gain the resolution itself buys.} The sign
replicates; the magnitude does not survive contact with the larger sample. We
therefore keep the theorem and withdraw any suggestion that the choice between
these two grids matters in practice: \emph{at fixed resolution the placement of
scale points is worth under one percent of what the resolution is worth}, which
is not a defeat for this paper's central law on this axis but its sharpest
confirmation --- two grids of equal point count agreeing to within $0.7\%$ is
exactly what ``monotone in points per binade and in nothing else'' predicts, and
it is a stronger test of that law than any pair of unequal grids can be.
...
conformance. One ternary-neuron datapath, XC7A200T, no DSP, every accumulator bit observed. Every row is the \emph{mean} of fifty-four placement seeds with a $95\%$ interval, all on the same $128$-bit harness. \texttt{int8} is in
Table~\ref{tab:rejected}. \textbf{Of the three ladder rungs only TNF16 remains here, at its specified parameters}, and it moved: the modules previously measured
...
pricing one of four paths. That $28\%$ figure for IBM hex32 is withdrawn here
along with the $253$ reported for LNS16, for the reason developed in
\S\ref{sec:wellposed}: neither was measuring a decoder. Repaired and
re-measured, IBM hex32 and VAX~F are exact on every comparable code, and both
fell in the table because the logic they had been missing is logic they now pay
for. \texttt{fp8} omitted subnormals entirely. The
...
unsynthesised logic. \textbf{Resolution.} The pooled coefficient of variation across seeds is $3.71\%$, so at fifty-four seeds two rows separate at $\alpha=.05$ only when their MHz/LUT differ by more than $1.43\%$ relative; smaller gaps are not results. The detection floor for a difference in \emph{decoder} cost is $41.3$ LUT, which is $79\%$ of a whole decoder at the median row --- the $472$-LUT harness inflates that floor tenfold. Every row is measured on the same $128$-bit harness; rows that were not have been re-measured or withdrawn.}\label{tab:fullthroughput}
\small
\tiny
\begin{tabular}{rlrrrlll}
\toprule
& format & bits & LUT & MHz/LUT & $\pm95\%$ & status & conformance \\
\midrule
1 & \textbf{GFTernary} & 2 bits & 527 & \textbf{0.1584} & $\pm0.0018$ & ours & all 4 codes \\
2 & binary32 & 32 bits & 536 & 0.1429 & $\pm0.0013$ & --- & identity (exact) \\
3 & fp8 e4m3 & 8 bits & 549 & 0.1225 & $\pm0.0012$ & --- & all 256, 0 wrong \\
4 & \textbf{TNF16a} & 16b & 566 & \textbf{0.1214} & $\pm0.0012$ & legacy & all 65{,}534 \checkmark \\
5 & fp8 e5m2 & 8 bits & 544 & 0.1188 & $\pm0.0011$ & --- & all 256, 0 wrong \\
6 & \textbf{GF10} & 10 bits & 597 & \textbf{0.1172} & $\pm0.0012$ & ours & all 1{,}024 \\
7 & \textbf{TNF17e} & 17b & 596 & \textbf{0.1155} & $\pm0.0013$ & \textbf{rec.} & all 124{,}416 \checkmark \\
8 & \textbf{TNF16c} & 16b & 591 & \textbf{0.1152} & $\pm0.0010$ & \textbf{rec.} & all 62{,}208 \checkmark \\
9 & \textbf{TNF16a-guarded} & 16b & 613 & \textbf{0.1079} & $\pm0.0011$ & control & all 41{,}472 \checkmark \\
10 & binary16 & 16 bits & 586 & 0.1078 & $\pm0.0012$ & --- & all 65{,}536 \\
11 & \textbf{TNF17e-bare} & 17b & 605 & \textbf{0.1072} & $\pm0.0010$ & control & sweep pending \\
12 & minifloat & 8 bits & 622 & 0.1072 & $\pm0.0011$ & --- & all 254 \checkmark \\
13 & \textbf{GF14} & 14 bits & 651 & \textbf{0.1055} & $\pm0.0012$ & ours & all 16{,}384 \\
14 & \textbf{TNF8} & 10 bits & 609 & \textbf{0.1045} & $\pm0.0010$ & legacy & all 1{,}024, 0 wrong \\
15 & \textbf{BNF16} & 17 bits & 635 & \textbf{0.1043} & $\pm0.0011$ & legacy & all 131{,}072, 0 wrong \\
16 & \textbf{TNF16} & 17 bits & 629 & \textbf{0.1036} & $\pm0.0010$ & legacy & all 131{,}072 \\
17 & \textbf{TNF16b} & 16b & 604 & \textbf{0.1022} & $\pm0.0010$ & legacy & all 65{,}534 \checkmark \\
18 & VAX F & 32 bits & 624 & 0.0956 & $\pm0.0011$ & --- & all 40{,}000 \checkmark \\
19 & \textbf{GF+8} & 8b (+2 sel) & 692 & \textbf{0.0893} & $\pm0.0009$ & ours & 1{,}024/1{,}024 \checkmark \\
20 & posit8 & 8 bits & 614 & 0.0738 & $\pm0.0007$ & --- & all 255 \checkmark \\
21 & takum16 & 16 bits & 853 & 0.0708 & $\pm0.0008$ & --- & 632/61{,}505 \\
22 & LNS16 & 16 bits & 722 & 0.0603 & $\pm0.0005$ & --- & 107/65{,}536 \\
23 & posit16 & 16 bits & 731 & 0.0492 & $\pm0.0004$ & --- & all 65{,}535 \checkmark \\
24 & posit32 & 32 bits & 1031 & 0.0310 & $\pm0.0002$ & --- & all 40{,}000 \checkmark \\
25 & IBM hex32 & 32 bits & 1072 & 0.0295 & $\pm0.0002$ & --- & all 40{,}000 \checkmark \\
\bottomrule
\end{tabular}
\end{table}
...
An \texttt{int8} datapath scores $0.1736$\,MHz/LUT on the same $128$-bit harness --- $454$ LUT at $78.83$\,MHz, median of five seeds --- against GFTernary's $0.1584$. That is a lead of $9.6\%$, above the $5.50\%$ five-seed separation threshold, so the difference is real and \texttt{int8} leads. An earlier version reported $0.180$ against $0.181$ and called the two indistinguishable; that was taken on the narrow $64$-bit harness, where GFTernary measures $463$ LUT rather than $527$, and is withdrawn. An earlier version of
...
\section{What the throughput table is actually measuring}
\label{sec:confound}

The row this work had left marked \emph{not swept} turned out to be the most
informative one in the table. \texttt{binary32}'s decoder is

\begin{center}\ttfamily assign fp32\_out = binary32\_in;\end{center}

\noindent
a wire, because \texttt{binary32} \emph{is} \texttt{fp32}. Its conformance is
therefore exact by construction, and its $536$ LUT are the harness alone: the
LFSR, the accumulator, and the output fold, with no decoder in them at all.

That number is a calibration this work had never taken. Every row of the table lies between $527$ and $1{,}072$ LUT, so \emph{the harness is between $50\%$ and $102\%$ of every measurement}, and for the top ten rows it is never below $80\%$.
Two consequences follow, and both are structural rather than fixable by more
seeds.

\begin{proposition}[Harness dominance]
\label{prop:harness}
Let a measurement compose a fixed harness of cost $H$ with a decoder of cost
$D_f$, and report $R_f = M_f/(H+D_f)$. Then
\[
  \frac{\partial R_f}{\partial D_f} \;=\; -\frac{M_f}{(H+D_f)^2},
\]
which tends to $-M_f/H^{2}$ as $D_f/H\to 0$. When $H\gg D_f$ the report varies
almost entirely with $M_f$ and the decoder's cost is compressed out of it. Here $H=536$ and $D_f\in[-9,536]$, with $D_f<115$ for the top ten rows.
\end{proposition}

\begin{proposition}[Output-space pruning]
\label{prop:pruning}
A decoder with $n$ input bits has at most $2^{n}$ distinct outputs. Any logic
observing only those outputs may be specialised to that image, so its synthesised
cost is non-increasing in $n$ independently of the decoder's own complexity.
A comparison across differing $n$ therefore confounds format design with format
width, and only a within-$n$ comparison isolates design.
\end{proposition} This is observability-don't-care minimisation~\cite{savoj1990}, standard in logic synthesis and implemented in the tool used here; we state it because its consequence for cross-width comparison is easy to miss, not because the mechanism is new.

Proposition~\ref{prop:pruning} is not a worry, it is visible in the numbers.
\texttt{GFTernary} takes $2$ input bits and measures $527$ LUT --- \emph{nine
fewer than the bare wire} --- because four distinct outputs let synthesis
specialise the accumulator behind it. Measured alone against a minimal
register-and-fold sink, the bare wire costs $112$ LUT and \texttt{GFTernary}
costs $66$. A format cannot be cheaper than a wire; what is being measured is
the width of its image.

\begin{table}[htbp]
\centering
\vspace{0.6em}
\caption{The same measurements grouped by storage width, which is the only
grouping Proposition~\ref{prop:pruning} leaves valid. Ours in bold.}
\label{tab:bywidth}
\scriptsize
\begin{tabular}{rlrrr}
\toprule
bits & format & LUT & MHz & MHz/LUT \\
\midrule
2 & \textbf{GFTernary} & 527 & 83.49 & 0.1584 \\
8 & fp8 e4m3 & 549 & 67.27 & 0.1225 \\
8 & fp8 e5m2 & 544 & 64.65 & 0.1188 \\
8 & minifloat & 622 & 66.67 & 0.1072 \\
8 & \textbf{GF+8} & 692 & 61.79 & 0.0893 \\
8 & posit8 & 614 & 45.30 & 0.0738 \\
10 & \textbf{GF10} & 597 & 69.95 & 0.1172 \\
10 & \textbf{TNF8} & 609 & 63.67 & 0.1045 \\
14 & \textbf{GF14} & 651 & 68.69 & 0.1055 \\
16 & \textbf{TNF16a} & 566 & 68.69 & 0.1214 \\
16 & \textbf{TNF16c} & 591 & 68.11 & 0.1152 \\
16 & \textbf{TNF16a-guarded} & 613 & 66.14 & 0.1079 \\
16 & binary16 & 586 & 63.16 & 0.1078 \\
16 & \textbf{TNF16b} & 604 & 61.74 & 0.1022 \\
16 & takum16 & 853 & 60.38 & 0.0708 \\
16 & LNS16 & 722 & 43.56 & 0.0603 \\
16 & posit16 & 731 & 35.95 & 0.0492 \\
17 & \textbf{TNF17e} & 596 & 68.85 & 0.1155 \\
17 & \textbf{TNF17e-bare} & 605 & 64.88 & 0.1072 \\
17 & \textbf{BNF16} & 635 & 66.22 & 0.1043 \\
17 & \textbf{TNF16} & 629 & 65.17 & 0.1036 \\
32 & binary32 & 536 & 76.57 & 0.1429 \\
32 & VAX F & 624 & 59.65 & 0.0956 \\
32 & posit32 & 1031 & 31.96 & 0.0310 \\
32 & IBM hex32 & 1072 & 31.59 & 0.0295 \\
\bottomrule
\end{tabular}
\end{table}

Read that way the table says something different from its ranking, and the
difference is worth stating plainly.

\paragraph{Where we meet a competitor at the same width, and where we do not.}
Of the seven width classes, four contain only our formats and one contains only other people's. \emph{Two classes contain both.} At $8$ bits \texttt{GF+8} measures $0.0882$ against \texttt{fp8~e4m3}'s $0.1244$, and we lose by $29.1\%$; at $16$ bits \textbf{TNF16a} measures $0.1233$ against \texttt{binary16}'s $0.1078$, and we win --- the class Section~\ref{sec:sixteen} is about.
That row's conformance column reads \emph{selector live; no reference}, so it is
also the one row of ours that has never been checked against an oracle --- the
loss and the missing check are the same debt.

The nearest honest comparison is $16$ against $17$ bits, one bit apart:

\begin{center}\small
\begin{tabular}{lr}
\toprule
\textbf{BNF16} (17 bits) versus & \\
\midrule
\texttt{binary16} (16 bits) & $-3.1\%$ \\
\texttt{takum16}  (16 bits) & $+57.4\%$ \\
\texttt{LNS16}    (16 bits) & $+79.8\%$ \\
\texttt{posit16}  (16 bits) & $+113.4\%$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent
Against every \emph{research} format at comparable width --- takum, LNS, posit,
the formats this paper is actually arguing with --- our 17-bit design wins by
between a half and slightly over double. Against IEEE half it loses by three
percent, and against IEEE single it cannot win at all, because IEEE single's
decoder is a wire and no decoder beats no decoder.

That is the defensible claim, and it is narrower than the ranking suggests.
\emph{The overall first place belongs to a two-bit format with no same-width
competitor in the catalogue}, and second place belongs to an identity function.
Neither fact was visible while the table was sorted on a single number.


\subsection{Conformance does not certify the specification}
\label{sec:degenerate}

\texttt{GF+8} was the last row of ours without an oracle, and it is the one
class where a format of ours meets a competitor at the same width and loses.
Writing the oracle settled both questions and raised a third that this work had
not previously had to ask.

\texttt{GF+A} is an adaptive container: a two-bit header selects one of four
pockets over a shared eight-bit payload --- $\phi$-e3m4 $(1,3,4)$ at bias $3$,
e2m5 $(1,2,5)$ at bias $1$, a symmetric fixed point at scale $1/64$, and a slot
declared logarithmic. Implemented from those four parameter sets and compared
against the module: \textbf{$1{,}024$ of $1{,}024$ codes exact, zero
disagreements.} The decoder is correct.

The fourth pocket is not.

\begin{proposition}[Conformance does not certify the specification]
\label{prop:degenerate}
Let $D$ implement $f$ exactly on all of $C$. Conformance establishes $D=\rho\circ
f$ and says nothing about $f$. In particular $|f(C)|$ may be arbitrarily smaller
than $|C|$, and a decoder faithfully realising a degenerate $f$ is both
conformant and useless.
\end{proposition}

The logarithmic pocket emits $2^{-s}$ with the mantissa identically zero, where
$s=\lfloor \iota/16\rfloor+\lfloor \iota/32\rfloor$ and $\iota = 127-|w|$. It is
a power-of-two ladder, and it produces \textbf{seventeen distinct values from
$256$ codes} where each of the other three pockets produces $255$. Ninety-three
percent of that pocket's code space maps onto nothing new. Its own comment
declares a scale of $\iota\cdot 8/127$ while the code implements
$\iota\cdot 3/32$, half again as steep --- the second time in this catalogue
that a line of RTL contradicted the comment beside it.

The degeneracy is not free of consequence for the table, and its direction is
the one this work has now seen eleven times. Rebuilding that pocket as the
\texttt{lns8} this repository already declares --- a signed seven-bit log field
at \texttt{frac\_bits}${}=3$, value $2^{L/8}$, giving $255$ distinct values ---
costs what completing a decoder always costs:
...
as \texttt{lns8} (255 values) & $182$ & $271.22$ & $1.4902$ \\
\midrule
...
\noindent
So \texttt{GF+8}'s $0.1004$, which already loses to \texttt{fp8~e4m3}'s $0.1254$
by $19.9\%$ in the only head-to-head class in the catalogue, is itself inflated
by a pocket that discards most of its own code space. Repaired, it loses by
more. \emph{We report this because it is true and because the alternative --- a
number resting on a degenerate component --- is the kind of claim this paper
spends twenty-seven sections dismantling in other people's work.}
...
\section{What was already known, and what is left}
\label{sec:relatedternary}

The construction this paper measures --- a binary-radix float whose exponent
field carries a ternary-cardinality alphabet packed densely into
$\lceil E_t\log_2 3\rceil$ bits --- sits close enough to several existing lines
of work that the boundaries are worth drawing explicitly rather than left to a
reader to find.

\paragraph{A ternary exponent field is not new.} TERNAC~\cite{ternac1,ternac2}
was a 48-trit floating-point format with a 42-trit mantissa and a
\emph{six-trit exponent}, emulated on binary hardware at SUNY Buffalo in
1972--73. It differs from the format here in two ways that matter --- its value
semantics are radix-3, and its trits are represented as a pair of binary numbers
rather than packed --- but \emph{the idea of an exponent field counted in trits
predates this work by half a century}, and no claim of priority over it is made
or implied anywhere above.

\paragraph{The $k=5$ packing is public.} Compilade~\cite{compilade} published
the five-trits-in-eight-bits packing in June 2024 and reported it as
$99.06\%$ efficient, which is exactly $1-w(5)$ in the notation of
Theorem~\ref{thm:tiling}. The accompanying discussion also records that
$17$ trits in $27$ bits and $111$ in $176$ beat $1.6$ bits per trit --- that is,
\emph{the unevenness of $w(k)$ has been observed publicly}, if not written as a
function. What Section~\ref{sec:tiling} adds is the closed form, the proof that
the waste is positive at every $k$, and the consequence for choosing a rung; the
$k=5$ datum itself is Compilade's.

\paragraph{The packed decoder has been built.} TerEffic~\cite{tereffic}
implements exactly the packed-$3^5$-in-eight-bits decoder in FPGA LUTs, for
ternary \emph{weights} rather than for an exponent field. It is one abstraction
level from the decoders measured in Section~\ref{sec:fiftyfour} and any reader of
that section will know it.

\paragraph{The binary encoding of balanced ternary is textbook.}
Parhami and McKeown~\cite{bctparhami} treat binary-encoded balanced ternary
directly, at two bits per trit --- which is the baseline dense packing improves
on, and the source of the encoding penalty this paper quantifies. Knuth's
treatment~\cite{knuthv2} and the Setun machines~\cite{setun} establish balanced
ternary as a radix; \emph{this work does not change the radix} and therefore does
not inherit the radix-economy argument that literature carries.


\paragraph{The construction is standard for decimal, and the standard took a
third horn.} Packing $b^{k}$ values into $\lceil k\log_2 b\rceil$ bits and
disposing of the surplus is the problem Chen and Ho~\cite{chenho1975} solved for
$b=10$ in 1975 and Cowlishaw~\cite{dpd2002} standardised as Densely Packed
Decimal, normative in IEEE~754~\cite{ieee754}: three decimal digits in ten bits,
$w=2.34\%$ in the notation of Theorem~\ref{thm:tiling}. \emph{That theorem is
the same construction at $b=3$, and the theorem itself is unique
factorisation.} What is ours is the closed form of $w(k)$, its behaviour across
$k$, and the consequence for choosing a rung.

The comparison is worth making twice over, and the second time corrects us.
Proposition~\ref{prop:reserve} offers two horns --- guard the surplus, or extend
the specification to cover it --- and we first presented the decimal answer as a
third. It is not: assigning the surplus \emph{is} horn~(ii), taken in a
particular way. The genuinely narrower statement is that \textbf{horn~(ii) can
be taken by extending the specification's \emph{code} coverage without extending
its \emph{value} set}, and that is what a non-canonical alternate encoding is.

Nor did IEEE~754 leave the choice implicit. It enumerates the dispositions and
names them: \S2.1.14 distinguishes the $1000$ canonical declets from the $24$
that are ``accepted in operands'' but never produced; Table~3.4 records that
those $24$ map onto values already in range; \S3.5.2 specifies a \emph{different}
disposition for the binary-integer encoding, where a non-canonical significand
is read as zero; Annex~B.4 recommends that debuggers offer signalling-NaN
behaviour on such patterns, which is the guarding horn, in the same standard.
\S5.7.2 gives the predicate \texttt{isCanonical}. \textbf{The enumeration is
prior art~\cite{ieee754,dpd2002}. What is not is the cost comparison}, and the
measurement below is the narrower contribution that leaves us.

\paragraph{What is left.} Setting those aside, what remains is narrow and stated
narrowly: the combination of ternary \emph{cardinality} with binary
\emph{radix} and binary \emph{storage}, with the capacity surplus
$2^{\lceil k\log_2 3\rceil}-3^k$ treated as a design variable rather than as
overhead --- reserved, guarded, and priced. Around that: the closed form of
$w(k)$ and its consequences for rung choice
(Sections~\ref{sec:tiling},~\ref{sec:window}), the reservation cost measured
rather than argued, and placed-and-routed silicon with exhaustive conformance for
each rung claimed. Every one of those is a contribution about \emph{measurement
and boundary}, not about being first.


\section{At fifty-four seeds the claims separate}
\label{sec:fiftyfour}

Section~\ref{sec:bands} left the ranking resolving into seven levels on five
seeds, with a separation threshold of $5.50\%$ and a detection floor for a
decoder-cost difference of $41.3$ LUT --- $79\%$ of a whole decoder. Both are
consequences of the sample size and of reporting a median, whose efficiency at
$n=5$ is $0.698$. The nine formats the paper's claims rest on were therefore
re-swept at \textbf{fifty-four seeds}, and the reported statistic is the
\textbf{mean}.

The separation threshold falls from $5.50\%$ to $\mathbf{1.43\%}$.

\begin{table}[htbp]
\centering
\vspace{0.6em}
\caption{Fifty-four seeds, mean with a $95\%$ confidence interval, all on the
common $128$-bit harness. Welch against \texttt{binary16}, Holm-corrected over all $24$ non-baseline comparisons in the table rather than over the eight shown --- correcting over the displayed subset is anti-conservative. Every one still survives.}
\label{tab:fiftyfour}
\small
\begin{tabular}{lrrrl}
\toprule
& MHz/LUT & $\pm95\%$ & vs \texttt{binary16} & $p_{\text{Holm}}$ \\
\midrule
\textbf{GFTernary} & $0.1584$ & $0.0018$ & $+46.99\%$ & $<10^{-300}$ \\
\texttt{binary32}  & $0.1429$ & $0.0013$ & $+32.53\%$ & $<10^{-300}$ \\
\texttt{fp8 e4m3}  & $0.1225$ & $0.0012$ & $+13.68\%$ & $1.9\times10^{-63}$ \\
\textbf{TNF16a}    & $0.1214$ & $0.0012$ & $+12.59\%$ & $8.1\times10^{-56}$ \\
\texttt{fp8 e5m2}  & $0.1188$ & $0.0011$ & $+10.26\%$ & $5.6\times10^{-41}$ \\
\textbf{GF10}      & $0.1172$ & $0.0012$ & $+8.70\%$  & $5.6\times10^{-28}$ \\
\textbf{TNF17e}    & $0.1155$ & $0.0013$ & $+7.17\%$  & $2.1\times10^{-17}$ \\
\textbf{TNF16c}    & $0.1152$ & $0.0010$ & $+6.92\%$  & $6.9\times10^{-21}$ \\
\texttt{binary16}  & $0.1078$ & $0.0012$ & --- & --- \\
\bottomrule
\end{tabular}
\end{table}

\noindent
The margins are smaller than the five-seed estimates suggested --- \texttt{TNF16c}
read $+12.5\%$ there and reads $+6.92\%$ here --- and that is the expected
direction: a point estimate taken near the resolution floor is as likely to
flatter as to understate, and the larger sample removes the flattery along with
the noise.

\textbf{The claim this paper makes at equal storage is $+6.92\%$ over IEEE half,
with a $95\%$ interval of $\pm0.0010$ on $0.1152$ and $p_{\text{Holm}} =
6.9\times10^{-21}$.} \texttt{TNF16c} is sixteen stored bits against
\texttt{binary16}'s sixteen, its out-of-specification offsets are reserved so
every code is defined or flagged, and its exponent spans $\pm121$ --- inside the
window Proposition~\ref{prop:window} requires.

\texttt{TNF17e} at $+7.17\%$ and \texttt{TNF16c} at $+6.92\%$ are not separated
from \emph{each other}: their intervals overlap. The paper claims the pair, not
an order within it.

\section{Why the ladder stops at five trits when the target is \texttt{fp32}}
\label{sec:window}

Every decoder in this catalogue forms the \texttt{fp32} exponent the same way:

\begin{center}\ttfamily wire [7:0] e32 = e[7:0] + 8'd127;\end{center}

\noindent
which is correct only while $e+127$ lies in $[0,255]$. Outside that window the
addition wraps rather than saturating, and a wrapped exponent is a wrong finite
number, not an overflow --- so nothing downstream can detect it. \texttt{TNF32}'s
offsets $364$, $620$ and $876$, whose exponents are $0$, $256$ and $512$, all
decode to exactly $1.0$.

\begin{proposition}[The exponent window bounds the rung]
\label{prop:window}
A rung with $E_t$ trits has $E = (3^{E_t}-1)/2$ and its exponent spans
$[1-E,\;3^{E_t}-2-E]$. That span lies inside $[-127,+128]$ exactly when
$3^{E_t}/2 \le 128$, that is
\[
  E_t \;\le\; 5 .
\]
$3^{5}/2 = 121$ fits and $3^{6}/2 = 364$ does not, so on a decoder emitting
\texttt{fp32} the ladder admits at most five trits of exponent. A wider rung must
saturate outside the window or emit more than $32$ bits, and either changes its
cost.
\end{proposition}

The boundary is sharp and it falls exactly where
Corollary~\ref{cor:rungs} put the least-waste rung. $E_t=5$ is simultaneously the
$k$ of minimum $w(k)$ below $k=10$ and the widest exponent an \texttt{fp32}
decoder can carry without wrapping --- a coincidence of two unrelated
constraints, one arithmetic and one about the target type, and the reason
\texttt{TNF16c} and \texttt{TNF17e} are the two rungs this paper recommends.

Three rungs were withdrawn for violating it. \texttt{tools/check\_exponent\_window.py}
now fails on any rung whose span leaves the window, lists the withdrawn ones
explicitly rather than deleting them --- a gate that silently ignores a known-bad
entry is how the defect returns --- and is verified red by removing that list.


\section{The block axis, measured}
\label{sec:blockmeasured}

The rule this work set itself was to beat MXFP4 on the block axis by measurement
before publishing anything about it. Eleven agents, two independent codebases and
$14.9$ million real blocks later, the rule is met and the reason is not the one
this paper expected.

\paragraph{The measurement.} A five-trit balanced-ternary scale field at step
$2^{k/3}$, byte-identical \texttt{E2M1} elements, block $32$, \textbf{exactly
$4.25$ bits per element}:

\begin{center}\small
\begin{tabular}{lrrrr}
\toprule
source & blocks & MXFP4 & ours & $\Delta$ \\
\midrule
smollm2, $106.2$M weights & $3{,}317{,}760$ & $0.02259767$ & $0.01873104$ & $\mathbf{+17.11\%}$ \\
qwen, $357.8$M weights    & $11{,}182{,}080$ & $0.00238612$ & $0.00199422$ & $\mathbf{+16.42\%}$ \\
iid $N(0,1)$              & $200{,}000$ & $0.11501641$ & $0.09672548$ & $+15.90\%$ \\
Student-$t(3)$            & $200{,}000$ & $0.24983310$ & $0.19157210$ & $+23.32\%$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent
\textbf{Zero losing blocks} in $14{,}899{,}840$ real and $400{,}000$ synthetic.
Downstream on \texttt{wikitext2-test}, $304{,}986$ tokens through $210$ linear
modules, perplexity falls from MXFP4's $25.056$ to $\mathbf{20.034}$, closing
$52.1\%$ of the gap to \texttt{fp32}. A re-implementation written from the
specification text, which never imports the candidate's code, reproduces every
figure \textbf{to ten significant figures}.

\paragraph{The reason is resolution, and it is not ours alone.}
The gain is $100\%$ scale axis and $0\%$ element axis --- every winning arm uses
the byte-identical \texttt{E2M1} codebook. Decomposing the squared error on
smollm2: the floor rule of Algorithm~1 costs $6.9\%$, and the one-point-per-binade
grid costs $35.8\%$. The grid is the six-times-larger defect.

Within the scale axis, error is monotone in points per binade \emph{and in
nothing else}:
...
$2^{k/4}$ & $+18.57\%$ & $+17.95\%$ \\
$2^{k/8}$ --- plain binary & $\mathbf{+22.37\%}$ & $\mathbf{+21.79\%}$ \\
$2^{k/16}$ & $+23.74\%$ & $+23.23\%$ \\
\bottomrule
\end{tabular}
\end{center}
...
\noindent
$\sqrt{\varphi}^{\,k}$ at $2.8808$ points per binade lands beside $2^{k/3}$ at $3.0000$, which is what a law of step size predicts and what a law of algebra does not. The direction is the expected behaviour of a logarithmic companding law, but \emph{the rate is not}, and Proposition~\ref{prop:notbennett} measures how far off it is. \textbf{$\varphi$ is not special \emph{for accuracy} on this axis; it is a step finer than a binade.} And a plain binary ladder at the same eight bits beats the ternary one by five to six points of RMSE and $1.1$ of perplexity. At matched dynamic range, $243$ ternary codes lose to $255$ binary ones on \emph{six of six} range settings across four sources.

\begin{proposition}[The scale axis does not obey Bennett's law, and the discrepancy replicates]
\label{prop:notbennett}
Bennett's law for a companding quantiser gives squared error
$\mathrm{MSE}(N) = a + b/N^{2}$ in the points per binade $N$, so along a
\emph{nested} chain of ladders the ratio of successive differences
$\mathrm{MSE}(N)-\mathrm{MSE}(2N)$ must equal $2^{2}=4$, independent of $N$ and
of the model. Measured on the nested chain $2^{k/2} \subset 2^{k/4} \subset
2^{k/8} \subset 2^{k/16}$, the implied exponent is
$$p = 1.310 \ \text{then} \ 1.537 \quad(\text{SmolLM2}), \qquad
  p = 1.327 \ \text{then} \ 1.545 \quad(\text{Qwen}).$$
Two conclusions follow, and the second is the one that costs us something.
\emph{The exponent is near $1.5$, not $2$}, so error falls with resolution
distinctly more slowly than the law predicts; and \emph{it is not constant},
drifting upward as the ladder refines. The two models agree to within $0.02$ on
both values, across a $3.4\times$ difference in block count, so this is a
property of the compound quantiser and not of a corpus.
\end{proposition}

\noindent The immediate consequence is arithmetic and unwelcome: at $p=1.5$ a
doubling of scale resolution returns $2^{-1.5}=35\%$ of the previous doubling
rather than $25\%$, so \textbf{the case for stopping at $2^{k/8}$ rests on the
area column alone and not on saturation.} A second consequence is a retraction:
this section previously read the monotone ordering as confirming a known result,
and the ordering does hold, but \emph{the law it was attributed to is measurably
not the law being obeyed.}

\begin{theorem}[The exponent of a lattice-minimised objective is set by the local shape, not by the law]
\label{thm:latticeexp}
Let $g$ be continuous with a unique minimiser $t^{*}$, and let the minimum be
sought over a lattice $\{t_{0}+jh : j \in \mathbb{Z}\}$ whose offset $t_{0}$ is
uniform on $[0,h)$. Write $\mathcal{E}(h) = \mathbb{E}\,[\min_{j} g(t_{0}+jh)] -
g(t^{*})$.
\begin{enumerate}
\item[(i)] If $g$ is $C^{2}$ near $t^{*}$ with $g''(t^{*})>0$, then
  $\mathcal{E}(h) = \tfrac{1}{24}g''(t^{*})\,h^{2} + o(h^{2})$.
\item[(ii)] If instead $g$ has a corner at $t^{*}$, with one-sided slopes
  $-c_{-}<0<c_{+}$, then
  $\mathcal{E}(h) = \dfrac{c_{-}c_{+}}{2(c_{-}+c_{+})}\,h + o(h)$.
\end{enumerate}
\end{theorem}

\begin{proof}
Let $u \in [0,h)$ be the distance from $t^{*}$ up to the next lattice point, so
that the two neighbours sit at $-(h-u)$ and $+u$ and $u$ is uniform.
For (i), the nearer neighbour is at distance $d = \min(u,h-u)$ and the excess is
$\tfrac12 g''d^{2}+o(d^2)$; since
$\mathbb{E}\,d^{2} = \tfrac{2}{h}\int_{0}^{h/2}\!d^{2}\,\mathrm{d}d = h^{2}/12$,
the claim follows.
For (ii), the two candidate excesses are $c_{+}u$ and $c_{-}(h-u)$ and the
smaller is taken, so
$\mathcal{E} = \tfrac1h\int_{0}^{h}\min\{c_{+}u,\,c_{-}(h-u)\}\,\mathrm{d}u$.
The integrand switches at $u^{*} = c_{-}h/(c_{-}+c_{+})$; integrating the two
linear pieces gives
$\tfrac1h\big[\tfrac12 c_{+}u^{*2} + \tfrac12 c_{-}(h-u^{*})^{2}\big]
= h\,c_{-}c_{+}/\big(2(c_{-}+c_{+})\big)$.
\end{proof}

\noindent The relevance is that \emph{our} $g$ --- a block's squared error as a
function of $\log_{2}$ of the shared scale --- is neither case but a chain of
both. The E2M1 rounding index of each element is piecewise constant in $t$ and
jumps when that element crosses a midpoint boundary, at
$t = \log_{2}(|v_{i}|/\beta_{j})$, so $g$ is a sequence of smooth arcs joined at
corners. Which regime a ladder of spacing $h=1/N$ reports therefore depends on
$h$ against the local corner spacing, and \textbf{the exponent must rise from
near $1$ toward $2$ as the ladder refines} --- which is the drift
Proposition~\ref{prop:notbennett} measured and could not explain.

\begin{table}[t]\centering
\caption{Excess over the fine-grid optimum, averaged over every lattice phase,
and the exponent read straight off successive excesses with nothing fitted.
smollm2, $86{,}400$ blocks, reference ladder $512$ points per binade.}
\label{tab:latticeexp}
\begin{tabular}{rrr}
\toprule
$N$ & excess (fraction) & $p(N \to 2N)$ \\
\midrule
$2$   & $4.214\times10^{-1}$ & $1.417$ \\
$4$   & $1.579\times10^{-1}$ & $1.604$ \\
$8$   & $5.193\times10^{-2}$ & $1.728$ \\
$16$  & $1.568\times10^{-2}$ & $1.838$ \\
$32$  & $4.385\times10^{-3}$ & $1.933$ \\
$64$  & $1.148\times10^{-3}$ & $\mathbf{2.030}$ \\
$128$ & $2.812\times10^{-4}$ & --- \\
\bottomrule
\end{tabular}
\end{table}

\noindent \textbf{The curve is a property of the codebook, not of the model.}
Repeating the sweep on Qwen --- a different size and a different architecture ---
returns $p = 1.596$, $1.726$, $1.837$, $1.932$, $2.029$ against smollm2's
$1.604$, $1.728$, $1.838$, $1.933$, $2.030$ --- agreeing to within $0.008$ at
every point, with $D = 46.7$ against $46.5$. Two models of different scale producing
the same exponent curve to three decimals is what the corner picture requires and
what a corpus-dependent explanation forbids.

\noindent \textbf{The prediction is quantitative and it holds.} The corner
positions $\log_{2}(|v_{i}|/\beta_{j})$ are computable without evaluating the
error even once, so the corner density is an \emph{independent} instrument.
Measured within $\pm\tfrac14$ octave of each block's optimum it is $46.5$ corners
per octave, mean and median agreeing, i.e.\ a typical corner spacing of $0.0215$
octave --- a ladder of $N \approx 47$. Table~\ref{tab:latticeexp} reaches
$p = 1.933$ across $N{=}32{\to}64$ and $p = 2.030$ across $64{\to}128$: the
exponent arrives at Bennett's value exactly where the corner spacing says the
lattice starts resolving a single smooth arc. \emph{The law was never wrong; it
was being applied on the wrong side of a crossover this paper had not
identified.}

\paragraph{The diagnosis is a design rule, not a fact about E2M1.} One codebook
is an anecdote, so the prediction was registered
(\texttt{research/block/PREREG\_codebook\_exponent\_2026-08-12.md}) and then run
on four four-bit codebooks chosen to spread $D$ as widely as four-bit codebooks
can: E2M1; \texttt{int4}, whose linear magnitudes bunch its boundaries at the top
in log space; NF4, which is \emph{asymmetric} and so carries fifteen distinct
magnitudes where a symmetric codebook carries eight; and E3M0, a pure exponent
whose boundaries are spread over $6.58$ octaves. $D$ was measured first, by
counting, with the error never evaluated.

\begin{table}[t]\centering
\caption{Corner density against measured crossover, smollm2, $82{,}944$ blocks.
$D$ counts corners within $\pm\tfrac14$ octave of each block's optimum and
evaluates no error; the crossover is where the measured exponent reaches $2$,
interpolated in $\log N$. The registered prediction was ordinal only.}
\label{tab:codebookexp}
\begin{tabular}{lrrr}
\toprule
codebook & $D$ (counted) & crossover (measured) & ratio \\
\midrule
E3M0 --- pure exponent      & $28.0$ & $36.7$ & $1.31$ \\
E2M1 --- MXFP4's own        & $46.9$ & $49.0$ & $1.05$ \\
\texttt{int4} --- linear    & $51.2$ & $53.0$ & $1.04$ \\
NF4 --- asymmetric          & $53.1$ & $54.3$ & $1.02$ \\
\bottomrule
\end{tabular}
\end{table}

\noindent \textbf{The ordering holds, on values that are distinct rather than
tied}, and three of the four cardinal predictions land within $5\%$ --- better
than the registration asked for, since it had recorded that a cardinal hit
``would be luck at this level of modelling.'' The exception is E3M0 at $1.31$,
and its boundaries are the sparsest of the four, spread over $6.58$ octaves
against E2M1's $4.32$; with corners that sparse, fewer of them fall inside the
window the density is estimated over, and the local-arc picture the ratio assumes
is correspondingly weaker. We report the outlier rather than dropping it.

Two cautions belong with this table. The discrete crossover --- the first $N$ in
a doubling sequence at which $p \ge 1.9$ --- has too little resolution to
separate $D=46.9$ from $D=53.1$: it returns $32$ for three of the four codebooks,
and reading an ordering off it would be reading off a tie. The interpolated
crossover is what Table~\ref{tab:codebookexp} reports. And the final doubling of
each sweep, $N=64 \to 128$, is discarded: at a reference ladder of $256$ points
per binade the coarse grid is only twice the reference, so its excess is biased
low and its exponent high --- \emph{all four codebooks return $\approx 2.29$
there, which is the signature of an instrument artefact and not of four
independent physical results.}

\begin{corollary}[The crossover is monotone in corner density --- and only monotone]
\label{cor:designrule}
For a block format with element codebook $\mathcal{C}$, let $D(\mathcal{C})$ be
the corner density per octave and $N^{*}(\mathcal{C})$ the scale-ladder
resolution at which the measured exponent reaches $2$. Then $N^{*}$ is
\emph{increasing} in $D$, so a codebook's corner count --- computable from
$\mathcal{C}$ and the weight distribution alone, with no quantisation run ---
ranks codebooks by how far scale refinement keeps paying. \textbf{It does not
predict the value.}
\end{corollary}

\noindent \emph{The stronger form of this corollary was our own and we withdraw
it.} An earlier draft asserted $N^{*} \approx D$, on the strength of four
four-bit codebooks whose ratios $N^{*}/D$ ran $1.02$--$1.31$. That was a lever of
under $2\times$ in $D$, and the test that broke it was registered in advance
(\texttt{research/block/PREREG\_codebook8\_2026-08-12.md}) precisely because a
rule confirmed over so short a range is not yet a rule.

\begin{table}[t]\centering
\caption{The registered eight-bit test. Corner density spans $6.8\times$ here
against under $2\times$ in the four-bit run. The ordering survives; the
identification $N^{*}\approx D$ does not. \texttt{int8}'s crossover is an
\emph{extrapolation} past the usable sweep, declared as such before the run.}
\label{tab:codebook8}
\begin{tabular}{lrrrr}
\toprule
codebook & width & $D$ & $N^{*}$ & $N^{*}/D$ \\
\midrule
E3M0          & $4$ & $28.0$  & $36.7$ & $1.31$ \\
E2M1          & $4$ & $46.9$  & $49.0$ & $1.05$ \\
\texttt{int4} & $4$ & $51.2$  & $53.0$ & $1.04$ \\
NF4           & $4$ & $53.1$  & $54.3$ & $1.02$ \\
\addlinespace
E5M2          & $8$ & $128.7$ & $388$  & $\mathbf{3.01}$ \\
E4M3          & $8$ & $256.5$ & $530$  & $\mathbf{2.07}$ \\
\texttt{int8} & $8$ & $869.8$ & $598^{\dagger}$ & $\mathbf{0.69}$ \\
\bottomrule
\end{tabular}
\end{table}

\noindent The registration named the band $1.0$--$1.35$ and \textbf{all three
eight-bit codebooks fall outside it}, at $3.01$, $2.07$ and $0.69$. The failure
is systematic rather than noisy: $D$ moves by $6.8\times$ across the eight-bit
row and $N^{*}$ by $1.5\times$, so \emph{the crossover is far less sensitive to
corner density than the identification assumed.} In hindsight the identification
was a heuristic and never a derivation --- Theorem~\ref{thm:latticeexp} puts the
crossover where $\tfrac{1}{24}g''h^{2}$ overtakes $\tfrac18 c_{-}c_{+}
h/(c_{-}{+}c_{+})$, which is a ratio of local slopes to curvature. Corner
\emph{spacing} enters that ratio but does not determine it, and a codebook with
many levels has small corner slopes as well as closely spaced corners. The two
effects pull opposite ways, which is why the ratio falls as $D$ rises.

\paragraph{One reading below the theorem's own floor.} \texttt{int8} returns
$p = 0.490$ on its coarsest doubling, and Theorem~\ref{thm:latticeexp} admits
nothing below $1$. Neither regime describes it: with $255$ levels the error
surface is nearly flat in the scale, the lattice minimum at $h = 1/16$ sits far
from $t^{*}$, and what is being measured there is the global bowl and not any
local shape. We report it and claim nothing from it.

\paragraph{We tried to restore the constant by derivation, and it cannot be
done at this level of description.} The obvious repair is to stop reading a
spacing off Theorem~\ref{thm:latticeexp} and instead equate its two branches
directly. Setting $\tfrac{1}{24}g''h^{2} = \tfrac12 c_{-}c_{+}h/(c_{-}{+}c_{+})$
gives a crossover with no fitted constant anywhere,
$$N^{*}_{\text{theory}} \;=\; \frac{g''(t^{*})\,(c_{-}+c_{+})}{12\,c_{-}c_{+}},$$
and every term is exactly computable per block rather than estimated: on a single
arc the rounding indices are frozen, so $g(t) = s^{2}A - 2sB + C$ with $s=2^{t}$,
$A = \sum L_{i}^{2}$, $B = \sum L_{i}v_{i}$, whence
$g'' = 2(\ln 2)^{2}s(2sA-B)$ and each one-sided slope is
$2(\ln 2)s(sA-B)$ taken with its own arc's $(A,B)$. No finite differences are
involved, so the estimate does not inherit a lattice bias.

It does not work. Against the seven measured crossovers, and across five ways of
aggregating the per-block value to the population the measurement is taken over
--- median, mean, $90$th percentile, and weighting by each block's corner
coefficient or by its curvature --- \emph{no predictor is even approximately
constant}:

\begin{center}\small
\begin{tabular}{lrr}
\toprule
predictor of $N^{*}$ & mean ratio & coefficient of variation \\
\midrule
$1/w^{*}$, arc containing the optimum & $0.27$ & $32.1\%$ \\
$N^{*}_{\text{theory}}$, median over blocks & $0.30$ & $37.6\%$ \\
$N^{*}_{\text{theory}}$, corner-weighted & $0.21$ & $37.8\%$ \\
$D$, the withdrawn heuristic & $0.84$ & $40.5\%$ \\
$N^{*}_{\text{theory}}$, curvature-weighted & $1.22$ & $51.4\%$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent A usable predictor would sit near $10\%$; the best of these is $32\%$,
and it is not the derived one. \textbf{The crossover is not a local functional of
the error curve at the optimum.} That is not a defect in the theorem, which
concerns two asymptotic regimes and is exact in each: it is that equating leading
terms locates a crossover only when every block is deep in one regime or the
other, and near the crossover none is. The measured $N^{*}$ is where the exponent
of a \emph{sum} over millions of blocks reaches two, and that is a property of the
whole distribution of per-block behaviours over a range of $h$ --- not of any one
block's slopes and curvature at its own minimum. We record the attempt and the
formula so the next reader does not spend the afternoon we spent.

\paragraph{What survives, and it is what a designer needs.} The ordering held
across \emph{seven} codebooks, two element widths and a $31\times$ range of $D$.
A designer choosing between element codebooks can still rank them by counting
corners --- and now knows not to read a number off that count.

\paragraph{What is and is not ours here.} \textbf{Theorem~\ref{thm:latticeexp}
is elementary and we claim no priority for it.} Case (i) is the standard
$O(h^{2})$ rate for grid search against a smooth minimum, familiar from
polynomial optimisation over a simplex~\cite{deklerk2017grid}, and the behaviour
of grid and descent methods against kinks is an active
literature~\cite{griewank2019kink}; we state and prove both cases only because
we need the constants and the contrast in one place. What we have not found
stated anywhere is the \emph{diagnosis}: that a block format's scale-ladder
exponent is a measurement of the element grid's corner density, that the two
quantities can be measured by instruments sharing no code, and that they agree.
An implication follows for anyone designing such a format --- \textbf{the return
on scale resolution is governed by the element codebook, not by the scale field}
--- and it is testable on any codebook by counting corners before running a
single quantisation.

\paragraph{One mechanism, proposed and refuted.} The E2M1 element grid
$\{0,\tfrac12,1,\tfrac32,2,3,4,6\}$ repeats identically in every binade, and so
does every $2^{k/N}$ ladder; a natural guess is that the two interact, that a
ladder commensurate with the element grid spends points where the grid is
already dense, and that this is what costs the missing half-exponent. It also
would have been a pleasing result, since it would have distinguished
$\varphi^{k}$ --- the worst-approximable ratio --- on exactly the grounds this
paper cares about. \textbf{The control refutes it.} Sliding the ladder's phase
against the element grid in eighths of a step moves the gain by $0.10$ points at
$N=2$, $0.01$ at $N=3$, $0.04$ at $N=4$ and $0.02$ at $N=8$, against the
$7.28$-point gap the mechanism was invented to explain. Phase does not matter,
so alignment is not the cause. What remains consistent with the measurement is
that the scale here is chosen by \emph{argmin of downstream error} rather than
by rounding, and that the downstream error saturates on the element grid's own
floor --- neither of which is the high-resolution rounding regime Bennett's law
describes. We state the measurement and decline the mechanism.

\paragraph{What that comparison leaves out, and it is the axis this paper is about.} Accuracy is one column and the multiplier is the other. Applying a scale drawn from $2^{k}$ is an exponent add and costs nothing; from $\varphi^{k}$ it is the Fibonacci step and costs an addition; from $2^{k/3}$ it is a shift and one of three constants; from $2^{k/8}$, one of eight. \emph{The ladders that win on accuracy here are the ones that stop being multiply-free}, and Section~\ref{sec:geoscale}'s claim --- that among \emph{multiply-free} grids $\varphi^{k}$ is the finest available at four bits --- survives this section untouched, because $2^{k/8}$ is not one of them. What does not survive is any reading in which $\varphi$ wins on accuracy by virtue of its algebra. \textbf{We have not synthesised the non-power-of-two scale appliers, so the cost column above is an argument and not a measurement} --- which is the one place in this paper where a comparison rests on reasoning while its neighbour rests on silicon.

\paragraph{Two things this hands back.} Replacing Algorithm~1's floor with a
squared-error argmin over the same $254$ \texttt{E8M0} codes is an
\emph{encoder} change that emits a byte-legal MXFP4 bitstream, and it buys
$+12.1\%$ perplexity --- about a fifth of the reported win, and it belongs to the
baseline rather than to any format. Separately, the observation earlier in this
paper that squared error mispredicts perplexity on this axis has been read as a
bound on element-format headroom. It is not one: measured headroom for eight
magnitudes ranges from $+1.6\%$ to $+8.5\%$ depending on the ladder beneath.

\paragraph{The accounting hole is closed below.} The first version of this
section reported that the chosen index at the finest ladder did not fit MXFP4's
bias-$127$ origin, and priced the format at ``$4.25$ bits plus an origin constant
MXFP4 does not have''. Measuring four models rather than two settles it: one
origin covers all of them, and MXFP4 has an origin constant of its own.



\paragraph{The cost column, measured.} The comparison above priced the ladders
on accuracy and argued about their silicon. The appliers were then built and
placed --- one instance, XC7A200T, \texttt{-nodsp}, five seeds, the harness of
Section~\ref{sec:bands} --- and each was checked against a reference to within
one \textsc{ulp} before being measured.

\begin{center}\small
\begin{tabular}{lrrrl}
\toprule
scale applier & LUT & MHz & vs $2^{k}$ & what it is \\
\midrule
$2^{k}$ (MXFP4's) & $207$ & $149.10$ & $1.0\times$ & one exponent add \\
$2^{k/3}$ (five trits) & $1{,}244$ & $66.24$ & $\mathbf{6.0\times}$ & add $+$ one of $3$ constants \\
$2^{k/8}$ & $1{,}576$ & $61.60$ & $7.6\times$ & add $+$ one of $8$ \\
$2^{k/16}$ & $1{,}989$ & $60.19$ & $9.6\times$ & add $+$ one of $16$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent
\textbf{The accuracy the finer ladders buy costs six to ten times the applier's
area and more than half its frequency}, against decoders that run $500$ to $600$
LUT --- so a step-3 applier is heavier than the entire decoder it feeds, and a
step-16 applier is heavier than three of them. The applier is one instance
amortised over a block of $32$ in \emph{time}, not replicated per element, so
this is area rather than throughput; but it is area on the same die, and the
paper's own frontier is drawn in area.

Two things follow that neither the accuracy table nor the argument would have
given. First, \textbf{$2^{k}$ is not merely cheaper, it is a different order of
cheap}: everything with a fractional step pays a mantissa multiply, and the jump
from $207$ to $1{,}244$ is that multiply appearing, not the ladder getting finer.
Second, \emph{once the multiply is paid for, finer is nearly free}: $2^{k/3}$ to
$2^{k/16}$ costs $60\%$ more area for four times the resolution. \textbf{The
design rule is to take $2^{k}$ or to take the finest ladder that fits} --- and
Proposition~\ref{prop:origin} says that is $2^{k/18}$. Anything between pays the
multiplier without spending it.

That also settles what the ternary step buys here. $2^{k/3}$ is where five trits
land, and its fraction field holds four slots for three values --- $25\%$ of the
field naming nothing, the same packing loss Theorem~\ref{thm:tiling} describes
for the exponent, now appearing in the scale. It is neither the cheap option nor
the accurate one.



\paragraph{Best against best, which halves the headline.}
The $+20.04\%$ above compares our best encoder against MXFP4's reference one.
That is the comparison the deployed format invites, since Algorithm~1 is what a
reader will run --- but it is not the comparison that isolates the format.
Giving each side its own best encoder:
...
baseline & ours & gain \\
\midrule
MXFP4, Algorithm~1 (floor) & $2^{k/3}$, search & $+20.04\%$ \\
MXFP4, Algorithm~1 (floor) & $2^{k/3}$, floor & $+15.52\%$ \\
\textbf{MXFP4 $+$ argmin encoder} & $\mathbf{2^{k/3}}$\textbf{, search} & $\mathbf{+8.99\%}$ \\
MXFP4 $+$ argmin encoder & $2^{k/8}$, search & $+14.12\%$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent
So the headline decomposes: of its $20.04$ points, \textbf{$12.15$ --- sixty-one
percent --- are the encoder, and the encoder's output is a byte-legal MXFP4
bitstream.} It belongs to the baseline. The remaining $7.90$ points, thirty-nine
percent, are the ladder, and they are ours.

\textbf{The number this paper should be quoted on is $+8.99\%$}, five-trit ladder
against a properly encoded MXFP4, and $+14.12\%$ for the finest binary ladder
against the same. Both are real, both are smaller than the figure a comparison
against the reference encoder produces, and the difference between them is a
choice about who is allowed their best implementation. \emph{We give the
competitor its best, because a result that needs the opponent handicapped is not
a result} --- and Section~\ref{sec:selfcheck} records four earlier claims in this
paper that failed exactly that test.


\paragraph{The same question, asked of the squared error.} The paragraph above
gave each side its best encoder and halved the perplexity headline. The
$+17.11\%$ RMSE figure was measured against the same reference encoder and
deserves the same treatment. An argmin encoder improves MXFP4's own RMSE by
$3.50\%$ on SmolLM2 and $2.86\%$ on Qwen, so best against best:

\begin{center}\small
\begin{tabular}{lrrr}
\toprule
& vs Algorithm~1 & encoder alone & \textbf{best vs best} \\
\midrule
RMSE, SmolLM2 & $+17.11\%$ & $+3.50\%$ & $\mathbf{+14.10\%}$ \\
RMSE, Qwen & $+16.42\%$ & $+2.81\%$ & $\mathbf{+14.01\%}$ \\
perplexity, SmolLM2 & $+20.04\%$ & $+12.15\%$ & $+8.99\%$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent
The squared-error headline survives the correction nearly intact and the
perplexity headline does not, and the gap between those two outcomes is the
finding. Both RMSE rows were re-measured after the fact by an independently written script, \texttt{research/block/verify\_block\_rmse.py}, which reproduces the baseline to ten significant figures and the ladder arms exactly; the Qwen encoder figure moved from $2.86\%$ to $2.81\%$ under its search, and the best-against-best column with it. \emph{That script is in the repository because the original figures came from a run whose working directory no longer exists, and a number whose only witness is a log has no instrument behind it.}

\begin{proposition}[The encoder's share is metric-dependent, and strongly]
\label{prop:encodershare}
Fixing the scale-selection rule, holding the codebook, the block size and the
bit budget constant, recovers $20.5\%$ of the achievable squared-error
improvement on SmolLM2 and $60.6\%$ of the achievable perplexity improvement on
the same weights and the same windows. \emph{The two metrics therefore rank the
same intervention three times apart.}
\end{proposition}

\noindent
\textbf{A referee will reduce this in one line, so we do it first.} Unweighted
squared error is the $H=I$ case of the Hessian-weighted layer-wise objective
$\lVert WX-\widehat{W}X\rVert_F^2$ with $H=2XX^{\!\top}$; ``the proxy and the
task metric disagree'' is then definitionally the statement $H\neq I$, and the
lineage that says so runs from optimal brain damage~\cite{lecun1990} through
optimal brain surgeon~\cite{hassibi1993} to AdaRound~\cite{adaround} and
GPTQ~\cite{gptq}. AdaRound reports a stronger version of our observation:
at essentially equal per-weight squared error, $48$ of $100$ random roundings
beat the squared-error-optimal one, which is a \emph{sign} inversion.
\textbf{An earlier draft said ours was only a magnitude disagreement. A third
model retired that sentence:} on GPT-2 the squared-error argmin encoder makes
perplexity \emph{worse} than MXFP4's reference rule, $36.1482$ against $35.9747$,
so the encoder's share of the achievable gain is $-7.2\%$. Ours is a sign
inversion too, and we found it only by asking a third model.

So Proposition~\ref{prop:encodershare} is a rediscovery of an established class,
and we label it as one. What we did not find in that literature is the
\emph{quantified attribution ratio for a single named fix} --- scale selection,
at fixed codebook, block size and bit budget, on the MX block axis --- and that
number, $20.5\%$ against $60.6\%$, is the width at which the contribution should
be read.

\paragraph{The proposition's headline is a SmolLM2 statement, and the second
model halves it.} Repeating the attribution on Qwen, with the independently
written instruments of Section~\ref{sec:blockmeasured} and not the original run:

\begin{table}[t]\centering
\caption{The encoder's share of the achievable improvement, four models, both
metrics. \emph{Same layers, same blocking, same quantiser, same weights} --- the
squared-error column is computed by a script that imports the perplexity
harness's quantiser rather than reimplementing it, so \textbf{only the metric
changes between the two columns}.}
\label{tab:encodershare4}
\begin{tabular}{lrr}
\toprule
...
SmolLM2 & $20.5\%$ & $+58.7\%$ \\
Qwen    & $17.1\%$ & $+23.3\%$ \\
Pythia  & $20.6\%$ & $\mathbf{-0.6\%}$ \\
GPT-2   & $18.6\%$ & $\mathbf{-7.2\%}$ \\
\midrule
spread  & $\mathbf{1.20\times}$ & \textbf{crosses zero} \\
\bottomrule
\end{tabular}
\end{table}

\noindent \textbf{The proxy is nearly constant across four models and the task
metric changes sign.} Squared error puts the encoder's worth at $17.1$--$20.6\%$
of the achievable gain on every model tried, a spread of $1.20\times$; perplexity
puts it anywhere from $+58.7\%$ to $-7.2\%$. On two of the four models the
squared-error-optimal encoder makes perplexity \emph{worse}.

\emph{One row of this table is a control and not a result.} The argmin encoder
minimises squared error over the E8M0 ladder block by block, so its squared-error
share is positive by construction; measuring it anyway is what makes the
perplexity column trustworthy, because a negative entry in the squared-error
column would have condemned the harness rather than the theory. All four are
positive. The SmolLM2 entry also reproduces the $20.5\%$ published above, from
code that shares only the quantiser with the run that produced it.

\noindent \textbf{``Three times apart'' was SmolLM2's number.} Two models later
the disagreement is not a factor at all but a change of sign, and the honest
statement of Proposition~\ref{prop:encodershare} is the one the table makes:
squared error answers this question the same way on every model and perplexity
does not answer it the same way twice.

\begin{proposition}[What transfers between models, and what does not]
\label{prop:transfer}
Measuring one intervention --- the $2^{k/3}$ scale ladder against MXFP4, at fixed
codebook, block size and bit budget --- four ways on two models gives:
\begin{center}\small
\begin{tabular}{lrrrrr}
\toprule
claim variant & SmolLM2 & Qwen & Pythia & GPT-2 & spread \\
\midrule
sq.\ error, best encoder both sides & $14.10\%$ & $14.01\%$ & $13.54\%$ & $14.10\%$ & $\mathbf{1.04\times}$ \\
sq.\ error, against Algorithm~1     & $17.11\%$ & $16.42\%$ & $16.47\%$ & $16.79\%$ & $\mathbf{1.04\times}$ \\
perplexity, best encoder both sides & $9.50\%$  & $7.18\%$  & $12.38\%$ & $7.19\%$ & $1.72\times$ \\
perplexity, against Algorithm~1     & $20.25\%$ & $9.16\%$  & $12.31\%$ & $6.74\%$ & $\mathbf{3.00\times}$ \\
\bottomrule
\end{tabular}
\end{center}
At four models the two axes are no longer symmetric, and the asymmetry is the
result. \textbf{The metric axis dominates:} squared error answers to within
$1.04\%$ on every model and perplexity to within $1.72$--$3.00\times$.
\emph{The baseline axis operates only inside perplexity} --- there,
best-against-best spreads $1.72\times$ against the reference encoder's
$3.00\times$; inside squared error both baselines spread $1.04\times$ and the
choice does not matter at all. So \textbf{the least transferable of the four
variants is the combination most often quoted}, and the most transferable pair is
indistinguishable from each other.

One entry makes the point better than the spreads do. Pythia is the model MXFP4
damages most --- its perplexity rises $75.7\%$ against fp32, where Qwen's rises
$23.8\%$ --- and \emph{its squared-error rows are the most ordinary in the
table}, $16.47\%$ and $13.54\%$. \textbf{The proxy gives no signal at all that
this is the hard case.} Its stability is not robustness; it is insensitivity to
the thing that varies.
\end{proposition}

\begin{proposition}[The model spread is sensitivity, not perturbation]
\label{prop:sensitivity}
At $4.25$ bits per weight with MXFP4's reference encoder, on the same layers in
each model, the relative weight perturbation is
$$\frac{\lVert \Delta W\rVert_{F}}{\lVert W\rVert_{F}}
 = 11.79\%,\; 11.72\%,\; 11.76\%,\; 11.57\%$$
for GPT-2, Qwen, SmolLM2 and Pythia --- a spread of $1.02\times$ --- while the
induced perplexity degradation is $20.1\%$, $23.8\%$, $62.7\%$, $75.7\%$, a spread
of $3.77\times$. \textbf{The two are anti-ordered:} Pythia takes the smallest
perturbation and suffers the largest damage. Hence the model-to-model variation
in quantisation damage carries \emph{no} information about the size of the
perturbation, and is entirely a property of the model's sensitivity to it.
\end{proposition}

\noindent This is the measured form of the $H \neq I$ statement above, and it is
tighter than that statement suggests: the perturbation is not merely a poor
predictor of the damage, it is \emph{constant to two percent} across four
architectures while the damage nearly quadruples. Any figure of merit that is a
functional of $\Delta W$ alone is therefore pinned by measurement and cannot
separate a model that loses a fifth of its perplexity from one that loses three
quarters.

\paragraph{And the sensitivity is a property of the model, not of the pairing.}
Proposition~\ref{prop:sensitivity} was measured on one encoder, which leaves open
whether the ratio is a stable model property or a model--format interaction that
happens to look stable at one point. Four models by four encoders --- MXFP4's
reference rule, its squared-error argmin, $2^{k/3}$ and $2^{k/8}$ --- settles it,
and the prediction was registered first
(\texttt{research/block/PREREG\_sensitivity\_scope\_2026-08-12.md}).

Second order requires the invariant to be
$S_{2} = \Delta_{\mathrm{ppl}} / p^{2}$ with $p = \lVert\Delta W\rVert_{F} /
\lVert W\rVert_{F}$, and \emph{not} the ratio $\Delta_{\mathrm{ppl}}/p$ that
Proposition~\ref{prop:sensitivity} is diagnosed by; the registration says so
explicitly, because printing the first-power ratio as though it were a candidate
invariant would be a units error of the kind this paper keeps finding in itself.

\begin{table}[t]\centering
\caption{Sensitivity across four encoders. Within-model spread of
$S_{2}=\Delta_{\mathrm{ppl}}/p^{2}$ against the first-power ratio, and both
against the between-model spread. \emph{Every registered prediction held.}}
\label{tab:sensscope}
\begin{tabular}{lrrr}
\toprule
model & spread of $\Delta_{\mathrm{ppl}}/p$ & spread of $S_{2}$ & $S_{2}$, mean \\
\midrule
GPT-2   & $1.82\times$ & $\mathbf{1.46\times}$ & $13.4$ \\
Qwen    & $1.62\times$ & $\mathbf{1.33\times}$ & $15.1$ \\
SmolLM2 & $2.15\times$ & $\mathbf{1.67\times}$ & $34.4$ \\
Pythia  & $1.40\times$ & $\mathbf{1.13\times}$ & $57.2$ \\
\midrule
between models & $4.14\times$ & $\mathbf{4.26\times}$ & \\
\bottomrule
\end{tabular}
\end{table}

\noindent \textbf{$S_{2}$ is tighter than the first-power ratio on all four
models}, the worst within-model spread is $1.67\times$ against a between-model
spread of $4.26\times$, and the ordering by $S_{2}$ reproduces the ordering by
damage exactly. So sensitivity is a model property: \emph{one number per model
predicts what any of four structurally different block encoders will cost it, to
within a factor of $1.67$.} The registration also carried a caution that a
second-order expansion has no right to hold at $p \approx 11.7\%$ with perplexity
rising by up to three quarters. That caution proved unnecessary, which is
informative in itself --- the quadratic regime reaches further into this
amplitude than we expected it to.

Two consequences. For anyone measuring: $S_{2}$ can be calibrated on the cheapest
encoder and used to predict the rest, which is the model-side companion to
Corollary~\ref{cor:designrule}'s codebook-side ranking. For this paper: it
explains why every magnitude in Proposition~\ref{prop:transfer} failed to
transfer while every sign did. \emph{A format's ranking is carried by the
perturbation, which is constant across models; a format's margin is carried by
$S_{2}$, which is not.}

\paragraph{The standard explanation does not survive either, and its failure is
inverted.} Block-level outliers are what the quantisation literature reaches for
here~\cite{awq,mxgap}, and a block whose maximum sits far above its own RMS does
spend most of its codebook on unreachable headroom. Measured by counting, with
nothing quantised:

\begin{center}\small
\begin{tabular}{lrrrr}
\toprule
model & ppl degradation & $\mathbb{E}[a_{\max}/\mathrm{rms}]$ & blocks $>3$ & kurtosis \\
\midrule
GPT-2   & $20.1\%$ & $2.454$ & $9.3\%$ & $\mathbf{76.00}$ \\
Qwen    & $23.8\%$ & $\mathbf{2.465}$ & $9.9\%$ & $5.28$ \\
SmolLM2 & $62.7\%$ & $2.431$ & $8.1\%$ & $4.67$ \\
Pythia  & $\mathbf{75.7\%}$ & $\mathbf{2.404}$ & $6.8\%$ & $7.77$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent The peak ratio orders \emph{exactly opposite} to the damage --- Pythia
lowest and worst, Qwen highest and nearly best --- and it barely varies at all,
$2.404$ to $2.465$, a range of $2.5\%$ against a $3.8\times$ range in damage.
Kurtosis fares no better: GPT-2 carries a kurtosis of $76$, sixteen times the next
model's, and takes the \emph{least} damage of the four. \emph{We registered the
peak-ratio prediction before running it and it failed on all three statistics.}

Two things follow. Empirically, the whole class of explanations resting on the
\emph{shape of the weight distribution} is closed on these four models, since
every such statistic we measured is either constant or anti-ordered. And
methodologically, this is why the squared-error proxy's stability in
Proposition~\ref{prop:transfer} should not be read as a virtue: the proxy is
stable because \emph{the perturbation it measures is stable}, and the perturbation
is not what varies.

\noindent Two cautions. The perplexity rows rest on $146$--$148$ windows where the
squared-error rows rest on millions of blocks, so part of the gap between the two
metrics is sampling and not substance; the $2.21\times$ is far outside anything
that noise explains, the $1.32\times$ is not necessarily. And the ordering, not
the individual ratios, is what two models can establish --- a third would be
needed to call these numbers rather than a direction.

\emph{The practical reading is uncomfortable and we state it against our own
interest.} This paper's own headline for the block axis is a perplexity figure
measured against Algorithm~1, which is the bottom row. The figure in the top row
is less than half as large and roughly ten times more stable between models. A
reader deciding whether to adopt a format should be given the top row; a reader
deciding whether the axis exists at all is well served by either.

This is a second and independent route to the observation this paper already
records --- that on a short block with a shared scale, squared error does not
predict perplexity. The first route compared codebooks and found the ranking
inverts. This one compares \emph{encoders}, changes no codebook at all, and
finds a threefold disagreement about how much a single fix is worth. A design
loop tuned on squared error would have judged the encoder a minor cleanup worth
a fifth of the available gain; on the metric that matters it is worth three
fifths, and it is free.

\emph{So the paper's block-axis recommendation stands on the stronger of the two
readings.} Against a properly encoded MXFP4 the five-trit ladder is worth
$+14.10\%$ of squared error and $+8.99\%$ of perplexity, and the encoder that
must be given to the baseline first is worth more than the ladder is.

\paragraph{Accuracy and area in one calculation, which changes the recommendation.}
The two measurements above --- perplexity from the sweep, area from the placed
appliers --- say something together that neither says alone.

\begin{center}\small
\begin{tabular}{lrrrr}
\toprule
arm, all $4.25$ b/el & ppl & vs MXFP4 & applier LUT & gain per $100$ LUT \\
\midrule
MXFP4, Algorithm 1 & $25.056$ & --- & $207$ & --- \\
\textbf{MXFP4 $+$ argmin encoder} & $22.013$ & $+12.15\%$ & $\mathbf{207}$ & $\mathbf{5.87}$ \\
$\varphi^{k}$ & $23.061$ & $+7.96\%$ & $1{,}244$ & $0.64$ \\
$2^{k/3}$, five trits & $20.034$ & $+20.04\%$ & $1{,}244$ & $1.61$ \\
$2^{k/8}$ & $18.906$ & $+24.55\%$ & $1{,}576$ & $1.56$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent
\textbf{The cheapest arm is $3.6$ times more efficient per LUT than the best
ladder}, and it is not a format at all: an argmin over the same $254$
\texttt{E8M0} codes emits a byte-legal MXFP4 bitstream. Past it the marginal
price is $131$ LUT per point of perplexity for $2^{k/3}$ and $110$ for
$2^{k/8}$.

The recommendation this section reaches is therefore not the one its accuracy
table alone would give. \emph{Fix the encoder first --- it is free, it is legal
MXFP4, and it buys half of what the finest ladder buys.} Take a fractional
ladder only where a further tenth of the \texttt{fp32} gap is worth roughly a
thousand LUT, and then take the finest that fits rather than an intermediate
one: the multiply is the cost and the resolution is nearly free once it is paid.

\emph{And the improvement is not ours to report.} We first wrote that it ``had
been left on the table by its own reference encoder''. It had not: the defect in
Algorithm~1 was named and repaired in the literature before we measured it.
Tseng~et~al.~\cite{tseng2025} identify the clipping bias directly --- values
scaled between $6$ and $8$ are clipped, about $3\%$ of entries --- and
AMXFP4~\cite{amxfp4} reports floor-to-round alone, at fixed codebook, scale
width and group size, buying between $+8.4\%$ and $+33.0\%$ of perplexity across
six models, \emph{and a $13.3\%$ regression on a seventh}. Chhugani~et~al.
\cite{chhugani2026} implement a two-candidate argmin over adjacent
\texttt{E8M0} codes and report it engaging on about $15\%$ of blocks;
Schaefer and Tabak~\cite{schaefer2026} state outright that the block-size
paradox is partly an artefact of naive scale selection. MR-GPTQ~\cite{mrgptq} names the same defect in its abstract --- MXFP4's
power-of-two scale quantisation reduces accuracy --- and repairs it a different
way, with \emph{learned per-block scale shifts} rather than a finer grid. That
is worth distinguishing carefully, because it is the closest published
alternative to the mechanism this section measures: a learned shift adapts each
block independently and costs a trained parameter per block; a finer ladder is
fixed, untrained, and costs a multiplier in the applier. \textbf{We have not
compared them, and a reader choosing between the two axes should know that only
one of them has been priced in silicon here.}

Our $+12.15\%$ sits at the low end of an already published range, from a rule
weaker than the argmin we describe. \textbf{What remains ours on this point is
narrower and we state it at that width}: the attribution ratio ---
that the same fix is worth $20.5\%$ of the achievable squared-error gain and
$60.6\%$ of the achievable perplexity gain --- at fixed codebook, block size and
bit budget. No source we found reports that number.

\paragraph{The origin constant, measured rather than assumed.}
A finer ladder buys resolution by spending range: at step $2^{k/N}$ an eight-bit
field spans $255/N$ octaves against MXFP4's $255$. Whether that is enough is a
question about weights, not about arithmetic, so it was measured --- every
$32$-element block of every two-dimensional projection weight in four models,
$19{,}808{,}256$ blocks in total.

\begin{center}\small
\begin{tabular}{lrrr}
\toprule
model & blocks & index span at $2^{k/3}$ & at $2^{k/16}$ \\
\midrule
SmolLM2-135M & $3{,}317{,}760$ & $26$ & $134$ \\
Qwen2 & $11{,}182{,}080$ & $29$ & $147$ \\
GPT-2 & $2{,}654{,}208$ & $38$ & $195$ \\
Pythia & $2{,}654{,}208$ & $24$ & $126$ \\
\midrule
\textbf{all four, one origin} & $19{,}808{,}256$ & $\mathbf{39/243}$ & $\mathbf{201/255}$ \\
\bottomrule
\end{tabular}
\end{center}

\begin{proposition}[A single origin suffices, and the step has a ceiling]
\label{prop:origin}
Real weight tensors ask for $14$ octaves of block scale. A fixed origin therefore
covers every model measured at both ladders, with $204$ spare codes at
$2^{k/3}$ and $30$ at $2^{k/16}$; and since a step of $2^{k/N}$ needs
$14N+1$ codes of $255$, the finest ladder that fits an eight-bit field is
\[
  N \;\le\; \left\lfloor 254/14 \right\rfloor \;=\; 18 .
\]
\end{proposition}

\noindent
This settles the one hole the block-axis accounting carried. \textbf{The origin is
a compile-time constant, not a per-block or per-tensor field, so the cost stays
at $4.25$ bits per element} --- the earlier phrasing, ``$4.25$ plus an origin
constant MXFP4 does not have'', overstated it: MXFP4 has an origin constant too,
it is $127$, and the only difference is that ours was not yet chosen.

It also bounds the axis. $2^{k/16}$ is within one step of the ceiling, so the
resolution gain measured in this section is close to everything an eight-bit
scale field can give: past $N=18$ a block whose scale falls outside the window
is annihilated, and the range is not recoverable by any choice of origin.

\paragraph{What this section claims.} That at $4.25$ bits per element a finer
scale ladder beats MXFP4 by $17\%$ in squared error on two real models and $20\%$
in perplexity, with zero losing blocks in $14.9$ million; that the gain is
entirely scale resolution; and that a \emph{binary} ladder realises it better
than a ternary one. It does not claim that ternary wins the block axis. The
measurement says otherwise. An earlier draft of this section closed by observing
that a referee would need one afternoon and $2^{k/2}$ to say so first; that
afternoon has now been spent here. The prediction was registered before the run,
in \texttt{research/block/PREREG\_step2\_step4\_2026-08-12.md}: Bennett's law
fitted at $N\in\{3,8\}$ gave $+10.01\%$ at $N=2$ and $+19.75\%$ at $N=4$, with
bands of $10.0$--$11.5\%$ and $19.7$--$20.7\%$. \textbf{Both cardinal
predictions failed, low, by $0.2$ and $1.2$ points} --- which is what
Proposition~\ref{prop:notbennett} then quantified. \emph{The ordinal prediction
held}: the ordering is $\varphi^{k} < 2^{k/2} < 2^{k/3} < 2^{k/4} < 2^{k/8} <
2^{k/16}$ with no inversion, so the section's ordering survives its own
referee.

\section{The instrument resolves seven levels, not twenty-eight}
\label{sec:bands}

Until this point the throughput table stored a median and a spread per row. A
median with a spread cannot answer whether two rows differ, so \emph{no ordering
in this paper had ever been tested}. Every row was re-swept keeping all five
per-seed frequencies, and two things came out of it.

\paragraph{The table had been measured on two harnesses.} Seventeen of
twenty-one rows reproduced their LUT count exactly; four fell. The cause is
exact: two rows were driven from a $128$-bit LFSR and the rest from a $64$-bit
one. Re-measuring \texttt{GFTernary}, which reproduces at $463$ LUT on the narrow
harness, gives $527$ on the wide one --- \textbf{a difference of exactly $64$
LUT, and exactly $64$ for all twenty-one rows}. Two rows had therefore been
ranked against nineteen others while carrying $64$ LUT of harness the others did
not. Everything in this section is re-measured on the wide harness, so the
comparison is between decoders and not between wrappers.

\paragraph{The ranking resolves into seven levels.} Testing each adjacent pair by
whether its five-seed ranges overlap gives seven bands, and one of them holds
twenty formats. \emph{A band is a chain of overlaps, not an equivalence class};
two members at opposite ends of a chain may still be separated, so the useful
test is pairwise and direct.
...
1 & \textbf{GFTernary} \\
2 & \texttt{binary32} \\
3 & twenty formats, from \texttt{fp8~e4m3} to \textbf{GF+8} \\
4 & \texttt{posit8}, \texttt{takum16} \\
5 & \texttt{LNS16} \\
6 & \texttt{posit16} \\
7 & \texttt{posit32}, IBM hex32 \\
\bottomrule
\end{tabular}
\end{center}

\noindent
\textbf{\texttt{GFTernary} is alone in the first band}: $0.1524$--$0.1616$
against \texttt{binary32}'s $0.1376$--$0.1474$, with no overlap. That result
stands. Within band three the fine ordering does not: no reader should conclude
from a rank that \texttt{TNF16} beats \texttt{BNF16}, because the instrument
cannot see the difference.

\paragraph{What is separated from IEEE half at equal or near-equal storage.}
Five of our formats have five-seed ranges entirely above \texttt{binary16}'s
$0.0985$--$0.1071$:

\begin{center}\small
\begin{tabular}{lrl}
\toprule
& vs \texttt{binary16} & five-seed range \\
\midrule
\textbf{GFTernary} & $+51.4\%$ & $0.1524$--$0.1616$ \\
\textbf{TNF16a}    & $+18.6\%$ & $0.1192$--$0.1278$ \\
\textbf{TNF17e}    & $+16.1\%$ & $0.1135$--$0.1238$ \\
\textbf{GF10}      & $+14.4\%$ & $0.1134$--$0.1204$ \\

\textbf{TNF16c}    & $+11.3\%$ & $0.1118$--$0.1210$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent
\texttt{TNF16c} is the sixteen-bit rung at $E_t=5$ \emph{with its
out-of-specification offsets reserved}: equal storage to \texttt{binary16},
every code either defined or flagged, and separated above it. Section~\ref{sec:sixteen}
reported that comparison as $+5.1\%$ from the narrow harness; on the common
harness it is $+11.3\%$, and it is the first claim in this paper to be stated
with a range rather than a point.

\emph{A rank order the instrument cannot resolve is not a result.} The table
above is printed with ranges for that reason, and every ordering claim elsewhere
in this paper that falls inside band three should be read as unresolved.

\section{A ternary exponent never tiles a binary field}
\label{sec:tiling}

Section~\ref{sec:sixteen} ended on a dilemma: rung A's offset field holds $128$
values and its four trits name $81$, so either $37\%$ of the word lies outside
the format and must be guarded, or the exponent is not ternary. The dilemma has
a structural cause, and naming it resolves the case rather than choosing a side.

\begin{theorem}[No exact tiling]
\label{thm:tiling}
For integers $k,m \geq 1$, $3^{k} \neq 2^{m}$. Hence for every rung the offset
field of width $\lceil k\log_2 3\rceil$ has capacity strictly greater than the
$3^{k}$ offsets it must hold, and the wasted fraction
\[
  w(k) \;=\; 1 - \frac{3^{k}}{2^{\lceil k\log_2 3\rceil}}
        \;=\; 1 - 2^{-\left(1-\{k\log_2 3\}\right)}
\]
is strictly positive at every $k$, where $\{\cdot\}$ is the fractional part.
\end{theorem}

\begin{proof}
$3^k$ has $3$ in its factorisation and $2^m$ does not, so unique factorisation
forbids equality for $k,m\geq1$. The capacity is $2^{\lceil x\rceil}$ for
$x=k\log_2 3$ and the content is $2^{x}$; their ratio is $2^{-(\lceil x\rceil-x)}$,
and $\lceil x\rceil - x = 1-\{x\}$ whenever $x\notin\mathbb{Z}$, which
Theorem's first sentence guarantees.
\end{proof}

\begin{corollary}[The waste is the undefined code space]
\label{cor:wasteiscodes}
Because the sign and mantissa fields are full --- every one of their $2^{1+M}$
patterns denotes a distinct value --- the fraction of the \emph{stored word}
that the specification leaves undefined equals the fraction of the offset field
it leaves undefined:
\[
  \frac{2^{\text{width}} - 2\cdot 3^{E_t}\cdot 2^{M}}{2^{\text{width}}}
  \;=\; 1 - \frac{3^{E_t}}{2^{\lceil E_t\log_2 3\rceil}} \;=\; w(E_t).
\]
So $w$ is not an abstract packing loss: it \emph{is} the share of the format's
own codes that name nothing. At $E_t=4$ that is $24{,}064$ of $65{,}536$ codes;
at $E_t=5$, $3{,}328$.
\end{corollary}

\noindent
\textbf{That corollary needs a hypothesis it did not state, and finding the
hypothesis is the useful part.} It holds when every code names at most one value
and every value has one code --- a \emph{canonical} encoding. IEEE 754's decimal
formats do not require that: the surplus patterns are assigned as non-canonical
alternates, and the identity breaks in a way worth writing down.

\begin{proposition}[Two quantities the identity conflates]
\label{prop:twowastes}
Let $C$ be the code space, $V$ the set of values a format defines, and
$d\colon C\to V\cup\{\bot\}$ its decoder. Two ratios are distinct once
$d$ is allowed to be non-injective:
\[
  w_{\text{void}} \;=\; \frac{|d^{-1}(\bot)|}{|C|},
  \qquad
  w_{\text{redundant}} \;=\; 1-\frac{|V|}{|C|} .
\]
Corollary~\ref{cor:wasteiscodes} states $w_{\text{void}} = w(E_t)$, and that is
correct for the guarding and extending encoders, where $d$ is injective on its
domain. Under an assigning encoder $w_{\text{void}} = 0$ by construction while
$w_{\text{redundant}} = w(E_t)$ remains. \textbf{The surplus can be removed from
the code space or from the value space, and $w(E_t)$ counts the second no matter
what is done about the first.}
\end{proposition}

\noindent
So the reservation dilemma has a third horn, it is standardised, and it is
strictly better on one axis: under assignment \emph{no code of the word lies
outside the format}, so a corrupted offset cannot produce a value the
specification does not define --- the failure mode Section~\ref{sec:sixteen}
names. Whether it is better on area is a separate question and is measured
below. What is not in doubt is that a two-horned dilemma in a paper is a prompt
to ask what a shipping standard does about the same problem, and this one had an
answer since 1975~\cite{chenho1975}.

\paragraph{The construction, and it verifies.} For \texttt{TNF16c} the thirteen
surplus offsets $243\ldots255$ alias onto $0\ldots12$ by a conditional subtract.
Swept exhaustively: $62{,}208$ of $62{,}208$ in-specification codes exact, and
all $3{,}328$ surplus codes decode \emph{bit-identically to their canonical
twins}, which is what ``non-canonical alternate encoding'' means and is the
property a decoder must have for the scheme to be legal.

\paragraph{And it was measured, against a prediction written first.} All three
horns were placed on the common harness at fifty-four seeds. The prediction ---
recorded before the sweep finished, and wrong --- was that gate count would
decide it and give $\text{extend} < \text{guard} < \text{assign}$.

\begin{center}\small
\begin{tabular}{lrrrl}
\toprule
horn & LUT & MHz & MHz/LUT & vs \texttt{extend} \\
\midrule
extend & $573$ & $67.87$ & $\mathbf{0.1185}$ & --- \\
\textbf{guard} & $591$ & $68.14$ & $0.1153$ & $-2.66\%$, $p=6.3\times10^{-5}$ \\
assign & $576$ & $59.24$ & $0.1029$ & $-13.17\%$, $p=7.5\times10^{-38}$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent
\emph{Assign is cheaper in area than guard}, which the gate-count argument
forbids, and it loses on \emph{frequency}, $59.24$ against $68.14$. We first
attributed that to the conditional subtract sitting in the critical path. That
explanation is withdrawn: a control substituting a mux to a constant --- zero
arithmetic, same series position --- costs $-12.81\%$, which is $98\%$ of
assign's whole penalty and is statistically indistinguishable from it
($p=0.90$). The penalty is the extra level between the offset field and the
exponent path, not the arithmetic in it. \textbf{We have no mechanism for the
$15$-LUT area gap either}, and two further controls at $588$ and $604$ LUT show
no monotone packing story exists.

So the recommendation is \textbf{guard}: $2.66\%$ buys a format in which every
code is either defined or \emph{flagged}, and the flag is the point. We earlier
called assignment the strongest of the three on correctness; that conflates two
axes and is withdrawn. Under assignment a corrupted offset still yields a
plausible finite value --- now guaranteed legal, hence if anything \emph{less}
detectable --- which is verbatim the failure this paper condemns in the
extending horn. The defence assignment does have is narrower and worth stating
exactly: \emph{only out-of-specification codes alias, so no legal value is ever
harmed}. That is a real property and it is not the same as detectability. That is not a criticism of the
decimal standard: it reasons about interchange between machines, and a format
shipped as a datapath does not face the same cost.

\emph{Both arguments predicted the wrong ordering, in opposite directions, and
the answer turned on placement.} A cost argument about a decoder that does not
mention the critical path is not an argument about the decoder's cost.


\noindent
This is why Proposition~\ref{prop:reserve} bites exactly where $w$ is large: the
codes a rung must guard are the codes $w$ counts, and there is no third place for
them to be.

\begin{corollary}[The waste is uneven, and the good rungs are sparse]
\label{cor:rungs}
$w(k)$ is small exactly where $\{k\log_2 3\}$ is close to $1$, that is where
$3^{k}$ falls just under a power of two. Over $k\leq 15$ the minimum is
$k=5$ at $w=5.1\%$; the maximum is $k=7$ at $w=46.6\%$.
\end{corollary}

\begin{table}[htbp]
\centering
\vspace{0.6em}
\caption{Waste at every rung, and which rungs this work had built. The ladder
had taken $E_t \in \{3,4,6,7\}$, three of them poor, and had never taken $E_t=5$,
which is the best in the range by a factor of three.}
\label{tab:tiling}
\scriptsize
\begin{tabular}{rrrrll}
\toprule
$E_t$ & $3^{E_t}$ &b & capacity & waste & built here \\
\midrule
1 & 3 & 2 & 4 & 25.0\% &  \\
2 & 9 & 4 & 16 & 43.8\% &  \\
3 & 27 & 5 & 32 & 15.6\% & TNF8 \\
4 & 81 & 7 & 128 & 36.7\% & TNF16{,} TNF16a \\
5 & 243 & 8 & 256 & 5.1\% & \textbf{TNF16c} \\
...
\paragraph{The unevenness is equidistribution, and its worst case is ancient.}
$w(k)$ depends on $k$ only through $\{k\log_2 3\}$, which is equidistributed in
$[0,1)$ by Weyl~\cite{weyl1916}; the extremes of $w$ over an interval of $k$ are
therefore governed by the continued-fraction convergents of $\log_2 3$ and their
spacing by the three-distance theorem~\cite{sos1958}. Rungs where $k\log_2 3$
falls just \emph{below} an integer are cheap --- $k=5$ at $5.08\%$, $k=17$ at
$3.78\%$, which is the global minimum for $k\le24$ and which
Compilade~\cite{compilade} also names --- and rungs just \emph{above} are
ruinous. The worst in range is $k=12$ at $\mathbf{49.32\%}$, and it is the
Pythagorean comma: $3^{12}/2^{19}=1.013643$, the near-miss that has defeated
musical temperament since antiquity~\cite{barbour1951}. \emph{A designer
choosing a rung is choosing a temperament.}

\paragraph{The rung the ladder had skipped.} $E_t=5$ needs $8$ offset bits, so
$1+8+7=16$: a third sixteen-bit candidate, and the one this work had not
considered. Built, and checked exhaustively against the ladder reference:
\textbf{$62{,}208$ of $62{,}208$ in-specification codes exact, all $3{,}328$
out-of-specification codes flagged, zero errors.}

\begin{center}\small
\begin{tabular}{lrrrr}
\toprule
& LUT & MHz & MHz/LUT & vs \texttt{binary16} \\
\midrule
rung A, unguarded          & $502$ & $69.87$ & $0.1392$ & $+14.7\%$ \\
rung A, offsets reserved   & $549$ & $64.86$ & $0.1181$ & $-2.6\%$ \\
rung C, unguarded          & $509$ & $66.11$ & $0.1299$ & $+7.1\%$ \\
\textbf{rung C, offsets reserved} & $527$ & $67.20$ & $\mathbf{0.1275}$ & $\mathbf{+5.1\%}$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent
The guarded build of rung A measures $15.1\%$ below its unguarded build and the
guarded rung C only $1.8\%$ below --- but \emph{those percentages are not the
comparator's price}. Synthesised in isolation the guard is
$1$ LUT at $E_t=5$, $2$ at $E_t=4$ and $3$ at $E_t=8$, against modules of some
five hundred, so at most half a percent of the area is the comparison itself.
What changes is the decoder's output image: reserving $47$ offsets collapses
$24{,}064$ codes onto one \textsc{NaN}, and everything downstream is specialised
to the image it observes (Proposition~\ref{prop:pruning}). \textbf{The difference
between a guarded and an unguarded build is a fact about the whole datapath, not
a cost that can be attributed to the guard}, and this paper stated it as the
latter until an adversarial review of the derivation refuted the attribution.
At seventeen bits the same difference proved to lie inside the seed spread
entirely. \textbf{The claim this paper can make at equal storage, with every code either defined or flagged, is the fifty-four-seed $+6.92\%$ of Section~\ref{sec:fiftyfour}} --- smaller than the $14.7\%$ first reported, and unlike it, not resting on an unchecked field. The $+5.1\%$ above is the same rung on five seeds and the narrow harness, and is superseded by it.


\paragraph{The ladder rebuilt.} Corollary~\ref{cor:rungs} is a construction
rule, not only a diagnosis, once the range constraint is added: choose the $k$
of least $w(k)$ among those whose span $3^{k}/2$ covers the class. At $17$ bits
that is $E_t=5$, $M=8$ --- $36.7\%$ waste falls to $5.1\%$ for exactly one
mantissa bit. Generated from the parameters rather than hand-edited, guarded,
and checked exhaustively: \textbf{$124{,}416$ of $124{,}416$ in-specification
codes exact, all $6{,}656$ out-of-specification codes flagged, zero errors.}
...
\midrule
TNF16, as built    & $4$ & $36.7\%$ & $0.1005$ & no \\
\textbf{TNF17e}    & $5$ & $5.1\%$  & $\mathbf{0.1207}$ & \textbf{yes} \\
\bottomrule
\end{tabular}
\end{center}

\noindent
$20.1\%$ faster per LUT \emph{while} checking its offset field, which the rung
it replaces does not --- both figures re-measured on the common harness of
Section~\ref{sec:bands}. The same rule sends the $65$-bit rung to $E_t=8$, but
that rung is withdrawn: an independent review found its exponent path keeps
eight bits of an exponent spanning $\pm3279$, so \texttt{off} and
\texttt{off}${}+256$ decode identically. \texttt{TNF32} and \texttt{TNF64} carry
the same defect and are withdrawn with it; \texttt{TNF16c} and \texttt{TNF17e}
do not, their exponents spanning $\pm121$ inside \texttt{fp32}'s window.

The dilemma of Section~\ref{sec:sixteen} does not need to be settled because it
was an artefact of standing on $E_t=4$. Theorem~\ref{thm:tiling} says no rung
escapes waste; Corollary~\ref{cor:rungs} says the ladder should be built on the
$k$ where the waste is least, and this work had built it on three of the worst
and skipped the best.

\section{A rung at exactly sixteen bits, and what winning there costs}
\label{sec:sixteen}

Section~\ref{sec:confound} left this work with one honest comparison and an
apology attached to it: our nearest rung to the sixteen-bit competitors was
seventeen bits wide, so every same-width claim carried a one-bit caveat. A
rung's stored width is $1 + \lceil E_t \log_2 3\rceil + M$, and exactly two
assignments reach sixteen:

\begin{center}\small
\begin{tabular}{clrrr}
\toprule
& $E_t$ & offsetb & $M$ & dynamic range \\
\midrule
A & $4$ & $7$ ($81$ offsets) & $8$ & $2^{-39}\ldots2^{39}$ \\
B & $3$ & $5$ ($27$ offsets) & $10$ & $2^{-12}\ldots2^{12}$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent
Which is \emph{the} sixteen-bit member is a measurement, not a preference, so
both were built and both were checked exhaustively against the ladder
reference: \textbf{$65{,}534$ of $65{,}534$ exact, each.} Rung A wins on
throughput, $0.1392$ against $0.1144$, and it wins its width class outright:

\begin{center}\small
\begin{tabular}{lr}
\toprule
\textbf{TNF16a} (16 bits) versus & \\
\midrule
\texttt{binary16} & $\mathbf{+14.7\%}$ \\
\texttt{takum16}  & $+86.3\%$ \\
\texttt{LNS16}    & $+112.8\%$ \\
\texttt{posit16}  & $+152.6\%$ \\
\bottomrule
...
\noindent
This is the first same-width win in the catalogue against IEEE, and it is the
result the previous section said the paper did not yet have. It also has a price
that must be printed beside it.

\paragraph{The price, as estimated and as measured.}
This section first reported that rung A's win was bought with code space: four
trits give $81$ offsets where the binary field holds $128$, so $37.5\%$ of the
offset codes lie outside the format, and the rung would deliver $15.32$
effective bits against \texttt{binary16}'s sixteen. \emph{That estimate was
computed, not measured, and measurement refutes it.}

Sweeping every code through the RTL and counting distinct outputs gives rung A
$64{,}518$ distinct values of $65{,}536$ codes: \textbf{$\codeuse{TNF16a}{98.4}$
utilisation}, against \texttt{binary16}'s $\codeuse{binary16}{100.0}$. The out-of-specification offsets are not
discarded --- the decoder assigns each of them a distinct value. The win over
IEEE half at equal storage costs $1.6\%$ of code space, not $37.5\%$, and the
result is stronger than this section originally claimed.

It leaves a different defect in view. \emph{The decoder does not reserve the
$47$ offsets that lie outside the format}: a corrupted offset field yields a
plausible finite value rather than a signal. Every other rung in the ladder
shares this, and no test in this work would have found it, because a decoder
that answers every input answers every test.

\begin{table}[htbp]
\centering
\vspace{0.6em}
\caption{Code utilisation, measured: distinct decoder outputs over codes swept.
Exhaustive at $2^{18}$ codes and below, uniformly sampled at $200{,}000$ above,
with the sweep kind stated so a partial sweep never reads as a complete one.}
\label{tab:codeuse}
\scriptsize
\begin{tabular}{lrrl}
\toprule
format & distinct & codes & utilisation \\
\midrule
GF+8 & 454 & 1{,}024 & 44.3\% \\
IBM hex32 & 105{,}526 & 200{,}000 & 52.8\% (sampled) \\
TNF16a-guarded & 40{,}454 & 65{,}536 & 61.7\% \\
GFTernary & 3 & 4 & 75.0\% \\
...
VAX F & 199{,}211 & 200{,}000 & 99.6\% (sampled) \\
posit32 & 199{,}964 & 200{,}000 & 100.0\% (sampled) \\
binary32 & 199{,}997 & 200{,}000 & 100.0\% (sampled) \\
...
\paragraph{The waste decides whether a guarded rung beats IEEE half.}
Corollary~\ref{cor:wasteiscodes} says $w(E_t)$ \emph{is} the share of a rung's
codes that name nothing, and Proposition~\ref{prop:reserve} says those codes must
be guarded. Putting the two together predicts that a rung's advantage over
\texttt{binary16} should survive guarding in proportion to how little it wastes.
Both sixteen-bit rungs were built in guarded and unguarded form and swept at
fifty-four seeds on the common harness:

\begin{center}\small
\begin{tabular}{lrrrl}
\toprule
16-bit rung & $w(E_t)$ & unguarded & guarded & guarded vs \texttt{binary16} \\
\midrule
\textbf{TNF16a} ($E_t{=}4$) & $36.7\%$ & $0.1214$ & $0.1079$ & $+0.10\%$, $p=0.90$ \\
\textbf{TNF16c} ($E_t{=}5$) & $5.1\%$  & --- & $0.1152$ & $\mathbf{+6.92\%}$, $p_{\text{Holm}}=6.9\times10^{-21}$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent
Guarding rung A costs it $11.10\%$ ($p=9\times10^{-59}$) and lands it on
\texttt{binary16} exactly: $0.1079$ against $0.1078$, indistinguishable. Rung C,
wasting seven times less, keeps $+6.92\%$ after paying the same kind of price.
\textbf{The rung that wins at equal storage is the low-waste rung, and it is the
one Corollary~\ref{cor:rungs} names from the arithmetic alone} --- the theory
picked the winner before the silicon was built, which is the strongest form this
paper's central quantity has been tested in.

\paragraph{What the win rests on.}
The unguarded decoder gives each of the $47$ out-of-specification offsets a
distinct finite value, and that is where the same-width win comes from. Guarding
them --- a comparator on the offset field, invalid codes signalling
\textsc{NaN} --- was built and swept: $41{,}472$ of $41{,}472$ in-specification
codes exact, all $24{,}064$ out-of-specification codes flagged, zero errors.
...
\noindent
\texttt{binary16} measures $0.1213$. So the rung beats IEEE half by $14.7\%$
while its invalid codes are undetectable, and loses to it by $2.6\%$ once they
are detected. \textbf{The win at equal storage depends on not checking the
offset field.} IEEE half needs no such check --- all $65{,}536$ of its codes are
defined --- so the comparison is between a format that must pay for a guard and
one that has nothing to guard.

\begin{proposition}[The reservation cost]
\label{prop:reserve}
Let a format's specification cover $S \subsetneq C$ of its code space. Any
implementation either (i) guards $C\setminus S$, paying the logic to recognise
it, or (ii) assigns values to $C\setminus S$, which extends the specification to
$C$. There is no implementation that preserves both the specification and the
cost of (ii).
\end{proposition}

This puts a question under the ladder rather than beside it. Four trits name
$81$ offsets; the field that holds them is seven bits wide and names $128$.
Either the exponent is ternary and $37\%$ of the word's codes lie outside the
format --- in which case Proposition~\ref{prop:reserve} charges us $15.1\%$ and
the IEEE comparison is lost --- or the exponent is a seven-bit binary field, all
codes are defined and the win stands, but then \emph{the ternary structure this
paper is named for does not reach $37\%$ of its own code space}. Both readings
are defensible and they cannot both be used at once, which is why the number
appears twice above instead of once.

\emph{We have no measurement that settles it, and say so rather than choosing
the reading that flatters the result.}

\paragraph{And the mechanism this section proposed is not there.}
The claim was that a throughput metric rewards a format for discarding code
space, resting on Proposition~\ref{prop:pruning} and three points. Across all
$23$ formats measured, the correlation between utilisation and MHz/LUT is
$\mathbf{+0.173}$ --- $t=0.80$ on $21$ degrees of freedom, not significant, and
of the \emph{opposite} sign to the one predicted. The two least efficient
formats in the catalogue, \texttt{GF+8} at $\codeuse{GF+8}{44.3}$ and IBM hex32 at
$\codeuse{IBM hex32}{52.8}$, sit at ranks $17$ and $22$.

\textbf{The hypothesis is withdrawn.} Proposition~\ref{prop:pruning} remains
true as stated --- a smaller image does permit specialisation --- but the
inference that discarded codes therefore buy measurable throughput does not
survive contact with twenty-two measurements. Three points agreed with it and
twenty-three do not, which is the entire reason to prefer measuring a caveat over
computing one.

One correction belongs to our own headline. This paper stated that
\texttt{GFTernary} ``uses all four of its codes and discards nothing''. It does
not: $\{-\varphi, 0, +\varphi\}$ is three values in two bits, and the measured
utilisation is $\mathbf{\codeuse{GFTernary}{75.0}}$, the third lowest in the catalogue. Its first
place stands on the throughput measurement, which is unaffected, but the
sentence defending it was false and is retracted.

\section{When a conformance test measures itself}
\label{sec:wellposed}

Three of the disagreements this work reported between a decoder and its
reference were not disagreements about the decoder. They are worth stating as a
proposition because the failure is silent: every number involved is correct, and
the test has no signal that could reveal the error.

\begin{theorem}[Well-posedness of conformance]
\label{thm:wellposed}
Let $f\colon C\to\mathbb{R}$ be a format's exact value map on its code space,
let $D\colon C\to F$ be a decoder into a finite floating type $F\subset
\mathbb{R}$, and let $\rho\colon\mathbb{R}\to F\cup\{\bot\}$ be correct rounding,
with $\rho(x)=\bot$ when $x$ has no finite image in $F$. Conformance of $D$ is
the predicate
\[
  D(c) \;=\; \rho(f(c)) \qquad\text{on}\qquad \{c\in C : \rho(f(c))\neq\bot\}.
\]
The predicate $D(c)=f(c)$ is not equivalent to it. It fails at every $c$ with
$f(c)\notin F$, and that set is determined by the dynamic ranges of $f$ and $F$
alone --- it is the same set for a correct $D$ and for an arbitrary one.
\end{theorem}

\begin{proof}
$F$ is finite and $f(C)\subset\mathbb{R}$, so $f(c)\in F$ holds only on the
codes whose exact value is a member of $F$. For $c$ outside that set,
$D(c)\in F$ gives $D(c)\neq f(c)$ whatever $D$ is, including $D=\rho\circ f$.
Hence the second predicate is false on a set independent of $D$, while the
first is true on all of the restricted domain when $D=\rho\circ f$.
\end{proof}

\begin{corollary}[A floor under spurious error]
\label{cor:floor}
A conformance test that omits $\rho$ reports at least $|\{c:f(c)\notin F\}|/|C|$
of the code space as error, for every decoder. For IBM hex32, whose range is
$16^{\pm 64}$, against \texttt{fp32}, this floor is $45.0\%$ measured on a
uniform sample of $40{,}000$ codes; the $28\%$ previously reported for that
decoder lies entirely beneath it and was therefore uninformative. Applying
$\rho$ moves the comparable set from $22{,}001$ to $30{,}245$ codes and the
disagreement to zero.
\end{corollary}

\begin{proposition}[Variant distance is not decoder error]
\label{prop:variant}
Let $D$ implement variant $f_1$ while the reference computes $f_2\neq f_1$.
The measured disagreement is $|\{c:\rho(f_1(c))\neq\rho(f_2(c))\}|$, a property
of the pair $(f_1,f_2)$ alone. No conformance test can separate this from
decoder error, because agreement is its only signal and both variants are
internally consistent. The variant must therefore be fixed by a declaration
external to the test.
\end{proposition}

Proposition~\ref{prop:variant} is not hypothetical here. Two formats in this
catalogue publish more than one variant, and both were compared against the
wrong one. \texttt{takum16} implements the logarithmic takum, value
$(-1)^{S}e^{\ell/2}$, and was compared against the linear one, value
$(1+M/2^{p})2^{c}$: reported disagreement $98.7\%$, actual $632$ of $61{,}505$,
all at \texttt{fp32}'s denormal floor. \texttt{lns16} implements scale $128$ and
was compared against scale $256$: reported $253$, actual $107$, of which three
are a zero-and-NaN convention and $104$ are sub-\textsc{ulp} precision in the
subnormal range. Each RTL states its variant in its header comment, and reading
it would have cost nothing.

The remedy is mechanical and is now enforced. \texttt{conformance/variant\_map.json}
records, per format, the reference module compared against and a token that must
appear in the decoder's own header; \texttt{tools/check\_variant\_declared.py}
fails if a declaration is missing or if the map and the header disagree. The
gate was verified by binding \texttt{lns16} to the wrong scale and confirming it
goes red.

\begin{table}[htbp]
\centering
\vspace{0.6em}
\caption{Reported disagreement against disagreement measured well-posedly. The
first column is what this work published; the second is what the same decoder
scores once the reference is rounded to the decoder's type and the declared
variant is used. No decoder was changed to produce the second column except
where the third column says so.}
\label{tab:wellposed}
\scriptsize
\begin{tabular}{lrrl}
\toprule
format & reported & well-posed & what the difference was \\
\midrule
takum16   & $98.7\%$ & $632/61{,}505$ & linear reference, logarithmic decoder \\
IBM hex32 & $28\%$   & $0/30{,}245$   & no $\rho$; subnormal path then added \\
LNS16     & $253$    & $107/65{,}536$ & scale-256 reference, scale-128 decoder \\
VAX F     & $151$    & $0/40{,}000$   & no $\rho$; subnormal path then added \\
posit16   & $4$      & $0/65{,}535$   & two regime defects at saturation, repaired \\
posit32   & $36$     & $0/40{,}000$   & one casez arm unreachable, repaired \\
\bottomrule
\end{tabular}
\end{table}

All six are now zero or explained, and the two that were real were repaired
rather than reported. That ratio
is the useful part of this section: a test that cannot be wrong about itself
would have found two defects instead of six, and the four false ones cost more
attention than the two real ones. 
\paragraph{The two real defects, and the channel that had already reported one.}
\texttt{posit16} was wrong at exactly four codes, the two saturation values and
their negations, each by $2^{\pm2}$ --- one step of an $es=2$ exponent field.
Two independent off-by-ones caused it: the regime-plus-terminator length was
capped one short when the run fills the word, so the terminator bit was read as
the exponent; and the all-zero branch of the leading-count assigned $k=13$ where
a run of fifteen identical bits gives $k=14$, its own comment saying
\emph{$k=+14$} beside code that produced $13$.

\texttt{posit32}'s $48$ wrong values had a plainer cause. One \texttt{casez}
pattern was written thirty-two characters wide in a thirty-one-bit case. Verilog
truncates from the left, which made the $lzc=11$ arm character-for-character the
arm above it --- unreachable --- so every code with exactly eleven leading zeros
fell through to the default. It was misread as regime arithmetic for two
iterations.

The compiler had said so on every single run:
\begin{center}
\texttt{warning: Extra digits given for sized binary constant.}
\end{center}
The harness redirected compiler output to a log file, and the log was read only
when something failed. \emph{A warning nobody reads is not a warning}; it is an
instrument whose output is disconnected, which is the same defect as an
instrument inside the failure domain and is harder to notice, because the
instrument is working perfectly. \texttt{tools/check\_literal\_widths.py} now
turns this class into an exit code: it checks $188{,}021$ sized literals, gates
the paths this paper measures, and prints --- rather than hides --- the $41$
occurrences it finds in test benches outside that scope.

\emph{The instrument was inside the failure
domain it was measuring} --- the same error, in a different medium, as
diagnosing a network fault with a tool that runs over that network.
...
\bibitem{deklerk2017grid} E. de Klerk, M. Laurent, Z. Sun, ``On the convergence rate of grid search for polynomial optimization over the simplex,'' \emph{Optimization Letters} 11 (2017), 597--608.

\bibitem{griewank2019kink} A. Griewank, A. Walther, ``Relaxing kink qualifications and proving convergence rates in piecewise smooth optimization,'' \emph{SIAM J. Optimization} 29(1) (2019), 262--289.
...
\bibitem{ternac1} G.~Frieder, ``Ternary computers: Part I: motivation for
  ternary computers,'' \emph{MICRO-5}, 1972. DOI 10.1145/776378.776392.

\bibitem{ternac2} G.~Frieder and C.~Luk, ``Ternary computers: Part 2: emulation
  of a ternary computer,'' \emph{MICRO-5}, 1972. DOI 10.1145/776378.776393.
  TERNAC: a 48-trit floating-point format with a 42-trit mantissa and a
  \emph{six-trit exponent}, emulated on binary hardware.

\bibitem{bctparhami} B.~Parhami and M.~McKeown, ``Arithmetic with Binary-Encoded
  Balanced Ternary Numbers,'' \emph{Asilomar Conf.\ on Signals, Systems and
  Computers}, 2013, pp.~1130--1133.

\bibitem{compilade} Compilade, ``How to pack ternary numbers in 8-bit bytes,''
  \texttt{compilade.net/blog/ternary-packing}, 26 June 2024. Publishes the
  five-trits-in-eight-bits packing and its $99.06\%$ efficiency, which is
  $1-w(5)$ in the notation of this paper.

\bibitem{tereffic} C.~Yin, Z.~Bai, P.~Venkatram, S.~Aggarwal, Z.~Li, and
  T.~Mitra, ``TerEffic: Highly Efficient Ternary LLM Inference on FPGA,''
  arXiv:2502.16473, 2025. Builds the packed-$3^5$-in-eight-bits decoder in FPGA
  LUTs, \S III-B.

\bibitem{knuthv2} D.~E.~Knuth, \emph{The Art of Computer Programming, Vol.~2:
  Seminumerical Algorithms}, \S4.1, Addison-Wesley, 3rd ed., 1997.

\bibitem{setun} N.~P.~Brusentsov and J.~Ramil Alvarez, ``Ternary Computers: The
  Setun and the Setun 70,'' \emph{IFIP SoRuCom}, AICT vol.~357, 2006,
  pp.~74--80.

\bibitem{chenho1975} T.~C.~Chen and I.~T.~Ho, ``Storage-efficient
  representation of decimal data,'' \emph{Communications of the ACM}, vol.~18,
  no.~1, pp.~49--52, 1975.

\bibitem{dpd2002} M.~F.~Cowlishaw, ``Densely packed decimal encoding,''
  \emph{IEE Proceedings --- Computers and Digital Techniques}, vol.~149, no.~3,
  pp.~102--104, 2002.

\bibitem{ieee754} IEEE Std 754-2019, \emph{IEEE Standard for Floating-Point
  Arithmetic}, 2019. Decimal interchange formats and non-canonical encodings,
  \S3.5 and \S5.12.

\bibitem{weyl1916} H.~Weyl, ``\"Uber die Gleichverteilung von Zahlen mod.
  Eins,'' \emph{Mathematische Annalen}, vol.~77, pp.~313--352, 1916.

\bibitem{sos1958} V.~T.~S\'os, ``On the distribution mod 1 of the sequence
  $n\alpha$,'' \emph{Ann.\ Univ.\ Sci.\ Budapest}, vol.~1, pp.~127--134, 1958.

\bibitem{barbour1951} J.~M.~Barbour, \emph{Tuning and Temperament: A Historical
  Survey}, Michigan State College Press, 1951.

\bibitem{savoj1990} H.~Savoj and R.~K.~Brayton, ``The use of observability and
  external don't cares for the simplification of multi-level networks,''
  \emph{Design Automation Conference}, 1990, pp.~297--301.

\bibitem{bennett1948} W.~R.~Bennett, ``Spectra of quantized signals,''
  \emph{Bell System Technical Journal}, vol.~27, no.~3, pp.~446--472, 1948.

\bibitem{grayneuhoff1998} R.~M.~Gray and D.~L.~Neuhoff, ``Quantization,''
  \emph{IEEE Transactions on Information Theory}, vol.~44, no.~6,
  pp.~2325--2383, 1998.

\bibitem{microexp} B.~Darvish Rouhani et al., ``With Shared Microexponents, A
  Little Shifting Goes a Long Way,'' \emph{ISCA}, 2023. arXiv:2302.08007.

\bibitem{adaptivfloat} T.~Tambe et al., ``Algorithm-Hardware Co-Design of
  Adaptive Floating-Point Encodings for Resilient Deep Learning Inference,''
  \emph{DAC}, 2020. arXiv:1909.13271.

\bibitem{tseng2025} A.~Tseng, T.~Yu, and Y.~Park, ``Training LLMs with MXFP4,''
  \emph{AISTATS}, 2025. arXiv:2502.20586. Identifies the clipping bias of the
  MX scale-selection rule.

\bibitem{amxfp4} S.~Lee et al., ``AMXFP4: Taming Activation Outliers with
  Asymmetric Microscaling Floating-Point,'' arXiv:2411.09909. Table~18 isolates
  floor-versus-round scale selection at fixed codebook and group size.

\bibitem{chhugani2026} J.~Chhugani et al., ``Overflow-aware scaling for
  microscaling formats,'' arXiv:2603.08713.

\bibitem{schaefer2026} C.~Schaefer and E.~Tabak, ``Scale selection and the
  block-size paradox in microscaling formats,'' arXiv:2605.08565.

\bibitem{fasoli2026} A.~Fasoli et al., ``On block size in microscaling
  quantisation,'' arXiv:2601.19026.

\bibitem{lecun1990} Y.~LeCun, J.~S.~Denker, and S.~A.~Solla, ``Optimal Brain
  Damage,'' \emph{NeurIPS}, 1990, pp.~598--605.

\bibitem{hassibi1993} B.~Hassibi and D.~G.~Stork, ``Second order derivatives for
  network pruning: Optimal Brain Surgeon,'' \emph{NeurIPS}, 1993, pp.~164--171.

\bibitem{adaround} M.~Nagel, R.~A.~Amjad, M.~van Baalen, C.~Louizos, and
  T.~Blankevoort, ``Up or Down? Adaptive Rounding for Post-Training
  Quantization,'' \emph{ICML}, 2020.
