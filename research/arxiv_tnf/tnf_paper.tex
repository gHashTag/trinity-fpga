...
Among the twenty-five carrying their own range --- those a training run can
live in --- GFTernary leads on throughput per area, by $10.9\%$ over the next and $5.24\times$ over \texttt{posit32}, with thirteen of the twenty-five ours.  On an XC7A200T a ternary neuron
...
A guarded 16-bit rung beats IEEE half by $6.92\%$. Ten retractions are marked in place below.
\end{abstract}
...
Ten of our own claims were falsified by the measurements meant to support them,
...
unsynthesised logic. \textbf{Resolution.} The pooled coefficient of variation across seeds is $3.71\%$, so two rows separate at $\alpha=.05$ only when their MHz/LUT differ by more than $5.50\%$ relative; smaller gaps in this table are not results. The detection floor for a difference in \emph{decoder} cost is $41.3$ LUT, which is $79\%$ of a whole decoder at the median row --- the $472$-LUT harness inflates that floor tenfold. Every row is measured on the same $128$-bit harness; rows that were not have been re-measured or withdrawn.}\label{tab:fullthroughput}
\small
\tiny
...
therefore exact by construction, and its $472$ LUT are the harness alone: the
LFSR, the accumulator, and the output fold, with no decoder in them at all.

That number is a calibration this work had never taken. Every row of the table
lies between $463$ and $1{,}008$ LUT, so \emph{the harness is between $47\%$ and
$102\%$ of every measurement}, and for the top ten rows it is never below $80\%$.
...
almost entirely with $M_f$ and the decoder's cost is compressed out of it. Here
$H=472$ and $D_f\in[-9,536]$, with $D_f<115$ for the top ten rows.
\end{proposition}
...
\texttt{GFTernary} takes $2$ input bits and measures $463$ LUT --- \emph{nine
...
Of the nine width classes, six contain only our formats and two contain only
other people's. \emph{One class contains both}: at $8$ bits \texttt{GF+8}
measures $0.1004$ against \texttt{fp8~e4m3}'s $0.1254$, and we lose by $19.9\%$.
...
Six of our formats have five-seed ranges entirely above \texttt{binary16}'s
$0.0985$--$0.1071$:
...
\textbf{TNF32}     & $+13.2\%$ & $0.1137$--$0.1243$ \\
...
entirely. \textbf{The claim this paper can make at equal
storage, with every code either defined or flagged, is $+5.1\%$ against IEEE
half} --- smaller than the $14.7\%$ first reported, and unlike it, not resting
