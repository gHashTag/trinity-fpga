...
\noindent
$\sqrt{\varphi}^{\,k}$ at $2.8808$ points per binade lands beside $2^{k/3}$ at $3.0000$, which is what a law of step size predicts and what a law of algebra does not. That is the expected behaviour of a logarithmic companding law: distortion falls with the square of the step and the step is all that enters~\cite{bennett1948,grayneuhoff1998}, so the measurement confirms a known result rather than reporting a surprise. \textbf{$\varphi$ is not special \emph{for accuracy} on this axis; it is a step finer than a binade.} And a plain binary ladder at the same eight bits beats the ternary one by five to six points of RMSE and $1.1$ of perplexity. At matched dynamic range, $243$ ternary codes lose to $255$ binary ones on \emph{six of six} range settings across four sources.
...
measurement says otherwise, and a referee would need one afternoon and
$2^{k/2}$ to say so first.
