...
\begin{tabular}{lrrr}
\toprule
encoder's share of the achievable gain & squared error & perplexity & disagreement \\
\midrule
SmolLM2 & $20.5\%$ & $58.7\%$ & $2.87\times$ \\
Qwen    & $17.1\%$ & $23.3\%$ & $1.36\times$ \\
GPT-2   & --- & $\mathbf{-7.2\%}$ & sign inversion \\
\bottomrule
\end{tabular}
\end{center}

\noindent The squared-error column reproduces the figure above exactly; the
perplexity column reads $58.7\%$ against the $60.6\%$ measured on the earlier
harness. \textbf{But ``three times apart'' is SmolLM2's number. On Qwen the two
metrics are $1.36$ times apart.} The sign replicates --- the encoder is worth
more on perplexity than on squared error, on both models --- and the magnitude
does not, which is the third time in this paper that a comparison has behaved
that way.
...
\begin{tabular}{lrrrr}
\toprule
claim variant & SmolLM2 & Qwen & GPT-2 & spread \\
\midrule
squared error, best encoder both sides & $14.10\%$ & $14.01\%$ & --- & $\mathbf{1.01\times}$ \\
squared error, against Algorithm~1     & $17.11\%$ & $16.42\%$ & --- & $1.04\times$ \\
perplexity, best encoder both sides    & $9.50\%$  & $7.18\%$  & $7.19\%$ & $\mathbf{1.32\times}$ \\
perplexity, against Algorithm~1        & $20.25\%$ & $9.16\%$  & $6.74\%$ & $\mathbf{3.00\times}$ \\
\bottomrule
\end{tabular}
\end{center}
...
four is the combination most often quoted.} The third model sharpens the second
axis rather than blunting it: adding GPT-2 leaves the best-against-best row at
$1.32\times$ and widens the against-Algorithm-1 row from $2.21\times$ to
$3.00\times$.
\end{proposition}
