...
Six of our own claims were falsified by the measurements meant to support them,
...
\paragraph{The price.} A ternary exponent of four trits has $81$ values and its
binary field holds $128$: \emph{thirty-seven percent of rung A's offset codes
lie outside the format}. Counting representable values, rung A uses $40{,}960$
of $65{,}536$ codes and delivers $15.32$ effective bits where \texttt{binary16}
delivers close to sixteen. \textbf{The win is $+14.7\%$ of throughput for
$-0.68$ bits of code space.}

That trade is not incidental, and Proposition~\ref{prop:pruning} already
predicts its direction. A decoder whose image is smaller needs less logic to
produce it, and logic observing that image may specialise to it --- and an image
can be small because the input is narrow \emph{or because the format discards
codes}. The mechanism is the same; only the cause differs.
...
\midrule
\texttt{GF+8} pocket 11, as published & $93.4\%$ & $+19.2\%$ \\
\textbf{TNF16a} vs \texttt{binary16}  & $37.5\%$ & $+14.7\%$ \\
\textbf{TNF16b} vs \texttt{binary16}  & $18.8\%$ & $-5.7\%$ \\
\bottomrule
\end{tabular}
\end{center}
...
correlation of $+0.81$ carries no significance. The claim rests on the mechanism,
which is deductive, and the points are consistent with it.

The consequence for reading this paper's own table is direct. \emph{A throughput
metric rewards a format for discarding code space}, so no ranking on MHz/LUT
alone should be read as a ranking of format quality. Our own first place is
unaffected --- \texttt{GFTernary} uses all four of its codes and discards
nothing --- but rung A's third place is bought in part with codes it does not
spend, and saying so is the difference between a result and an advertisement.
