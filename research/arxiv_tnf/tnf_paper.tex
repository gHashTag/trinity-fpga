...
Six retractions are marked in place below.
\end{abstract}
...
geometric  & $-1$ per doubling             & $8$  & takum $\times 4$, takum-layout ``tekum'' oracles $\times 3$ \\
\bottomrule
\end{tabular}
\end{table}
...
Superseded: the takum column was produced by an oracle wrong on every negative
code (fixed in \#606), and TNF16 stores 17 bits against takum16's 16; the
equal-width re-measurement is a three-way tie --- see the erratum below.}
\label{tab:acc}
\begin{tabular}{lrrrr}
\toprule
magnitude & GF16 ($\varphi$) & TNF16 (stores 17 bits) & takum16 (pre-\#606 oracle) & TNF16 vs takum16, superseded \\
\midrule
...
\paragraph{Erratum (2026-08-18): the ratios above are superseded.} Three
defects, each found after this table was measured, remove the claimed
advantage. First, the takum oracle it was measured against negated wrongly on
every negative code --- negation in takum flips the exponent sign --- and the
fixed oracle now matches libtakum on $65{,}534$ of $65{,}535$ codes (\#606).
Second, TNF16 as specified does not store 16 bits: four trits pack into seven,
so the rung stores 17, and every named rung is wide of its name --- TNF8
stores 10, TNF32 30, TNF64 65, TNF1024 1025. The exact-width rungs are
TNF(2,3) at 8 bits, TNF(4,8) at 16 and TNF(4,24) at 32. Third, re-measured at
equal stored width with the corrected oracles --- accumulation of 64 terms
over $\pm3$ decades --- the 16-bit class is a three-way tie: tekum10 ($15.85$
stored bits) at $8.56\mathrm{e}{-3}$, takum16 at $5.70\mathrm{e}{-3}$,
TNF(4,8) at $5.32\mathrm{e}{-3}$; the 32-bit class reads
$1.43/1.26/1.20\mathrm{e}{-7}$ for tekum20/takum32/TNF(4,24). No format wins
by more than the width it gives up. What survives is not accuracy but
structure: the fixed field's adder and decode (see Limitations) and the
...
stores 17 bits --- four trits pack into seven --- so it sits in this class by
...
takum16 (pre-\#606 oracle)  & $3.27\mathrm{e}{-4}$ & $1.00\mathrm{e}{-3}$ & $1.95\mathrm{e}{-3}$ \\
...
in any row is worse than the unfilled rung. The takum16 column is superseded:
it was measured with the pre-\#606 oracle, and neither variant stores 16 bits
--- $E_t{=}4$ packs into 7 bits, so $M{=}9$ stores 17 and the adopted $M{=}11$
stores 19. At equal stored width the corrected measurement is a tie (see the
erratum in Section~\ref{sec:accuracy}).}
...
spends at sixteen bits. So $M = 11$: the error falls by exactly $4.00$. The takum16 ratios this
paragraph once reported ($0.92\times/2.88\times/5.49\times$ moving to
$3.70\times/11.21\times/22.68\times$) are withdrawn: they were measured
against the pre-\#606 oracle, wrong on every negative code, and at unequal
storage --- the adopted rung stores 19 bits against takum16's 16. At equal
stored width the corrected measurement is a tie, TNF(4,8) at
$5.32\mathrm{e}{-3}$ against takum16's $5.70\mathrm{e}{-3}$. In
...
spent; against takum32 it is a table of $3.15$\,Mbit not instantiated. This is
now the load-bearing statement against takum: the accuracy result it once
outranked has since been re-measured to a three-way tie at equal stored width
(erratum, Section~\ref{sec:accuracy}), while this one has been joined by a
measured adder --- $397$ against $1{,}182$ LUT at the 16-bit class,
$2.98\times$ --- and Theorem~\ref{thm:floor} places no range bound on either.
\end{corollary}
...
wiring. What is visible in that source, and which we report as structure
