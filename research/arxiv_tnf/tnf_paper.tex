...
advantage is smaller and it is real.}
\label{tab:tnet}
...
8 bits & posit8 & $540$ & $44.33$\,MHz & $0.0821$ & --- \\
\addlinespace
16 bits & \textbf{TNF16} & $\mathbf{565}$ & $66.28$\,MHz & $\mathbf{0.1173}$ & $\mathbf{2.07\times}$ \\
16 bits & posit16 & $712$ & $40.38$\,MHz & $0.0567$ & --- \\
\addlinespace
32 bits & \textbf{TNF32} & $\mathbf{569}$ & $66.91$\,MHz & $\mathbf{0.1176}$ & $\mathbf{3.99\times}$ \\
32 bits & posit32 & $953$ & $28.16$\,MHz & $0.0295$ & --- \\
\bottomrule
\end{tabular}
\end{table}

$1.68\times$ in area and $2.38\times$ in frequency, so $3.98\times$ in throughput
...
\midrule
1 & \textbf{GFTernary} (ours) & 2 bits & 463 & 83.19 & \textbf{0.1797} & $6.61$ & all 4 codes \\
2 & binary32 & 32 bits & 472 & 77.00 & 0.1631 & $6.37$ & not swept \\
3 & fp8 e5m2 & 8 bits & 480 & 67.06 & 0.1397 & $5.46$ & 6 of 256 wrong \\
4 & VAX F & 32 bits & 527 & 73.51 & 0.1395 & $4.43$ & not swept \\
5 & fp8 e4m3 & 8 bits & 485 & 67.02 & 0.1382 & $6.78$ & 14 of 256 wrong \\
6 & \textbf{GF10} (ours) & 10 bits & 533 & 72.16 & \textbf{0.1354} & $5.08$ & all 1{,}024 \\
7 & binary16 & 16 bits & 522 & 63.30 & 0.1213 & $7.74$ & all 65{,}536 \\
8 & \textbf{GF14} (ours) & 14 bits & 587 & 71.15 & \textbf{0.1212} & $4.54$ & all 16{,}384 \\
9 & minifloat & 8 bits & 558 & 67.06 & 0.1202 & $5.49$ & not swept \\
...
16 & posit8 & 8 bits & 540 & 44.33 & 0.0821 & $4.02$ & not swept \\
17 & takum16 & 16 bits & 789 & 58.96 & 0.0747 & $5.75$ & not swept \\
18 & IBM hex32 & 32 bits & 687 & 46.78 & 0.0681 & $3.78$ & not swept \\
19 & LNS16 & 16 bits & 658 & 43.04 & 0.0654 & $0.45$ & not swept \\
20 & posit16 & 16 bits & 712 & 40.38 & 0.0567 & $1.02$ & 4 of 65{,}536 wrong \\
21 & posit32 & 32 bits & 953 & 28.16 & 0.0295 & $2.29$ & not swept \\
\bottomrule
\end{tabular}
\end{table}
...
\multicolumn{4}{l}{\quad\emph{post-route estimate from the same harness:} $454$ LUT, $78.83$\,MHz, $0.1736$ MHz/LUT, spread $11.00$} \\
...
seeds ($454$ LUT at $78.83$\,MHz), against GFTernary's $0.1797$ ($463$ LUT at
$83.19$\,MHz) --- a $3.5\%$ difference inside seed spreads of $11.0$ and $6.6$
percentage points, so the two are not separated by this flow. The rounded
...
\caption{Canon plate. Of 885 distinctive literals, 882 trace to the tree that produced them; the three that do not are retained and marked, and five document-to-code pairs still have no gate.}
...
Re-run on this revision over $6{,}108$ data files, the check finds $885$
distinctive literals in the paper of which \textbf{two} trace to no file; a third,
found by the audit described immediately below rather than by the check itself,
brings the listed total to \textbf{three}. They are
listed rather than deleted, because a figure that cannot be traced is not thereby
shown to be wrong:

The check has a weakness we found by auditing its passes rather than its failures,
and it belongs in the table's neighbourhood. It matches a literal as a
\emph{substring} of a data file's text, so a short literal is reported as sourced
whenever it occurs inside a longer unrelated number. Two figures reading $3.15$
passed on exactly that basis and neither had a generating file: a takum32 decode
table in Mbit, now withdrawn from Corollary~\ref{cor:zeropath}, and the octave span
of Theorem~\ref{thm:barrelrange}, which is retained and flagged in
Table~\ref{tab:untraced} below. A literal of three or four characters should be
matched on word boundaries; until the gate does so, its pass is weaker evidence for
short literals than for long ones.

\begin{table}[t]\centering
\caption{Figures with no generating file in the repository, on this revision. Each
is retained, flagged, and should be re-measured before it is relied upon. The
previous revision reported seven such figures against a smaller file set; five of
those now trace, and the count is restated rather than carried forward. The third
row was not reported by the gate at all --- it was found by auditing the gate's
...
\midrule
$318.47$\,MHz & plastic 16-bit row of the multiply-free ratio table \\
$63.30$\,MHz & \texttt{binary16} row of the conformance-and-cost table \\
$3.15$ octaves & scale range over 210 layers, Theorem~\ref{thm:barrelrange} \\
\bottomrule
\end{tabular}
\end{table}
...
value survives. The first two are achieved frequencies, the quantity with the
widest seed spread in this paper, so neither should be quoted without a
re-measurement; that is the pattern this check exists to surface. The third was
found by the substring audit above rather than by the gate, and the two-bit shift
field that Theorem~\ref{thm:barrelrange} concludes from it survives any span below
four octaves, so the conclusion is more robust than the figure supporting it.
...
repair~\cite{tran2026rtlrepair}. Against
...
means a UART transcript from the AX7203 board carrying XC7A200T silicon
...
iterations against that format's $27$. This is the first number in this paper
...
On this one system, TNF$(4,8)$ has a slightly smaller externally recomputed residual
than \texttt{takum16}, but a slightly larger solution error; \texttt{binary16}
has the smallest solution error among the three low-precision candidates. This
is a task-level observation for one matrix and one rung, not a format ranking.
The solver iteration count is not a throughput result, and the synthetic
system supplies no evidence about energy, area or FPGA timing. The generator and
the machine-written result record are stored with the other measurements. The
plain reading is that this second task shows no advantage for the format: it is
reported because it was run, not because it agrees.
