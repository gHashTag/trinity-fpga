...
five, $r^6{=}r{+}1$ at six ---  On the block axis the gain is scale \emph{resolution}: at $4.25$ bits per element a finer ladder beats MXFP4 by $17.1\%$ RMSE on two models, zero losing blocks in $14.9$M, and $9.0\%$ perplexity against a best-encoded baseline --- and a binary ladder realises it better than a ternary one.
 ...
 Eleven retractions are marked in place below.
\end{abstract}
...
Eleven of our own claims were falsified by the measurements meant to support them,
...
than twenty-eight. Eleven claims of ours are retracted in place below, and the
...
The comparison is worth making twice over. Proposition~\ref{prop:reserve} offers
two horns --- guard the surplus or extend the specification to cover it --- and
the decimal standard takes a third we did not consider: the $24$ surplus
patterns are \emph{assigned} as non-canonical alternate encodings of values the
format already names, decoders must accept them, encoders must not emit them.
That costs no recognition logic and extends no specification. \textbf{We have
not measured it, and it is the cheapest open experiment in this paper.}
...
\emph{This is the sharpest form of a finding that recurs through this paper.}
The largest single improvement available on the block axis belongs to the
baseline and had been left on the table by its own reference encoder --- not to
a new format, not to a new algebra, and not to us.
...
\textbf{guard} & $591$ & $68.14$ & $0.1153$ & $-2.66\%$, $p=3.0\times10^{-5}$ \\
assign & $576$ & $59.24$ & $0.1029$ & $-13.17\%$, $p=2.1\times10^{-89}$ \\
\bottomrule
\end{tabular}
\end{center}
...
forbids: the conditional subtract packs into the LUTs it already shares an input
with, while a mux to a $32$-bit constant does not. And the axis that decided the
comparison appears in neither argument --- assign loses on \emph{frequency},
$59.24$ against $68.14$, because the subtract lands in the critical path between
the offset field and the exponent adder where a constant needs no arithmetic
behind it.

So the recommendation is \textbf{guard}: $2.66\%$ buys a format in which every
code is either defined or flagged. The IEEE~754 answer is the strongest of the
three on correctness --- under assignment no code of the word lies outside the
format at all --- and the dearest on this fabric. That is not a criticism of the
