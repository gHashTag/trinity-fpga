...
UNITS = [
    ("fp4_e2m1", 4, lambda: (F8, F8.FORMATS["fp4_e2m1"])),
    ("gf4", 4, lambda: (G, G.FORMATS["gf4"])),
    ("tnf4", 6, lambda: (T, T.TNFFormat(2, 1))),
    ("gf8", 8, lambda: (G, G.FORMATS["gf8"])),
...
if __name__ == "__main__":
    ...
    for name, bits, get in UNITS:
        ...
        (OUT / "oracle_rtl.json").write_text(json.dumps(res, indent=1))
    print("WROTE " + str(OUT / "oracle_rtl.json"), flush=True)
