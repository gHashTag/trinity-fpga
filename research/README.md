...
| `gen_conformance_pack.py` | **derives a candidate pack** for a format that has an oracle but no published pack — `OK <fmt> mode=… n=… decode_errors=0 sha256=…`. It does **not** reproduce the published corpus: 68 of the 83 packs were made by `gen_all_formats.py`, which lives in `t27` at `conformance/vectors/`, and this tool made none of them. The one-line description here previously said "regenerates a pack", which the tool's own docstring contradicts. | 0 |
