...
| the HW conformance golden is an **exact logarithmic takum** — 60,485/60,485 | independent mpmath implementation, verified against the format author's own library | **⚠ citation broken.** `takum_variant_split.t27` does not exist in any repository (0 hits in code search), and the figure 60,485/60,485 appears in no spec. The related claim is corroborated — `conformance/takum16_decode_conformance_ax7203.py` computes `exp(ell/2)` at 120-bit mpmath precision, and pass 138 witnessed the published packs as correctly rounded — but **this row's evidence is not locatable and the count should not be quoted until it is.** |
