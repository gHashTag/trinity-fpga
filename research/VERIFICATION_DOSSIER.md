...
| the takum oracle was cross-validated against **libtakum**, the format author's own C99 reference | exhaustive at both enumerable widths, via `research/libtakum_bridge.c` and `research/crossval_libtakum.py` | `takum_libtakum_crossval.t27` — **takum8 256/256 and takum16 65,536/65,536 AGREE; 65,792 codes compared, 0 divergences.** This row replaces a retracted one. It used to read *positive half 32,768/32,768 EXACT, negative half divergent*, note that *re-running needs libtakum's `takum.h`, which is not in this tree*, and instruct the reader to *quote the 32,768/32,768*. All three are now wrong. The divergence was measured against `takum{N}_to_float64` while this corpus implements `takum_log{N}` — libtakum exposes both and names neither in words — and against the right family takum16 is correct on **every** finite code in **both** halves, median relative error 4.47e−16, max 7.38e−15, none worse than 1e−9. libtakum was built from source in pass 145 and the comparison run in pass 158, so nothing is missing from the tree. **Do not quote the 32,768/32,768.** |
