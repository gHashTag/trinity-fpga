...
multiplication follows $2W^2$ regardless of encoding: a GF16+ (Quire) multiply
(505 LUT, zero-DSP) equals a native takum16 LNS multiply (505 LUT); plain GF16
multiply is 587 LUT.
...
\midrule
GF16 \texttt{[1|6|9]}
  & \textbf{491} \textit{[measured, parametric]} / 176 \textit{[old top]}
...
\paragraph{Reproducibility caveat (core identity).} The LUT counts above are
\emph{core-dependent}: the dedicated per-width cores (\texttt{gf16\_add},
\texttt{gf16\_mul}, \texttt{gf16\_multiplier}, measured $\approx$485/432/483 LUT)
are $\sim$3$\times$ smaller than the parametric \texttt{gf\_adder\_param}/
\texttt{gf\_mul\_param} cores (measured $\approx$1689/1953 LUT at GF16, i.e.\ a
steeper law ADD$\approx$3.6W$^2$, MUL$\approx$5.8W$^2$), because the parametric
cores carry GF4--GF64 generality plus full denormal/NaN/Inf handling. Dedicated
cores exist only for $W{\le}32$; the $W{\ge}48$ rows therefore follow the
dedicated-core scaling law by extrapolation, not a separate dedicated measurement.
The exact core per row and the pinned yosys recipe are committed in
\texttt{scripts/lut\_measure.sh} and documented in
\texttt{research/PAPER\_INTEGRITY\_ISSUES.md} \S H--I; reproduce with
\texttt{bash scripts/lut\_measure.sh}.
...
identical to GF16+ (Quire) MUL (505 LUT in the zero-DSP regime); plain GF16 MUL
is 587 LUT (Table~\ref{tab:lutlaw}). This confirms that the
...
(IEEE-style linear vs.\ logarithmic tapered).  Both GF16+ (Quire) multiply
(mantissa product + normalize) and takum16 native multiply (logarithmic
addition + tapered re-encode) yield $\approx$505 LUT at $W=16$ (plain GF16
multiply is 587 LUT).  The
...
GF16 and above pass all four tests (GF14 is the matched-budget boundary case, 0/4 here vs 7/7 on the hold-out suite, \S\ref{sec:holdout}); FP16 fails dynamic range; BF16 is stochastically fragile on matmul.}
...
\item \textbf{Encoding invariance (Encoding Equivalence).} By the Encoding
   Equivalence (\S\ref{sec:named}), GF16+ (Quire) MUL and a native takum16 MUL
   both cost 505 LUT in the zero-DSP regime: $\text{GF16+} \equiv \text{takum16}$.
   (Plain GF16 MUL is 587 LUT.)
...
\paragraph{Beyond floating-point: INT7 at matched budget (preliminary).}
...
on synthetic Markov data ($d{\leq}128$, 300 steps) suggests that integer
quantization may achieve lower BPB than floating-point in the underfitting
regime. However, on a unified FineWeb benchmark (Table~\ref{tab:unified})
where all formats are evaluated on the same trained model, floating-point
formats (BF16, GF14, GF16, FP16) show negligible degradation ($\Delta < 0.0001$
BPB), while INT6 adds +0.018 BPB and SQ-INT6 adds +0.008 BPB. The matched-budget
advantage of integer formats may therefore be an artefact of the toy-data
regime rather than a fundamental property. This finding is
\emph{preliminary and scale-dependent}; GPU-scale validation is needed.

\paragraph{Format trade-off: training efficiency vs.\ robustness (preliminary).}
A comparison across six format families at matched artifact budget on synthetic
data suggests a trade-off between training efficiency and numerical robustness.
On the unified FineWeb benchmark (Table~\ref{tab:unified}), all floating-point
formats (BF16, GF14, GF16, FP16) show negligible degradation ($\Delta < 0.0001$
BPB vs.\ FP32), while integer formats show measurable error (INT6: +0.018,
SQ-INT6: +0.008). SmoothQuant reduces INT6 error by 57\%.
The $\varphi$-rule floating-point family remains uniquely valuable for its 7/7
robustness and dynamic range properties. These results are preliminary and
scale-dependent; GPU-scale validation is needed.
...
model size ($d{=}128$) suggests (preliminarily, at CPU scale) that QAT SQ-INT6 achieves 7.7\% lower BPB than
...
BPB metric) uses INT6 weights, consistent with our finding that integer quantization is competitive: the winner (1.057~BPB)
...
\centering
\small
\caption{Unified FineWeb benchmark: all formats evaluated on the same trained
model (4L, $d{=}128$, vocab 1024, seq 128, 300 steps, seed 42). PTQ applied
post-training. $\Delta$ = BPB increase vs FP32 master. Floating-point formats
show negligible degradation; integer formats show measurable but small error.}
\label{tab:unified}
\begin{tabular}{lcrr}
\toprule
Format & bpe & FineWeb BPB & $\Delta$ vs FP32 \\
\midrule
FP32 (master) & 32 & 1.8740 & --- \\
FP16 (E5M10)  & 16 & 1.8740 & $-0.0000$ \\
BF16 (E8M7)   & 16 & 1.8740 & $+0.0000$ \\
GF16 (E6M9)   & 16 & 1.8740 & $+0.0000$ \\
GF14 (E5M8)   & 14 & 1.8741 & $+0.0001$ \\
INT8          &  8 & 1.8749 & $+0.0009$ \\
INT7          &  7 & 1.8781 & $+0.0041$ \\
\textbf{SQ-INT6} & \textbf{6} & \textbf{1.8818} & $\mathbf{+0.0078}$ \\
INT6          &  6 & 1.8920 & $+0.0180$ \\
\bottomrule
\end{tabular}
\end{table}
TEXEO