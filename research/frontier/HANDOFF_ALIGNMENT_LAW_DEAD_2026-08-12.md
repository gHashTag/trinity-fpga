# ⚠️ HANDOFF: this narrows the spine of tnf_paper_v2.tex — read before the next revision

**Written by a parallel session working in `trinity-fpga/`, not in this worktree. I did not edit
`tnf_paper_v2.tex` — it is yours and you are actively committing to it. This is the finding, and
the exact places it touches.**

## What it changes

`tnf_paper_v2.tex` line 64 (the title) and lines 211, 1073, 1636, 2504 carry the alignment gain as
**2.78 / 0.51 on two models**. That scoping is honest and stays true. What is now measured, and is
NOT in the paper, is what happens on three more:

* **u\* does not transfer.** Across five families it spans **0.25–0.40** — three grid steps.
  SmolLM2 0.35, Qwen 0.30, **Pythia 0.40, OPT 0.25, GPT-2 0.25.**
* **The pre-registered leave-one-out fails.** `u` from the other four, applied to Pythia, is
  **1.22–1.42 ppl WORSE than shipping the OCP spec**, under all three tie rules.
* 🔑 **No single `u` beats OCP on all four measured curves, and the best minimax value over the
  common grid IS the OCP spec (u = 0.415).**
* On Pythia the tuned gain (+0.2457) is **inside its own tie-rule floor (0.5358)** — OCP with
  ties-away beats tuned-u\* with ties-even by 0.2886 there. The rounding convention is the bigger
  lever on that model.
* The natural repair fails too: clamp fraction at each model's own u\* spreads **17.6 points**
  against u's own 15. Wider, not tighter — "clamp a fixed fraction" is not the hidden law.

## The sentence the paper should now be able to make

Alignment is a **free per-model tuning knob worth 0.25–2.83 ppl at identical bit cost**, which
every specification fixes and nobody varies — **but the specification's own choice is the best
single value across the five families measured.** That partially *vindicates* OCP rather than
faulting it, and it is a more interesting result than a tuned constant.

The three-knob taxonomy (base / alignment / width) is untouched. What changes is the **value**
attached to the alignment knob: not "2.83 for everyone" but "0.25–2.83, per model, unpredictable,
and the spec is the best fixed compromise".

## Why this matters beyond one number

This is the **second** constant in this campaign fitted on two checkpoints and killed by the third,
at the budget it was invented for — λ was the first. The mechanism is the problem's: **4-bit block
quantisation has flat optima**, so a grid search finds a confident-looking minimum whose location
is set by noise. Recorded as §3b of the `measurement-discipline` skill.
