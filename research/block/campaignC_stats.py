...
Pooled out-of-sample rows concatenate d over ONLY the models an arm never saw.
A margin whose 95 % CI contains zero is reported as a TIE.
...
def main():
    ...
    print("=" * 96)
    print("POOLED OUT OF SAMPLE  (only models the arm never saw; "
          f"Bonferroni over {NCMP} arms)")
    print("=" * 96)
    print(f"{'arm':<12}{'OOS models':<28}{'%':>9}{'95% CI':>22}{'t':>9}"
          f"{'p':>11}{'win':>8}  verdict")
    pooled = {}
    for a in ARMS:
        if a == REF:
        ...
        d = np.concatenate([dvec(D, m, a) for m in oos])
        r = paired(d)
        pooled[a] = r
    ...
    print("THE QUESTION")
    print("=" * 96)
    beat = [a for a in ARMS if a not in (REF, "NF4")
            and verdict(pooled[a], NCMP) == "BEATS"]
    beat_any_model = {a: [m for m in MODELS
                          if verdict(per[(a, m)]) == "BEATS"]
                      for a in ARMS if a != REF}
    print("Arms fitted or designed in this repository that beat NF4-sym pooled "
          "out of sample:")
