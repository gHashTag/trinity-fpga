...
export default function TechTree() {
  ...
  // Memoize grid dimensions to avoid recalculating on every render
  const grid = useMemo(() => {
    const maxX = Math.max(...techBranches.flatMap(b => b.nodes.map(n => n.x)));
    const maxY = Math.max(...techBranches.flatMap(b => b.nodes.map(n => n.y)));
    const width = (maxX + 1) * (NODE_WIDTH + GAP_X) + PADDING * 2 + SIDEBAR_WIDTH;
    const height = (maxY + 1) * (NODE_HEIGHT + GAP_Y) + PADDING * 2;
    return { width, height };
  }, []);

  // Memoize localized tech tree strings
  const techTreeStrings = useMemo(() => t.techTree || {
  ...
  useEffect(() => {
    ...
    updateDimensions();
    // Use an AbortController or similar if needed, but resize is fine here
  ...
  return (
    <Section id="tech-tree">
      <div className="tight fade" style={{ textAlign: 'center', marginBottom: '2rem' }}>
        <h2 dangerouslySetInnerHTML={{ __html: techTreeStrings.title }} />
        <p className="sub">{techTreeStrings.sub}</p>
      </div>
      ...
      >
        ...
        <div style={{
          ...
          <div style={{
            ...
            {techTreeStrings.hint}
          </div>
        </div>

        {/* Scrollable tree area */}
        <div style={{
          minWidth: `${grid.width}px`,
          minHeight: `${grid.height}px`,
          position: 'relative',
          padding: `${PADDING}px ${PADDING}px ${PADDING}px ${PADDING + SIDEBAR_WIDTH}px`
        }}>
          {/* Sidebar Rail */}
          <div style={{
            position: 'absolute',
          ...
          {techBranches.map((branch, idx) => (
            <motion.div
              ...
              style={{
                position: 'absolute',
                left: `${PADDING / 2}px`,
                top: `${PADDING + idx * (NODE_HEIGHT + GAP_Y)}px`,
                width: `${SIDEBAR_WIDTH}px`,
            ...
            >
              <div style={{
                ...
                <span style={{ fontSize: '0.8rem', filter: 'none' }}>{branch.icon}</span>
          ...
          {techBranches.map(branch =>
            branch.nodes.map((node, nodeIdx) => (
              <motion.div
                ...
                style={{
                  position: 'absolute',
                  left: `${PADDING + SIDEBAR_WIDTH + node.x * (NODE_WIDTH + GAP_X)}px`,
      ...
      <div className="fade" style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <span style={{ fontSize: '0.8rem', color: 'rgba(255, 255, 255, 0.6)' }}>{t.techTree.legend.done}</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <motion.div
            animate={{ opacity: [0.5, 1, 0.5] }}
          ...
          />
          <span style={{ fontSize: '0.8rem', color: 'rgba(255, 255, 255, 0.6)' }}>{t.techTree.legend.progress}</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <div style={{
            width: '12px',
          ...
          <span style={{ fontSize: '0.8rem', color: 'rgba(255, 255, 255, 0.6)' }}>{t.techTree.legend.locked}</span>
        </div>
      </div>
