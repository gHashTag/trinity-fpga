...
| 1c | **Largest unmeasured threat: block scaling.** At the OCP MX block size 32, TNF4 zeroes 0.01 % against `fp6 e2m3`'s 2.51 %, but its relative RMS error is **3.46× worse** and is the worst of the three six-bit grids at every block size. The advantage exists only under **per-tensor** scaling of heavy-tailed data | `blockscale_w949.json` | **no training run here has ever used a block scale**, and the field deploys nothing else at 4–6 bits |
