...
| 1d | **Mechanism recovered (T799): saturation from a collapsing learned scale.** Over all 120 traces, saturation and failure agree **90.8 %**; `fp6 e2m3`'s 28 failures saturate **28/28**. TNF4's scale collapses **32.4×** — twenty times harder than the competitor's *successful* runs — and never fails: not stability, **headroom** | `mechanism_w950.json` | inferred from logged scales, not observed tensor maxima; 7 saturate-and-survive, 4 fail-without-saturating |
