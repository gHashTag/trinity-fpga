...
The full HDC application suite comprises 23 modules specified as `.vibee` specifications in the `specs/tri/` directory, covering classification, anomaly detection, knowledge graphs, reinforcement learning, and more. See the [HDC Applications](/hdc/applications) page for the complete catalog.
...
IGLA achieves competitive accuracy while offering massive compression and symbolic reasoning capabilities that distance-based embeddings lack. See the [IGLA GloVe Comparison](/hdc/igla-glove-comparison) for detailed benchmarks against Word2Vec, BERT, and fastText.
