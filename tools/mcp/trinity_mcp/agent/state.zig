// state.zig — File-based key-value state for Ralph agent
// Keys stored as individual files in .ralph/state/
...
test "state read/write roundtrip" {
    const allocator = std.testing.allocator;
    var state = try State.init(allocator, "/tmp/ralph-test");
    defer state.deinit();
