...
pub fn indexedAdvantage() !void {
// Indexed > flat by >20% — sub-memory isolation preserves capacity by keeping each relation's bundle small
...
// Test indexedSingleHop: verify behavior is callable
const func = @TypeOf(indexedSingleHop);
...
// Test flatComparison: verify behavior is callable
const func = @TypeOf(flatComparison);
    try std.testing.expect(func != void);
}
...
// When: Compare accuracy
// Then: Indexed > flat by >20% — sub-memory isolation preserves capacity by keeping each relation's bundle small
// Test indexedAdvantage: verify behavior is callable
const func = @TypeOf(indexedAdvantage);
    try std.testing.expect(func != void);
}
