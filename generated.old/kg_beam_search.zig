...
// All 100% — no noise means no benefit from beam
...
// Beam-3 +10-20% over greedy, beam-5 even better — multiple candidates recover from single-step errors
    const result = @as([]const u8, "implemented");
    _ = result;
}
...
// Greedy 10%, beam-3 30%, beam-5 60% — beam search is critical for noise robustness
    const result = @as([]const u8, "implemented");
    _ = result;
}
...
// When: Traverse with greedy/beam-3/beam-5
// Then: All 100% — no noise means no benefit from beam
// Test cleanTraversal: verify behavior is callable
const func = @TypeOf(cleanTraversal);
...
// When: Compare greedy vs beam
// Then: Beam-3 +10-20% over greedy, beam-5 even better — multiple candidates recover from single-step errors
// Test moderateNoise: verify behavior is callable
const func = @TypeOf(moderateNoise);
    try std.testing.expect(func != void);
}
...
// When: Compare
// Then: Greedy 10%, beam-3 30%, beam-5 60% — beam search is critical for noise robustness
// Test heavyNoise: verify behavior is callable
const func = @TypeOf(heavyNoise);
    try std.testing.expect(func != void);
}
