...
// Cross-domain queries return no matches (similarity < threshold) — domain isolation confirmed for all 5 domains
...
// Test massiveKGConstruction: verify behavior is callable
const func = @TypeOf(massiveKGConstruction);
...
// Test indexedQueryAll: verify behavior is callable
const func = @TypeOf(indexedQueryAll);
    try std.testing.expect(func != void);
}
