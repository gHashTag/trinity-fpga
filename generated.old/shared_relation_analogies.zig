...
// 0 noise=100%, 3 noise=99.2%, 5 noise=40.8%
...
// Test sharedRelationExtraction: verify behavior is callable
const func = @TypeOf(sharedRelationExtraction);
...
// When: Apply noisy relation to query A, find nearest B
// Then: 0 noise=100%, 3 noise=99.2%, 5 noise=40.8%
// Test noisyRelationDegradation: verify behavior is callable
const func = @TypeOf(noisyRelationDegradation);
    try std.testing.expect(func != void);
}
...
// Test treeBundledRelation: verify behavior is callable
const func = @TypeOf(treeBundledRelation);
    try std.testing.expect(func != void);
}
