...
async function loadState() {
  try {
    const result = await browser.storage.local.get(['neodetectState', 'savedProfiles']);
...
async function saveProfiles() {
  await browser.storage.local.set({ savedProfiles });
}
...
async function saveState() {
  try {
    await browser.storage.local.set({ neodetectState: state });
    // Notify background
    await browser.runtime.sendMessage({ action: 'setState', state });
  } catch (e) {
...
if (aiMode) {
  aiMode.addEventListener('change', async (e) => {
    ...
    if (state.aiMode) {
      // Initialize AI model
      await browser.runtime.sendMessage({ action: 'initAI' });
    }
    ...
    if (result.neodetectState?.updateAvailable) {
      ...
      if (header) {
        ...
        updateBanner.onclick = () => {
          browser.tabs.create({ url: result.neodetectState.updateUrl || 'https://github.com/gHashTag/trinity/releases' });
        };
