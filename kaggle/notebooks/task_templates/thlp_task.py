# Kaggle Benchmarks — Hippocampal Learning Probe
...
csv_path = None
for f in csv_files:
    if f.endswith('thlp_learning.csv'):
        csv_path = f
        break
...
@kbench.task(name="trinity_thlp_learning Single", store_task=False)
...
Answer:"""
    ...
    if not matched and len(thlp_single.debug_log) < 10:
        thlp_single.debug_log.append({
...
@kbench.task(
    ...
    description="Evaluates hippocampal-style few-shot learning, belief updating, error-driven learning. Based on Trinity's cognitive architecture."
)
def thlp_benchmark(llm) -> float:
    ...
    kbench.assertions.assert_true(True, expectation=f"Hippocampal Learning Probe accuracy: {accuracy:.2%} ({len(valid)}/{len(eval_df)})")
    return accuracy
...
# === CELL 9: Debug Output ===
if thlp_single.debug_log:
    print(f"\n🐛 First {len(thlp_single.debug_log)} failures:")
    for i, entry in enumerate(thlp_single.debug_log, 1):
        print(f"\n{i}. Q: {entry['question']}")
