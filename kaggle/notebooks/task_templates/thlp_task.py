...
Answer:"""
    ...
    global thlp_debug_log
    if not matched and len(thlp_debug_log) < 10:
        thlp_debug_log.append({
...
if thlp_debug_log:
    print(f"\n🐛 First {len(thlp_debug_log)} failures:")
    for i, entry in enumerate(thlp_debug_log, 1):
        print(f"\n{i}. Q: {entry['question']}")
