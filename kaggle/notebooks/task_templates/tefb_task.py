# Kaggle Benchmarks — Executive Function Battery
...
csv_path = None
for f in csv_files:
    if f.endswith('tefb_executive.csv'):
        csv_path = f
        break
...
def tefb_benchmark(llm) -> float:
    ...
    kbench.assertions.assert_true(True, expectation=f"Executive Function Battery accuracy: {accuracy:.2%} ({len(valid)}/{len(eval_df)})")
    return accuracy
