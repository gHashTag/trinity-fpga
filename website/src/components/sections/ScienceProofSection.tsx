  ...
  return (
    <Section id="science">
      ...
      <div className="grid fade" style={{ marginTop: '3rem', marginBottom: '4rem' }}>
        {s.stats?.map((stat: { value: string; label: string }, i: number) => (
          <div key={i} className="premium-card" style={{ textAlign: 'center' }}>
            <div style={{ fontSize: 'clamp(1.75rem, 6vw, 2.5rem)', fontWeight: 500, color: 'var(--accent)', marginBottom: '0.5rem' }}>{stat.value}</div>
            <div style={{ color: 'var(--muted)', fontSize: '0.9rem' }}>{stat.label}</div>
          </div>
        ))}
      </div>

      <div className="fade" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(300px, 100%), 1fr))', gap: '2rem', textAlign: 'left', marginBottom: '4rem' }}>
        <div className="premium-card compact" style={{ background: 'rgba(255,255,255,0.02)' }}>
           <h4 style={{ marginBottom: '1rem', color: 'var(--text)', fontSize: '0.9rem', textTransform: 'uppercase', letterSpacing: '0.1em' }}>{s.evidenceTitle}</h4>
           <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(200px, 100%), 1fr))', gap: '0' }}>
              {s.sections?.map((sec: { name: string; passed: number; total: number }, i: number) => (
        ...
        <div className="premium-card compact" style={{ background: 'rgba(255,255,255,0.02)' }}>
           <h4 style={{ marginBottom: '1rem', color: 'var(--text)', fontSize: '0.9rem', textTransform: 'uppercase', letterSpacing: '0.1em' }}>{s.formulasTitle}</h4>
            {s.formulas?.map((f: { formula: string; desc: string; error: string }, i: number) => (
      ...
      <div className="fade" style={{ marginTop: '3rem' }}>
        <div style={{ display: 'flex', justifyContent: 'center', gap: '2rem', flexWrap: 'wrap' }}>
          {s.sources?.map((src: { title: string; desc: string }, i: number) => (
             <div key={i} style={{ textAlign: 'center', width: 'min(250px, 80vw)' }}>
