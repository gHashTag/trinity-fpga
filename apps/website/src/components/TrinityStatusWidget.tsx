...
/**
 * Trinity Status Dashboard Widget v8.24
 *
 * Real-time display of Trinity MODE production swarm status:
 ...
 */
export default function TrinityStatusWidget({ width = 360, height = 220 }: Props) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await fetchTrinityStatus();
        setStatus(result);
      } catch (error) {
  ...
  const getStateColor = useCallback((state: TrinityState): string => {
    switch (state) {
  ...
  const getStateIcon = useCallback((state: TrinityState): string => {
    switch (state) {
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: '14px', fontWeight: 'bold' }}>
            {status.state === 'IMMORTAL' && 'Trinity IS IMMORTAL'}
