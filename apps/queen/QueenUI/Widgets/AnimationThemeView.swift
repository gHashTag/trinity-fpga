    ...
    private func themeCard(_ theme: AnimationTheme) -> some View {
        Button {
            ...
            VStack(spacing: 8) {
                ...
                Text(theme.name)
                    .font(.caption)
                    .foregroundStyle(selectedTheme == theme ? TrinityTheme.accent : TrinityTheme.textPrimary)
            }
            .padding()
            .frame(maxWidth: .infinity)
            .background(
                RoundedRectangle(cornerRadius: TrinityTheme.cornerSmall)
                    .fill(selectedTheme == theme ? TrinityTheme.accent.opacity(0.15) : TrinityTheme.bgCard)
            )
            .overlay(
                RoundedRectangle(cornerRadius: TrinityTheme.cornerSmall)
                    .stroke(selectedTheme == theme ? TrinityTheme.accent : TrinityTheme.bgCardBorder, lineWidth: 1)
            )
        }
        .buttonStyle(.plain)
    }
...
enum AnimationTheme: String, CaseIterable {
    ...
    var previewColor: Color {
        switch self {
        case .default: return TrinityTheme.accent
...
struct AnimatedToggle: View {
    ...
    var body: some View {
        HStack(spacing: 8) {
            Text(isOn ? "On" : "Off")
                .font(.caption)
                .foregroundStyle(isOn ? TrinityTheme.textPrimary : TrinityTheme.textMuted)
                .frame(width: 30)

            ZStack(alignment: isOn ? .trailing : .leading) {
                RoundedRectangle(cornerRadius: 12)
                    .fill(isOn ? TrinityTheme.accent : TrinityTheme.bgCardBorder)
...
struct LoadingSpinner: View {
    ...
    init(size: CGFloat = 20, color: Color = TrinityTheme.accent) {
        self.size = size
        self.color = color
    }
    ...
    var body: some View {
        Circle()
            ...
            .stroke(
                AngularGradient(
                    gradient: .init(colors: [color.opacity(0.3), color, color.opacity(0.3)]),
                    center: .center
                ),
                lineWidth: 3
            )
    ...
    static var previews: some View {
        VStack(spacing: 30) {
            ...
            HStack(spacing: 20) {
                ...
                ProgressRing(progress: 0.75, size: 40, color: TrinityTheme.accent)
                MorphingShape()
            }
        }
        .padding()
