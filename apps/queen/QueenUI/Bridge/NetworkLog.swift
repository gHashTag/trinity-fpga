...
class NetworkLog: ObservableObject {
    ...
    /// Tracks consecutive failures per provider. After 5 consecutive failures,
    /// the provider is marked unavailable for 30 seconds.
    private var consecutiveFailures: [String: Int] = [:]
    private var circuitOpenUntil: [String: Date] = [:]
    private let circuitBreakerThreshold = 5
    private let circuitBreakerCooldown: TimeInterval = 30
