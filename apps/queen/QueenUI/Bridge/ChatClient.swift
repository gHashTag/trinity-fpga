...
/// Request type for configurable timeouts
enum RequestType {
    case chat       // Default chat: 30s
    case reason     // Reasoning mode: 60s
    case image      // Image generation: 90s
    case search     // Web search: 45s
    case custom(Int) // Custom timeout in seconds

    var timeoutSeconds: TimeInterval {
        switch self {
        case .chat: return 30
        case .reason: return 60
        case .image: return 90
        case .search: return 45
...
class ChatClient: ObservableObject {
    ...
    /// Start timeout watchdog with configurable timeout per request type.
    /// Default: 30s connect, 15s inactivity after first token.
    /// Reason mode: 60s connect. Image generation: 90s. Search: 45s.
    func startStreamTimeout(requestType: RequestType = .chat) {
        streamTimeout = false
        lastTokenTime = .distantPast
        let connectTimeout: TimeInterval = requestType.timeoutSeconds
