...
pub fn main() !void {
    ...
    if (std.mem.eql(u8, command, "gen")) {
        ...
        // output to stdout, let shell redirect to file
        try generateCode(allocator, input_path);
...
fn printUsage() void {
    std.debug.print(
        ...
        \\  vibee_gen gen specs/tri/agent.vibee var/trinity/output/agent.zig
        \\
...
fn generateCode(allocator: std.mem.Allocator, input_path: []const u8) !void {
    ...
    if (spec.languages.items.len > 1) {
        for (spec.languages.items) |lang| {
            try generateSingleLang(allocator, &spec, lang);
        }
        return;
    }
    ...
    // Write to stdout (let shell redirect to file)
    const stdout = std.io.getStdOut().writer();
    try stdout.writeAll(generated_code);
}
