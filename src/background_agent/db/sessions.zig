...
pub fn createSession(allocator: Allocator, client: *PostgresClient, name: []const u8, service_id: []const u8) !Session {
    ...
    const sql = try std.fmt.allocPrint(allocator,
        \\INSERT INTO sessions (id, name, status, railway_service_id, created_at, updated_at)
        \\VALUES ('{s}', '{s}', 'active', '{s}', {d}, {d})
    , .{ session_id, name, service_id, now, now });

    defer allocator.free(sql);

    const result = try PostgresClient.query(client, sql, &.{}) catch |err| {
...
pub fn getSession(allocator: Allocator, client: *PostgresClient, session_id: []const u8) !Session {
    ...
    const sql = try std.fmt.allocPrint(allocator,
        \\SELECT id, name, status, railway_service_id,
    ...
    const created_at = if (row.values.items.len > 3)
        try std.fmt.parseInt(i64, row.values.items[3], 10) catch now
    else std.time.timestamp();

    const updated_at = if (row.values.items.len > 4)
        try std.fmt.parseInt(i64, row.values.items[4], 10) catch now
    else std.time.timestamp();
    ...
    return Session{
        ...
        .railway_service_id = try allocator.dupe(u8, row.values.items[0] orelse ""),
...
pub fn listSessions(allocator: Allocator, client: *PostgresClient) !std.ArrayList(Session) {
    const sql = "SELECT id, name, status, railway_service_id, EXTRACT(EPOCH FROM created_at) as created_at, EXTRACT(EPOCH FROM updated_at) as updated_at FROM sessions ORDER BY created_at DESC";
    ...
    for (result.rows.items) |*row| {
        const created_at = if (row.values.items.len > 2)
            try std.fmt.parseInt(i64, row.values.items[2], 10) catch std.time.timestamp()
        else std.time.timestamp();

        const updated_at = if (row.values.items.len > 3)
            try std.fmt.parseInt(i64, row.values.items[3], 10) catch std.time.timestamp()
        else std.time.timestamp();
        ...
        try sessions.append(Session{
            ...
            .name = try allocator.dupe(u8, row.values.items[1] orelse ""),
            .status = try allocator.dupe(u8, row.values.items[2] orelse "unknown"),
            .railway_service_id = try allocator.dupe(u8, row.values.items[2] orelse ""),
            .created_at = created_at,
            .updated_at = updated_at,
        });
    }
...
pub fn updateSession(allocator: Allocator, client: *PostgresClient, session_id: []const u8, data: struct {
    ...
    _ = try PostgresClient.query(client, sql, &.{}) catch |err| {
        return error.DatabaseError;
    };
}
...
test "sessions: create and get" {
    ...
    // Mock client
    var mock_client = PostgresClient{ .allocator = allocator };

    // This would actually connect in real use
