...
pub const HybridCodeGen = struct {
    ...
    fn analyzePrompt(self: *Self, prompt: []const u8) SymbolicAnalysis {
        ...
        // Detect task type
        if (trinity_swe.TrinitySWEAgent.containsAny(prompt, &.{ "hello world", "helloworld", "andin and" })) {
            analysis.task_type = .HelloWorld;
            analysis.addConcept("print");
            analysis.addConcept("main");
        } else if (trinity_swe.TrinitySWEAgent.containsAny(prompt, &.{ "fibonacci", "andonand" })) {
        ...
        } else if (trinity_swe.TrinitySWEAgent.containsAny(prompt, &.{ "function", "fn", "func", "toand" })) {
