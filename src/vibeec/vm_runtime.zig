...
pub const VM = struct {
    ...
    pub fn callFunction(self: *Self, func_addr: u16, args: []const Value) VMError!Value {
        ...
        self.fp += 1;
        self.call_stack[self.fp] = CallFrame.initWithArgs(saved_ip, saved_sp, saved_sp, @intCast(args.len));
    ...
    fn executeOpcode(self: *Self, opcode: Opcode) VMError!void {
        switch (opcode) {
            ...
            .CALL_INDIRECT => {
                ...
                self.fp += 1;
                self.call_stack[self.fp] = CallFrame.initWithArgs(self.ip, func_idx, // base_pointer points to func slot
                    arg_start, arity);
                self.ip = addr;
            },
...
fn nativeInt(vm: *VM, args: []const Value) VMError!Value {
    ...
    return switch (args[0]) {
        ...
        .string_val => .{ .int_val = 0 }, // DEFERRED (v12): parse int from string (strconv.parseInt, error handling)
        else => .{ .int_val = 0 },
    };
}
...
fn nativeFloat(vm: *VM, args: []const Value) VMError!Value {
    ...
    return switch (args[0]) {
        ...
        .string_val => .{ .float_val = 0.0 }, // DEFERRED (v12): parse float from string (strconv.parseFloat, error handling)
        else => .{ .float_val = 0.0 },
    };
}
...
test "VM comparison" {
    ...
    const code = [_]u8{
        ...
        @intFromEnum(Opcode.GT),         @intFromEnum(Opcode.HALT),
    };
