...
pub const VerilogCodeGen = struct {
    ...
    pub fn generate(self: *Self, spec: *const VibeeSpec) ![]const u8 {
        ...
        // Check if this is an FPGA-style spec (has behaviors defined)
        const is_fpga_style = spec.behaviors.items.len > 0;
    ...
    fn writeFPGAModule(self: *Self, spec: *const VibeeSpec) !void {
        ...
        // Note: spec.signals field doesn't exist, use default ports
    ...
    /// notand by Verilog file and withandtoand
    pub fn generate_old(self: *Self, spec: *const VibeeSpec) ![]const u8 {
        self.spec = spec;
        try self.writeHeader(spec);
        try self.writeTimescale();
        try self.writeSacredConstants(spec);
    ...
    fn writeTopModule(self: *Self, spec: *const VibeeSpec) !void {
        ...
        // Standard FPGA ports
        try self.builder.writeLine("input  wire        clk,");
        // Note: spec.reset field doesn't exist, use default reset values
        const has_reset = true;
        const rst_name = "rst_n";
        const rst_active = "!";
        const rst_edge = "negedge";
        ...
        if (has_reset) {
            // Note: spec.reset field doesn't exist, default to sync reset
            if (false) {
    ...
    fn writeClockGen(self: *Self) !void {
        // Note: spec.fpga_target field doesn't exist, default to xilinx
        const target = "xilinx";
    ...
    fn writeGenericBehavior(self: *Self, b: Behavior) !void {
        ...
        // Note: spec.fpga_target field doesn't exist, skip Intel DSP optimization
        if (false) {
            if (false) {
    ...
    fn writeTestbench(self: *Self, spec: *const VibeeSpec) !void {
        ...
        _ = vspec;
        // Note: vspec.reset field doesn't exist, use default reset values
        const has_reset = true;
        const rst_name = "rst_n";
        const rst_active = "0";
        const rst_release = "1";
    ...
    fn calculateLatency(self: *Self, spec: *const VibeeSpec) u32 {
        ...
        // Note: spec.pipeline field doesn't exist, always use default latency
        if (false) {
            base_latency += 1;
        } else if (false) {
            base_latency += 2;
