...
test "hybrid init without model" {
    ...
    const response = try chat.respond("прandinет");
...
test "hybrid symbolic hit" {
    ...
    const response = try chat.respond("прandinет"); // Russian greeting has higher confidence
    try std.testing.expect(response.source == .Symbolic);
...
test "wouldUseSymbolic" {
    ...
    try std.testing.expect(chat.wouldUseSymbolic("прandinет"));
