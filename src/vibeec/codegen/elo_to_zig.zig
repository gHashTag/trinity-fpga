// ELO Rating Codegen — Generate Zig from .tri spec
// φ² + 1/φ² = 3 | TRINITY
...
const ELO_TEMPLATE =
    ...
    \\}
    \\
    \\test "update ratings conserves total" {
...
pub fn generateElo(allocator: Allocator) ![]const u8 {
    return allocator.dupe(u8, ELO_TEMPLATE);
}

pub fn writeElo(allocator: Allocator, path: []const u8) !void {
    const content = try generateElo(allocator);
...
test "elo codegen" {
    const content = try generateElo(std.testing.allocator);
    defer std.testing.allocator.free(content);

    try std.testing.expect(content.len > 0);
    try std.testing.expect(std.mem.indexOf(u8, content, "pub const Verdict") != null);
    try std.testing.expect(std.mem.indexOf(u8, content, "pub fn expectedScore") != null);
    try std.testing.expect(std.mem.indexOf(u8, content, "K_FACTOR") != null);
}
