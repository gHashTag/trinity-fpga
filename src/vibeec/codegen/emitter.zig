...
pub const ZigCodeGen = struct {
    ...
    fn extractParamNames(phrase: []const u8, count: usize) []const []const u8 {
        _ = phrase;
        _ = count;
    ...
    fn parseMultiParamGiven(self: *Self, given: []const u8, name: []const u8) SignatureInfo {
        ...
        return self.inferSignatureFromSpecLegacy(given, name);
    }

    /// Build default params for given type and count
    fn buildDefaultParams(base_type: []const u8, count: usize) []const u8 {
        _ = base_type;
    ...
    fn extractNamedParams(self: *Self, given: []const u8, base_type: []const u8, count: usize) !?[]const u8 {
        ...
        if (std.mem.indexOf(u8, given, " and ")) |_| {
            ...
            if (names[1].len > 0) {
                ...
                const param1 = try std.fmt.allocPrint(self.allocator, "{s}: {s}", .{ names[0], base_type });
                const param2 = try std.fmt.allocPrint(self.allocator, "{s}: {s}", .{ names[1], base_type });
                defer self.allocator.free(param1);
                defer self.allocator.free(param2);
                return try std.fmt.allocPrint(self.allocator, "{s}, {s}", .{ param1, param2 });
            }
        }
        return null;
    }
    ...
    fn inferSignatureFromSpecAdvanced(self: *Self, given: []const u8, then: []const u8, name: []const u8) SignatureInfo {
        ...
        if (complex_result.params.len > 0 and
            ...
            const improved_ret = self.inferReturnTypeAdvanced(then, complex_result.ret);
            return .{ .params = complex_result.params, .ret = improved_ret };
        }

        // Fallback to keyword-based inference (original logic)
        return self.inferSignatureFromSpecFallback(given, then, name);
    }

    /// Enhanced return type inference (v10.1)
    fn inferReturnTypeAdvanced(self: *Self, then: []const u8, default_ret: []const u8) []const u8 {
        _ = self;
        ...
        if (containsCI(then, "resulting")) {
            ...
            if (std.mem.indexOf(u8, then, "resulting")) |idx| {
                ...
                if (rest.len > 0) {
                    const mapped = mapSemanticType(rest);
