// Sacred Constants Codegen — Generate Zig from .tri spec
// φ² + 1/φ² = 3 | TRINITY

const std = @import("std");
const Allocator = std.mem.Allocator;

const SACRED_CONSTANTS_TEMPLATE =
    ...
    \\// ============================================================================
    \\
    \\test "Sacred Constants - Golden Identity" {
...
pub fn generateSacredConstants(allocator: Allocator) ![]const u8 {
    return allocator.dupe(u8, SACRED_CONSTANTS_TEMPLATE);
}

pub fn writeSacredConstants(allocator: Allocator, path: []const u8) !void {
    const content = try generateSacredConstants(allocator);
...
test "sacred constants codegen" {
    const content = try generateSacredConstants(std.testing.allocator);
    defer std.testing.allocator.free(content);

    try std.testing.expect(content.len > 0);
    // Check that key constants are present
    try std.testing.expect(std.mem.indexOf(u8, content, "PHI: f64 = 1.618033988749895") != null);
    try std.testing.expect(std.mem.indexOf(u8, content, "TRINITY: f64 = 3.0") != null);
    try std.testing.expect(std.mem.indexOf(u8, content, "PHOENIX: i64 = 999") != null);
}
