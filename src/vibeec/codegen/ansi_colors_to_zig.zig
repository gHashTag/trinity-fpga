// ANSI Colors Codegen — Generate Zig from .tri spec
// φ² + 1/φ² = 3 | TRINITY
...
const ANSI_COLORS_TEMPLATE =
    ...
    \\// ============================================================================
    \\
    \\test "ANSI Colors - GREEN exists" {
    \\    try std.testing.expect(GREEN.len > 0);
    \\}
    \\
    \\test "ANSI Colors - RESET exists" {
    \\    try std.testing.expectEqualStrings("\x1b[0m", RESET);
    \\}
    \\
    \\test "ANSI Colors - All colors defined" {
    \\    try std.testing.expect(GREEN.len > 0);
...
pub fn generateAnsiColors(allocator: Allocator) ![]const u8 {
    return allocator.dupe(u8, ANSI_COLORS_TEMPLATE);
}

pub fn writeAnsiColors(allocator: Allocator, path: []const u8) !void {
...
test "ansi colors codegen" {
    const content = try generateAnsiColors(std.testing.allocator);
    defer std.testing.allocator.free(content);

    try std.testing.expect(content.len > 0);
    // Check that key colors are present
    try std.testing.expect(std.mem.indexOf(u8, content, "pub const GREEN") != null);
    try std.testing.expect(std.mem.indexOf(u8, content, "pub const RESET") != null);
    try std.testing.expect(std.mem.indexOf(u8, content, "pub fn printGold") != null);
}
