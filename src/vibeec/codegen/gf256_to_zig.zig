// GF256 Codegen — Generate Zig from .tri spec
// φ² + 1/φ² = 3 | TRINITY

const std = @import("std");
const Allocator = std.mem.Allocator;

const GF256_TEMPLATE =
...
pub fn generateGF256(allocator: Allocator) ![]const u8 {
    return allocator.dupe(u8, GF256_TEMPLATE);
}

pub fn writeGF256(allocator: Allocator, path: []const u8) !void {
    const content = try generateGF256(allocator);
...
test "gf256 codegen" {
    const content = try generateGF256(std.testing.allocator);
    defer std.testing.allocator.free(content);

    try std.testing.expect(content.len > 0);
    try std.testing.expect(std.mem.indexOf(u8, content, "pub const GF256") != null);
    try std.testing.expect(std.mem.indexOf(u8, content, "PRIMITIVE_POLY") != null);
}
