...
pub fn main() !void {
    ...
    var tokenizer = tokenizer_mod.SentencePieceTokenizer.load(allocator, "/workspaces/trinity/data/models/bitnet/tokenizer.json") catch |err| {
    ...
    std.debug.print("  Coherent generations: {d}/{d} ({d:.1}%)\n", .{ coherent_count, prompts.len, @as(f32, @floatFromInt(coherent_count)) / @as(f32, @floatFromInt(prompts.len)) * 100.0 });
