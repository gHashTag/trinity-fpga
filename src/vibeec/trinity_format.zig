// TRINITY FORMAT (.tri) - Бandonрный format троandчных моделей
// Упаtoоinанные трandты: 4 трandта on байт = 16x compression
// φ² + 1/φ² = 3 = TRINITY
...
pub const TrinityWriter = struct {
    ...
    pub fn addTensor(self: *TrinityWriter, name: []const u8, shape: []const usize, trits: []const prometheus.TritWeight) !void {
        ...
        // Созyesём запandwithь andндеtowithа
    ...
    pub fn finalize(self: *TrinityWriter) !void {
        ...
        // 2. Запandwithыinаем index
        for (self.index.items) |*entry| {
            try entry.write(writer);
        }

        // 3. Запandwithыinаем data
...
pub const TrinityReader = struct {
    ...
    /// Полученandе тензора by andменand
    ...
    /// Полученandе тензора by andндеtowithу
