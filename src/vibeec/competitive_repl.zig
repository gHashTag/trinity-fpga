...
pub const Lang = enum {
    ...
    pub fn getWelcome(self: Lang) []const u8 {
        return switch (self) {
            ...
            .RU => "[CYR:Добро] by[CYR:жало]in[CYR:ать] in Trinity REPL - Воwith[CYR:ьмая] Жand[CYR:знь]: [CYR:Кон]to[CYR:урентный] Agent",
...
pub const CompetitiveRepl = struct {
    ...
    fn printBanner(self: *Self, writer: anytype) !void {
        ...
        if (self.moe_engine.hardware.isKoSamuiMode()) {
            try writer.print("🏝️ Ko Samui mode enabled (low-latency optimizations active)\n\n", .{});
        }
    }
    ...
    fn executeTask(self: *Self, input: []const u8, writer: anytype) !void {
        ...
        // Parse task
        const plan = self.agent_tools.naturalLanguageParse(input);
        ...
        for (plan.steps) |step| {
            const result = try self.agent_tools.callWithCorrection(step.tool, step.args, self.verbose);
            if (result.success) {
                try writer.print("   ✅ {s}\n", .{result.output});
    ...
    fn printHelp(self: *Self, writer: anytype) !void {
        ...
        try writer.print("  [CYR:Опт]andмandзand[CYR:руй] inference for Qwen2.5-Coder-7B byд 8-core CPU\n", .{});
