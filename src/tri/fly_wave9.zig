...
pub fn deployWave9(allocator: Allocator, args: []const []const u8) !void {
    ...
    while (acct_idx <= ACCOUNT_COUNT) : (acct_idx += 1) {
        ...
        if (list_result.success()) {
            ...
            const parsed = std.json.parseFromSlice(std.json.Value, allocator, list_result.stdout, options) catch |err| {
                print("  {s}⚠️  Failed to parse JSON: {s}{s}\n", .{ YELLOW, @errorName(err), RESET });
                continue;
            };
            defer parsed.deinit();
...
pub fn recycleWave9(allocator: Allocator, args: []const []const u8) !void {
    ...
    for (farm.accounts[0..farm.account_count]) |*acct| {
        ...
        for (parsed.value.array.items) |app_val| {
            ...
            if (is_crashed) {
                total_crashed += 1;
                print("  {s}🔄 {s}: status={s} → restarting{s}\n", .{ YELLOW, name, status, RESET });
                ...
                if (restart_result.success()) {
                    print("    {s}✅ Restarted (seed={d}){s}\n", .{ GREEN, seed_counter, RESET });
                    restarted += 1;
                } else {
                    print("    {s}❌ Failed: {s}{s}\n", .{ RED, restart_result.stderr, RESET });
                    skipped += 1;
                }
                seed_counter += 1;
            }
