...
fn generateBashCompletion(allocator: Allocator, writer: anytype) !void {
    ...
    const commands_list = try getCommandsList(allocator);
    defer allocator.free(commands_list);

    // Build completion string without format placeholders
    try writer.writeAll("    if [[ $cword -eq 1 ]]; then\n");
    try writer.writeAll("        COMPREPLY=($(compgen -W \"");
    try writer.writeAll(commands_list);
...
fn getCommandsList(allocator: Allocator) ![]const u8 {
    ...
    for (commands, 0..) |cmd, i| {
        ...
        try list.appendSlice(allocator, cmd.name);
    }
    ...
    for (flat_commands) |cmd| {
        ...
        try list.appendSlice(allocator, cmd);
    }
...
fn generateZshCompletion(allocator: Allocator, writer: anytype) !void {
    ...
    const commands = getCommands();
    for (commands) |cmd| {
        // Build: (( $+functions[_tri_<cmd>] )) || _tri_<cmd>() {
        try writer.writeAll("    (( $+functions[_tri_");
        try writer.writeAll(cmd.name);
        try writer.writeAll("] )) || _tri_");
        try writer.writeAll(cmd.name);
        try writer.writeAll("() {\n");
        ...
        if (cmd.subcommands.len > 0) {
            ...
            const joined = try joinStrings(allocator, cmd.subcommands, " ");
            defer allocator.free(joined);
            try writer.writeAll(joined);
            try writer.writeAll(")\" \\\n");
            try writer.writeAll("            \"*::arg:->args\"\n");
            try writer.writeAll("        case $words[2] in\n");
            for (cmd.subcommands) |sub| {
                try writer.writeAll("            ");
                try writer.writeAll(sub);
                try writer.writeAll(") _arguments -s \"*:: :_files\" ;;\n");
            }
...
fn uninstallCompletion(shell: Shell) !void {
    ...
    try new_content.appendSlice(std.heap.page_allocator, content[0..content_start]);
    try new_content.appendSlice(std.heap.page_allocator, content[content_end..]);
