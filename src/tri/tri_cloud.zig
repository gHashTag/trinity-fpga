...
pub fn runCloudCommand(allocator: Allocator, args: []const []const u8) !void {
    ...
    if (eql(u8, subcmd, "status")) {
        ...
        return cloudFarm(allocator, sub_args);
    } else if (eql(u8, subcmd, "metal")) {
        return cloudMetal(allocator);
    } else if (eql(u8, subcmd, "delete-service")) {
        return cloudDeleteService(allocator, sub_args);
    } else if (eql(u8, subcmd, "train")) {
        return cloudTrain(allocator, sub_args);
    } else if (eql(u8, subcmd, "train-batch")) {
        return cloudTrainBatch(allocator);
    } else {
...
fn cloudMetal(allocator: Allocator) !void {
...
fn cloudFarm(allocator: Allocator, args: []const []const u8) !void {
    if (args.len == 0) {
...
fn spawnTrainService(allocator: Allocator, config: TrainConfig, farm: *railway_farm.RailwayFarm) !void {
    ...
    const account = if (config.account > 0) blk: {
        ...
        print("{s}✗ Account {d} not found{s}\n", .{ RED, config.account, RESET });
        return error.ApiError;
    } else farm.selectAccount() orelse {
        print("{s}✗ All farm accounts exhausted — no available slots{s}\n", .{ RED, RESET });
        return error.ApiError;
    };

    var api = farm.getApi(allocator, account.id) catch {
        print("{s}✗ Failed to init API for account {d}{s}\n", .{ RED, account.id, RESET });
        return error.ApiError;
    };
    defer api.deinit();
...
fn cloudDeleteService(allocator: Allocator, args: []const []const u8) !void {
    if (args.len < 1) {
...
fn cloudTrain(allocator: Allocator, args: []const []const u8) !void {
    if (args.len < 1) {
...
fn cloudTrainBatch(allocator: Allocator) !void {
    ...
    print("\n{s}═══ BATCH SUMMARY ══════════════════════════════{s}\n", .{ GOLDEN, RESET });
    print("  {s}✓ Spawned: {d}{s}\n", .{ GREEN, spawned, RESET });
    if (failed > 0) print("  {s}✗ Failed:  {d}{s}\n", .{ RED, failed, RESET });
