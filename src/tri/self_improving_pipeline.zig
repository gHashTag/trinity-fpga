...
// - Generates .tri specs for pipeline improvements
...
pub const ImprovementSuggestion = struct {
    ...
    /// .tri spec content (if applicable)
    vibee_spec: ?[]const u8,
...
pub const SelfImprovementEngine = struct {
    ...
    /// Generate .tri spec for pipeline improvement
    pub fn generateImprovementSpec(
        ...
        // Generate a .tri spec that improves the pipeline
        // MUST be allocated (not static) since caller will free it
        const spec =
        ...
        // Allocate a copy since caller will free this
...
pub fn executeSelfReferentialEvolution(
    ...
    // Step 4: Generate .tri spec for improvements
