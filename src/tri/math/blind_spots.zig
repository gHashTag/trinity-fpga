...
pub const registry = [_]KnowledgeEntry{
    ...
    .{ .id = "MATH_PI", .name = "Pi", .domain = "mathematics",
       .value = 3.141592653589793, .predicted_value = 3.141592653589793,
       .uncertainty = 0.0, .status = .{ .category = .verified, .confidence = 1.0, .discovery_date = "ancient" },
       .notes = "pi = circumference/diameter" },

    .{ .id = "MATH_E", .name = "Euler's number", .domain = "mathematics",
       .value = 2.718281828459045, .predicted_value = 2.718281828459045,
       .uncertainty = 0.0, .status = .{ .category = .verified, .confidence = 1.0, .discovery_date = "1618" },
       .notes = "Base of natural logarithms" },

    .{ .id = "MATH_PHI", .name = "Golden ratio", .domain = "mathematics",
       .value = 1.618033988749895, .predicted_value = 1.618033988749895,
       .uncertainty = 0.0, .status = .{ .category = .verified, .confidence = 1.0, .discovery_date = "ancient" },
       .notes = "phi^2 + 1/phi^2 = 3 = TRINITY" },

    .{ .id = "MATH_TRINITY", .name = "Trinity Identity", .domain = "mathematics",
       .value = 3.0, .predicted_value = 3.0,
       .uncertainty = 0.0, .status = .{ .category = .verified, .confidence = 1.0, .discovery_date = "2024" },
       .notes = "phi^2 + 1/phi^2 = 3 EXACTLY" },

    // Physical Constants - VERIFIED (Sacred Formula fits < 1%)
    .{ .id = "PHYS_FINE_STRUCTURE", .name = "Fine Structure Inverse", .domain = "physics",
       .value = 137.036, .predicted_value = 137.003,
       .uncertainty = 0.0001, .status = .{ .category = .verified, .confidence = 0.999, .discovery_date = "1916" },
       .notes = "1/alpha; Sacred fit: 4x3^2xpi^-1xphi^1xe^2 = 137.003 (error 0.024%)" },

    .{ .id = "PHYS_HIGGS", .name = "Higgs Mass", .domain = "physics",
       .value = 125.25, .predicted_value = 125.226,
       .uncertainty = 0.17, .status = .{ .category = .verified, .confidence = 0.999, .discovery_date = "2012" },
       .notes = "GeV; Sacred fit: 5x3^3xphi^4xe^-2 = 125.226 (error 0.019%)" },

    .{ .id = "PHYS_AGE_UNIVERSE", .name = "Age of Universe", .domain = "physics",
       .value = 13.787, .predicted_value = 13.788,
       .uncertainty = 0.020, .status = .{ .category = .verified, .confidence = 0.99, .discovery_date = "2001" },
       .notes = "Gyr; Sacred fit: 1x3^4xpi^-2xphi^-1xe^1 = 13.788 (error 0.005%)" },

    .{ .id = "PHYS_ELECTRON_MASS", .name = "Electron Mass", .domain = "physics",
       .value = 0.511, .predicted_value = 0.511,
       .uncertainty = 0.0001, .status = .{ .category = .verified, .confidence = 0.999, .discovery_date = "1897" },
       .notes = "MeV; Sacred fit: 2xpi^-2xphi^4xe^-1 = 0.511 (error 0.008%)" },

    .{ .id = "PHYS_NEUTRON_LIFETIME", .name = "Neutron Lifetime", .domain = "physics",
       .value = 879.4, .predicted_value = 879.4,
       .uncertainty = 0.6, .status = .{ .category = .verified, .confidence = 0.99, .discovery_date = "2024" },
       .notes = "seconds; Sacred fit: 2x3^4xpi^4xphi^-6 = 879.4 (PREDICTED then verified!)" },

    // Chemistry
    .{ .id = "CHEM_AVOGADRO", .name = "Avogadro Constant", .domain = "chemistry",
       .value = 6.022e23, .predicted_value = 6.022e23,
       .uncertainty = 1.8e17, .status = .{ .category = .verified, .confidence = 1.0, .discovery_date = "1811" },
       .notes = "mol^-1" },

    // === PREDICTED: Theory predicts, awaiting measurement ===

    .{ .id = "PHYS_NEUTRINO_MASS", .name = "Neutrino Mass", .domain = "physics",
       .value = 0.0, .predicted_value = 0.005695,
       .uncertainty = 0.001, .status = .{ .category = .predicted, .confidence = 0.5, .discovery_date = "future" },
       .notes = "eV; Sacred prediction: 1x3^-1xpi^-1xphi^-4xe^-1 = 0.0057 eV (BLIND SPOT!)" },

    .{ .id = "PHYS_PROTON_DECAY", .name = "Proton Lifetime", .domain = "physics",
       .value = 0.0, .predicted_value = 2.82e6,
       .uncertainty = 1e34, .status = .{ .category = .predicted, .confidence = 0.3, .discovery_date = "future" },
       .notes = "years; Sacred prediction: 3x3^4xpi^3xphi^4xe^4 = 2.82x10^34 yr (NOT TESTED!)" },
    ...
    .{ .id = "BLIND_DARK_MATTER_PARTICLE", .name = "Dark Matter Particle Mass", .domain = "physics",
       .value = 0.0, .predicted_value = 817.0,
       .uncertainty = 999.0, .status = .{ .category = .blind, .confidence = 0.0, .discovery_date = "unknown" },
       .notes = "GeV; Sacred fit: 4x3^4xphi^4 = 817 GeV (DM candidate!)" },

    .{ .id = "BLIND_DARK_ENERGY", .name = "Dark Energy Density", .domain = "physics",
       .value = 0.0, .predicted_value = 0.0,
       .uncertainty = 999.0, .status = .{ .category = .blind, .confidence = 0.0, .discovery_date = "unknown" },
       .notes = "WHAT IS IT? Omega_Lambda = 0.685 but no theory explains value" },

    .{ .id = "BLIND_CONSCIOUSNESS", .name = "Consciousness Mechanism", .domain = "intersection",
       .value = 0.0, .predicted_value = 0.0,
       .uncertainty = 999.0, .status = .{ .category = .blind, .confidence = 0.0, .discovery_date = "unknown" },
       .notes = "Physics + Mathematics + Philosophy blind spot" },

    // === ANOMALY: Contradicts theory ===

    .{ .id = "ANOMALY_MUON_G-2", .name = "Muon g-2 Anomaly", .domain = "physics",
       .value = 0.002331841, .predicted_value = 0.002331836,
       .uncertainty = 0.000000009, .status = .{ .category = .anomaly, .confidence = 0.95, .discovery_date = "2021" },
       .notes = "4.2 sigma deviation! New physics?" },

    .{ .id = "ANOMALY_HUBBLE_TENSION", .name = "Hubble Tension", .domain = "physics",
       .value = 73.0, .predicted_value = 67.4,
       .uncertainty = 2.0, .status = .{ .category = .anomaly, .confidence = 0.9, .discovery_date = "2019" },
       .notes = "Early vs late universe measurements disagree by 5 sigma!" },

    .{ .id = "ANOMALY_LHC_750GEV", .name = "LHC 750 GeV Diphoton", .domain = "physics",
       .value = 750.0, .predicted_value = 0.0,
       .uncertainty = 10.0, .status = .{ .category = .anomaly, .confidence = 0.3, .discovery_date = "2016" },
       .notes = "Went away with more data - statistical fluctuation" },
};
...
pub const blind_spots = [_]BlindSpot{
    .{ .id = "BS_NEUTRINO_MASS", .name = "Neutrino Absolute Mass Scale",
      .description = "We know neutrinos have mass, but not the absolute scale. Only mass differences measured.",
      .domain = "physics",
      .importance = 0.95,
      .feasibility = 0.6,
      .hypotheses = &[_][]const u8{
    ...
    .{ .id = "BS_PROTON_DECAY", .name = "Proton Decay Lifetime",
      .description = "Grand Unified Theories predict proton decay. Never observed.",
      .domain = "physics",
      .importance = 0.99,
      .feasibility = 0.3,
      .hypotheses = &[_][]const u8{
    ...
    .{ .id = "BS_DM_MASS", .name = "Dark Matter Particle Mass",
      .description = "Dark matter is 27% of universe. Particle unknown.",
      .domain = "physics",
      .importance = 0.98,
      .feasibility = 0.5,
      .hypotheses = &[_][]const u8{
    ...
    .{ .id = "BS_HUBBLE_TENSION", .name = "Hubble Tension Resolution",
      .description = "Early universe (CMB) vs late universe (supernovae) disagree on H0.",
      .domain = "physics",
      .importance = 0.92,
      .feasibility = 0.7,
      .hypotheses = &[_][]const u8{
    ...
    .{ .id = "BS_MUON_G2", .name = "Muon Anomalous Magnetic Moment",
      .description = "4.2 sigma deviation from Standard Model prediction.",
      .domain = "physics",
      .importance = 0.88,
      .feasibility = 0.8,
      .hypotheses = &[_][]const u8{
    ...
    .{ .id = "BS_TRINITY_EXTENSION", .name = "Why phi^2 + 1/phi^2 = 3?",
      .description = "This mathematical identity is too perfect. What is its physical origin?",
      .domain = "mathematics",
      .importance = 0.75,
      .feasibility = 0.9,
      .hypotheses = &[_][]const u8{
    ...
    .{ .id = "BS_SACRED_FORMULA_ORIGIN", .name = "Sacred Formula Origin",
      .description = "Why does V = nx3^kxpi^mxphi^pxe^q fit constants so well?",
      .domain = "intersection",
      .importance = 0.85,
      .feasibility = 0.6,
      .hypotheses = &[_][]const u8{
    ...
    .{ .id = "BS_COLD_FUSION", .name = "Low-Energy Nuclear Reactions",
      .description = "Nuclear fusion at room temperature. Claims controversial.",
      .domain = "chemistry_physics",
      .importance = 0.7,
      .feasibility = 0.5,
      .hypotheses = &[_][]const u8{
    ...
    .{ .id = "BS_EMERGENT_CONSCIOUSNESS", .name = "Emergence of Consciousness",
      .description = "How does matter become conscious? Hard problem.",
      .domain = "intersection",
      .importance = 1.0,
      .feasibility = 0.2,
      .hypotheses = &[_][]const u8{
    ...
    .{ .id = "BS_ORIGIN_OF_BIOASYMMETRY", .name = "Biological Homochirality",
      .description = "Why is life left-handed? L-amino acids, D-sugars only.",
...
pub const anomalies = [_]Anomaly{
    .{ .id = "ANOM_MUON_G2", .name = "Muon g-2",
      .expected = 0.002331836, .observed = 0.002331841, .deviation = 4.2,
      .domain = "physics" },

    .{ .id = "ANOM_HUBBLE", .name = "Hubble Tension",
      .expected = 67.4, .observed = 73.0, .deviation = 5.0,
      .domain = "physics" },

    .{ .id = "ANOM_LIKNIFE", .name = "Lithium Problem",
      .expected = 0.26, .observed = 0.24, .deviation = 3.0,
      .domain = "cosmology" },

    .{ .id = "ANOM_CORE", .name = "Core-Cusp Problem",
      .expected = 1.0, .observed = 0.5, .deviation = 10.0,
      .domain = "astrophysics" },
};
