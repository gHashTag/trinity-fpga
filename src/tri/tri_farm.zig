...
// LOCAL WAVE 9 was removed here.
//
// The `tri farm local-wave9` action group and its handlers imported
// src/tri/local_farm.zig, which does not exist: it was deleted in the wave-20
// cleanup, whose own record (.trinity/experience/wave_2026_07_15_wave20.md)
// lists what went with it -- "Removed: agent-*, docker-*, deploy-*, koschei,
// vibee, wave9, claude, etc." The deletion was intended; these handlers were
// the residue of an incomplete one, not a feature awaiting a decision.
//
// They made this file, and so src/tri/main.zig, impossible to compile:
//
//     src/tri/local_farm.zig:1:1: error: unable to load 'local_farm.zig':
//     FileNotFound
//
// Nothing outside this file referenced local_farm -- no spec, no doc, no test.
// Fly-based wave9 (src/tri/fly_wave9.zig) is a different module and stays.
// The deleted code is in git history if the Docker local farm is ever wanted.
