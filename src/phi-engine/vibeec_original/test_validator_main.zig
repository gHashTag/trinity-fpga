...
pub fn main() !void {
    // Test 1: Spec without output key (should fail)
    {
        ...
        const result = try spec_validator.validateSpecFile(std.heap.page_allocator, "/tmp/test_valid.tri");
    ...
    // Test 2: Spec with output key (should pass)
    {
        ...
        const result = try spec_validator.validateSpecFile(std.heap.page_allocator, "/tmp/test_valid_with_output.tri");
