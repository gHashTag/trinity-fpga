...
#ifndef TREE_SITTER_STUB_H
#define TREE_SITTER_STUB_H
...
static inline int ts_parser_set_language(TSParser* parser, const TSLanguage* language) { (void)parser; (void)language; return 0; }
...
static inline int ts_node_is_null(TSNode node) { (void)node; return 1; }
...
static inline int ts_node_is_named(TSNode node) { (void)node; return 0; }
static inline int ts_node_is_extra(TSNode node) { (void)node; return 0; }
static inline int ts_node_has_error(TSNode node) { (void)node; return 0; }
...
static inline int ts_query_cursor_next_capture(TSQueryCursor* cursor, TSQueryMatch* match, unsigned int* capture_index) { (void)cursor; (void)match; (void)capture_index; return 0; }

#ifdef __cplusplus
}
#endif

#endif // TREE_SITTER_STUB_H
