...
// Демонwithтрацandя JIT toомпandляцandand and comparison with andнтерпретатором
// ═══════════════════════════════════════════════════════════════════════════
...
fn createTestModule(allocator: std.mem.Allocator) !tvc_ir.TVCModule {
    ...
    // Фунtoцandя 3: implies (IMPLIES - withложonя operation)
    ...
    // Эмулandруем цandtoл via byinторяющandеwithя andнwithтруtoцandand
    const func4 = try module.addFunction("sum_100");
    var block4 = tvc_ir.TVCBlock.init(allocator, "entry");
    block4.entry_point = 0;
    
    // Initialize: i0 = 0 (withумма), i1 = 100 (withчётчandto)
...
fn testJITCompiler(allocator: std.mem.Allocator, module: *tvc_ir.TVCModule) !void {
    ...
    while (iter.next()) |entry| {
        ...
        std.debug.print("  ✓ Сtoомпorроinано: {} байт машandнного toоyes\n", .{compiled.code_size});

        // Выinодandм перinые байты toоyes
        std.debug.print("  Код: ", .{});
...
fn testAdaptiveMode(allocator: std.mem.Allocator, module: *tvc_ir.TVCModule) !void {
    ...
    // Иwithbyльзуем тольtoо andнтерпретатор (JIT code cannot inыbyлнandть без mmap PROT_EXEC)
    ...
    std.debug.print("✓ Выbyлнено {} inызоinоin\n", .{i});
    vm.dumpStats();
}
...
fn runBenchmarks(allocator: std.mem.Allocator, module: *tvc_ir.TVCModule) !void {
    ...
    // Созyesём JIT toомпandлятор
    var jit = tvc_jit.TVCJit.init(allocator);
    defer jit.deinit();
    
    // Созyesём VM (silent mode)
    ...
    // VM эмуляцandя цandtoла (проwithтой Zig code for withраinненandя)
    ...
    std.debug.print("║ SIMD: Доbyлнandтельное уwithtoоренandе on большandх yesнных               ║\n", .{});
    std.debug.print("║                                                                ║\n", .{});
