...
const Decoder = @import("tri27/emu/decoder.zig");
...
pub fn runTri27Command(allocator: Allocator, args: []const []const u8) !void {
    ...
    } else if (std.mem.eql(u8, subcmd, "experience")) {
...
fn runExperienceCommand(allocator: Allocator, args: []const []const u8) !void {
    if (args.len < 1) {
        std.debug.print("{s}Error: missing subcommand{s}\n", .{ RED, RESET });
        std.debug.print("  experience init                    Initialize experience log\n", .{});
        std.debug.print("  experience log <file> [ASM|DISASM|RUN|VAL]  Log operation\n", .{});
        std.debug.print("  experience status                  Show event history\n", .{});
        std.debug.print("  experience record <issue>        Record episode from last event\n", .{});
        return;
    }
    ...
    if (std.mem.eql(u8, subcmd, "init")) {
        ...
        std.debug.print("{s}TRI-27 experience log initialized\n", .{ GREEN, RESET });
        ...
        const event = if (event_opt) |ev| ev else {
            std.debug.print("{s}No events to record. Use 'log' first.{s}\n", .{ YELLOW, RESET });
            return;
        };

        try tri27_experience.recordEpisodeFromEvent(event.*, issue_num);
        std.debug.print("{s}Episode #{d} recorded for issue #{d}\n", .{ GREEN, event.timestamp, issue_num, RESET });
    } else {
        std.debug.print("{s}Unknown experience subcommand: {s}\n", .{ RED, subcmd, RESET });
    }
}
