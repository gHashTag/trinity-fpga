// Trinity Standard Library — Collections Module
// Generic data structures: Stack, Queue, RingBuffer, Pair, Triple, Optional, Result
...
/// Stack (LIFO) backed by ArrayList
...
/// Queue (FIFO) backed by ArrayList
...
/// Fixed-capacity ring buffer (no allocator needed)
...
/// Generic triple (Trinity!) of three values
...
/// Optional with default value support
...
/// Result type for error handling without Zig error unions
...
test "Optional" {
    const some_val = Optional(i32).some(42);
    const none_val = Optional(i32).none();
