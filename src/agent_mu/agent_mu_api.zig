...
pub fn generateMockEvolutionTree(allocator: Allocator, count: usize) ![]const u8 {
    ...
    for (0..count) |i| {
        ...
        const parent_str = if (i > 0)
            try std.fmt.allocPrint(std.heap.page_allocator, "\"node_{d}\"", .{i - 1})
        else
            "null";
        defer if (i > 0) std.heap.page_allocator.free(parent_str);
...
pub fn handleSacredMath(allocator: Allocator) ![]const u8 {
    return sacred_math.generateSacredMath(allocator);
}

/// Generate mock sacred math for offline dashboard
pub fn generateMockSacredMath(allocator: Allocator) ![]const u8 {
    const constants = sacred_math.getSacredConstants();
