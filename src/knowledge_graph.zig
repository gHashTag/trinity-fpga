...
// Граф знаний on основе Vector Symbolic Architecture
//
...
/// Magic bytes for идентификации файла
pub const FILE_MAGIC = [4]u8{ 'T', 'R', 'K', 'G' };

/// Версия формата файла
pub const FILE_VERSION: u32 = 1;

/// Размер packed вектора in байтах
pub const PACKED_VECTOR_BYTES = (VECTOR_DIM + 4) / 5; // 5 тритов on байт
...
/// Сущность in графе знаний (использует PackedBigInt for экономии памяти)
pub const Entity = struct {
    ...
    pub fn init(name: []const u8, id: u32) Self {
        // Генерируем vector из хэша имени
...
pub const Relation = struct {
    ...
    pub fn init(name: []const u8, id: u32) Self {
        const seed = Entity.hashString(name) ^ 0xDEADBEEF; // Другой seed for отношений
        const packed_vec = packed_vsa.randomPackedVector(VECTOR_DIM, seed);
        return Self{
            .name = name,
            .vector = packed_vec,
...
pub const Triple = struct {
    ...
    /// Создать триплет and закодировать in vector
    pub fn init(
...
/// Граф знаний on основе VSA (использует PackedBigInt)
pub const KnowledgeGraph = struct {
    ...
    /// Суперпозиция всех триплетов (граф how один vector)
    ...
    pub fn addTriple(self: *Self, subject: []const u8, predicate: []const u8, object: []const u8) void {
        ...
        // Добавить in list триплетов
        self.triples[self.triple_count] = triple;
        self.triple_count += 1;

        // Обновить граф-vector (bundle)
    ...
    /// Запрос: найти object by subject and predicate
    ...
    pub fn queryObject(self: *Self, subject: []const u8, predicate: []const u8) ?*Entity {
        ...
        // Создаём pattern запроса: bind(subject, predicate)
        ...
        // Найти ближайшую сущность to результату
        return self.findClosestEntityPacked(&result_vec);
    }

    /// Запрос: найти subject by predicate and object
    ...
    pub fn querySubject(self: *Self, predicate: []const u8, object: []const u8) ?*Entity {
        ...
        // Создаём pattern запроса: bind(predicate, object)
        ...
        for (0..result_count) |r| {
            ...
            for (0..self.entity_count) |i| {
                if (similarities[i] > best_sim) {
                    // Проверить what ещё не добавлен
    ...
    /// Найти ближайшую сущность to packed вектору
