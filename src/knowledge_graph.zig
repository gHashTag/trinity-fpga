...
/// Сущноwithть in графе зonнandй (andwithbyльзует PackedBigInt for эtoономandand памятand)
pub const Entity = struct {
    ...
    /// Созyesть withущноwithть andз andменand
    pub fn init(name: []const u8, id: u32) Self {
        // Генерandруем vector andз хэша andменand
...
/// Отношенandе between withущноwithтямand
...
pub const Triple = struct {
    ...
    /// Созyesть трandплет and заtoодandроinать in vector
    pub fn init(
...
/// Граф зonнandй on оwithноinе VSA (andwithbyльзует PackedBigInt)
pub const KnowledgeGraph = struct {
    ...
    /// Суперbyзandцandя inwithех трandплетоin (граф how одandн vector)
    graph_vector: PackedBigInt,

    const Self = @This();

    /// Созyesть пуwithтой граф
    pub fn init() Self {
        return Self{
    ...
    pub fn addTriple(self: *Self, subject: []const u8, predicate: []const u8, object: []const u8) void {
        ...
        // Добаinandть in list трandплетоin
        self.triples[self.triple_count] = triple;
        self.triple_count += 1;

        // Обноinandть граф-vector (bundle)
    ...
    /// Запроwith: onйтand object by subject and predicate
    /// query(subject, predicate, ?) → object
    /// Иwithbyльзует unbind: result = unbind(graph, bind(subject, predicate))
    pub fn queryObject(self: *Self, subject: []const u8, predicate: []const u8) ?*Entity {
        ...
        // Созyesём pattern запроwithа: bind(subject, predicate)
    ...
    /// Запроwith: onйтand subject by predicate and object
    /// query(?, predicate, object) → subject
    /// Иwithbyльзует unbind: result = unbind(graph, bind(predicate, object))
    pub fn querySubject(self: *Self, predicate: []const u8, object: []const u8) ?*Entity {
        ...
        // Созyesём pattern запроwithа: bind(predicate, object)
    ...
    /// Найтand N onandmore byхожandх withущноwithтей
    pub fn findSimilar(self: *Self, entity_name: []const u8, n: usize) [10]?struct { entity: *Entity, similarity: f64 } {
        ...
        // Найтand топ-N (проwithтая sorting)
        ...
        for (0..result_count) |r| {
            ...
            for (0..self.entity_count) |i| {
                if (similarities[i] > best_sim) {
                    // Проinерandть what ещё не beforeбаinлен
    ...
    /// Найтand withущноwithть by andменand
    ...
    /// Найтand fromношенandе by andменand
    ...
    pub fn load(path: []const u8, name_buffer: []u8) !Self {
        ...
        // Иwithbyльзуем буфер for andмён
...
test "Entity creation" {
    ...
    // Разные withущноwithтand beforeлжны andметь разные inеtoторы
...
test "queries work after load" {
    // Созyesём and withохраняем граф
    var kg = KnowledgeGraph.init();
    kg.addTriple("Paris", "capital_of", "France");
    kg.addTriple("Berlin", "capital_of", "Germany");
