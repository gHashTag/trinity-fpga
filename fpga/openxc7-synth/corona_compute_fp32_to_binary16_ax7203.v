...
`timescale 1ns / 1ps
// corona_compute_fp32_to_binary16_ax7203 — BINARY16 FP32_TO on AX7203.
module corona_compute_fp32_to_binary16_ax7203 (
    ...
    wire fs=a_reg[31]; wire [7:0] fe=a_reg[30:23]; wire [22:0] fm=a_reg[22:0];
    wire fz=(fe==0)&&(fm==0); wire fi=(fe==8'hFF)&&(fm==0); wire fn=(fe==8'hFF)&&(fm!=0);
    reg [15:0] rv;
    always @(*) begin
        if(fn) rv={1'b0,{5{1'b1}},{9{1'b0}},1'b1};
        else if(fi) rv={fs,{5{1'b1}},{10{1'b0}}};
        else if(fz) rv={16{1'b0}};
        else rv={fs,fe[4:0],fm[22:13]};
    end
    always @(posedge mclk or posedge rst) begin
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
            ...
            if(conv_trigger) result_reg<={{16{1'b0}},rv};
        end
    end
