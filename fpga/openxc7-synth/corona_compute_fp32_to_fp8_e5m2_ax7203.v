...
`timescale 1ns / 1ps
// corona_compute_fp32_to_fp8_e5m2_ax7203 — FP8_E5M2 FP32_TO on AX7203.
module corona_compute_fp32_to_fp8_e5m2_ax7203 (
    ...
    reg [7:0] rv;
    always @(*) begin
        if(fn) rv={1'b0,{5{1'b1}},{1{1'b0}},1'b1};
        else if(fi) rv={fs,{5{1'b1}},{2{1'b0}}};
        else if(fz) rv={8{1'b0}};
        else rv={fs,fe[4:0],fm[22:21]};
    end
    always @(posedge mclk or posedge rst) begin
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
            ...
            if(conv_trigger) result_reg<={{24{1'b0}},rv};
        end
    end
