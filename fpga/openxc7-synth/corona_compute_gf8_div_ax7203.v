...
`timescale 1ns / 1ps
// corona_compute_gf8_div_ax7203 — GF8 DIV on AX7203.
module corona_compute_gf8_div_ax7203 (
    ...
    reg [2:0] frm; reg [7:0] fmt_r; reg [7:0] a_r,b_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
    ...
    reg [7:0] a_reg,b_reg; reg comp_trigger;
    wire [7:0] fmt_a=a_reg, fmt_b=b_reg;
    wire f_sign_a = fmt_a[7];
    wire [2:0] f_exp_a = fmt_a[6:4];
    wire [3:0] f_mant_a = fmt_a[3:0];
    ...
    wire f_sign_b = fmt_b[7];
    wire [2:0] f_exp_b = fmt_b[6:4];
    wire [3:0] f_mant_b = fmt_b[3:0];
    wire f_zero_b = (f_exp_b == 0) && ((f_mant_b == 0));
    wire f_inf_b = 1'b0;   // gf8 has no Inf: exp=all-ones is finite
    wire f_nan_b = 1'b0;   // gf8 has no NaN encoding
    wire f_sub_b = (f_exp_b == 0) && ((f_mant_b != 0));
    wire signed [12:0] f_de_b = $signed({1'b0, f_exp_b}) - 13'sd3 + 13'sd127;
    wire [7:0] f_exp32_b = (f_de_b > 13'sd254) ? 8'd254 : (f_de_b < 0) ? 8'd0 : f_de_b[7:0];
    wire [22:0] f_mant32_b = {f_mant_b, 19'b0};
    wire [22:0] f_mant32_norm_b = {1'b0, f_mant_b, 18'b0};
    reg [31:0] fp32_b;
    always @(*) begin
        ...
        else if(f_sub_b) fp32_b={f_sign_b, 8'd125, f_mant32_norm_b};
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
            ...
            if(comp_ovld) result_reg<={24'b0,q_result};
        end
    end
