...
`timescale 1ns / 1ps
// corona_compute_fp32_to_fp6_e3m2_ax7203 — FP6_E3M2 FP32_TO on AX7203.
module corona_compute_fp32_to_fp6_e3m2_ax7203 (
    ...
    reg [5:0] rv;
    always @(*) begin
        if(fn) rv={1'b0,{3{1'b1}},{1{1'b0}},1'b1};
        else if(fi) rv={fs,{3{1'b1}},{2{1'b0}}};
        else if(fz) rv={6{1'b0}};
        else rv={fs,fe[2:0],fm[22:21]};
    end
    always @(posedge mclk or posedge rst) begin
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
            ...
            if(conv_trigger) result_reg<={{26{1'b0}},rv};
        end
    end
