...
`timescale 1ns / 1ps
// corona_compute_gf16_div_ax7203 — GF16 DIV on AX7203.
module corona_compute_gf16_div_ax7203 (
    ...
    wire f_sign_a = fmt_a[15];
    wire [5:0] f_exp_a = fmt_a[14:9];
    wire [8:0] f_mant_a = fmt_a[8:0];
    ...
    wire f_sign_b = fmt_b[15];
    wire [5:0] f_exp_b = fmt_b[14:9];
    wire [8:0] f_mant_b = fmt_b[8:0];
    wire f_zero_b = (f_exp_b == 0) && ((f_mant_b == 0));
    wire f_inf_b = (f_exp_b == 63) && ((f_mant_b == 0));
    wire f_nan_b = (f_exp_b == 63) && ((f_mant_b != 0));
    wire f_sub_b = (f_exp_b == 0) && ((f_mant_b != 0));
    wire signed [14:0] f_de_b = $signed({1'b0, f_exp_b}) - 15'sd31 + 15'sd127;
    wire [7:0] f_exp32_b = (f_de_b > 15'sd254) ? 8'd254 : (f_de_b < 0) ? 8'd0 : f_de_b[7:0];
    wire [22:0] f_mant32_b = {f_mant_b, 13'b0};
    wire [22:0] f_mant32_norm_b = {1'b0, f_mant_b, 12'b0};
    reg [31:0] fp32_b;
    always @(*) begin
    ...
    gf_div_param #(.EXP_BITS(8), .MANT_BITS(23), .HAS_INF(1)) u_comp (
    ...
    wire signed [14:0] tgt_exp_s = $signed({1'b0, q_exp}) - 15'sd127 + 15'sd31;
    reg [15:0] q_result;
    always @(*) begin
        if(q_nan) q_result=16'd0;
        else if(q_zero) q_result=16'd0;
        else if(q_inf) q_result={q_sign, 6'd63, 9'd0};
        else if(q_exp >= 8'd159) q_result={q_sign, 6'd63, 9'd0};
        else if(q_exp < 8'd88) q_result={q_sign, 15'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 6'b0, q_mant[22:14]};
        else q_result={q_sign, tgt_exp_s[5:0], q_mant[22:14]};
    end
