...
`timescale 1ns / 1ps
// corona_decode_gf8_ax7203 — GoldenFloat8 (S1E3M4, BIAS=3) decode on AX7203.
...
module corona_decode_gf8_ax7203 (
    ...
    wire [31:0] result;
    gf_decode_param #(.N(8), .E(3), .M(4), .BIAS(3), .OUT_REG(0)) u_dec (.clk(1'b0), .rst_n(1'b1), .gf_in(code_r[7:0]), .fp32_out(result), .is_nan_o(), .is_inf_o(), .is_zero_o(), .is_subnormal_o());
    assign led[2] = |result;
