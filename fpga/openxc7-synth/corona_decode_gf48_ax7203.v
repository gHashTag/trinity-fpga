...
`timescale 1ns / 1ps
// corona_decode_gf48_ax7203 — signed int128 → FP32 decode on AX7203.
// 16-byte code frame (128-bit input).
module corona_decode_gf48_ax7203 (
    ...
    reg [3:0] frm; reg [7:0] fmt_r; reg [47:0] code_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;code_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                4'd2:  begin fmt_r<=rx_byte;frm<=3; end
                4'd3:  begin code_r[7:0]<=rx_byte;frm<=4; end
                4'd4:  begin code_r[15:8]<=rx_byte;frm<=5; end
                4'd5:  begin code_r[23:16]<=rx_byte;frm<=6; end
