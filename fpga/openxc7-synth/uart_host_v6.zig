// VSA FPGA UART Host v6.0 — TRINITY V1
// Day 6: Unified system — VSA + BitNet + Quantum + UART
//
// Complete Trinity V1 command set:
// - PING (0xFF): Connectivity test
// - MODE (0x01): LED mode (separable|violation|zero|negative)
// - BIND (0x02): VSA bind operation
// - BUNDLE (0x03): VSA bundle operation
// - SIMILARITY (0x04): Cosine similarity score
// - BITNET (0x05): Tiny inference (prompt_id → token)
//
// φ² + 1/φ² = 3 = TRINITY
...
const Command = enum(u8) {
    ...
    BUNDLE = 0x03,
    SIMILARITY = 0x04,
    BITNET = 0x05,             // NEW: Tiny BitNet inference
    PING = 0xFF,
};
...
fn runModel(port: std.fs.File, prompt_id: u8) !u8 {
    ...
    _ = try sendPacket(port, Command.BITNET, &data);
