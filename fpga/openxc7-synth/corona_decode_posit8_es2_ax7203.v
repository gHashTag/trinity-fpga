...
// corona_decode_posit8_es2_ax7203 -- the board cell for Posit8 with es = 2.
//
// A clone of corona_decode_posit8_ax7203.v with one line changed: it instantiates
// posit8_es2_decode instead of posit8_decode. Everything else -- the STARTUPE2 clock,
// the 160000-baud UART framing, the LED map -- is byte-for-byte the original, so a
// diff between the two files shows exactly what differs about the format and nothing
// about the harness.
//
// Why a second cell rather than an edit. The existing wrapper's proof in issue #199 is
// a valid Tier-E result about posit(8,0); overwriting it would destroy evidence to fix
// a labelling problem. The catalogue's posit8 pack declares es = 2, so this cell is
// the one whose proof would support that pack.
//
// posit8_es2_decode is verified against SoftPosit on all 256 codes in simulation and
// synthesises to 103 LUTs against the legacy core's 45 -- 0.08 %% of an XC7A200T. See
// fpga/experience/2026-08-02-posit8-es2-core.trinity.md.
`default_nettype wire
`timescale 1ns / 1ps
module corona_decode_posit8_es2_ax7203 (
    ...
    wire [31:0] posit_o;
    posit8_es2_decode u_posit (.posit_in(code_r[7:0]), .fp32_out(posit_o), .is_zero(), .is_nar());
    wire [31:0] result = posit_o;
    assign led[2] = |result;
