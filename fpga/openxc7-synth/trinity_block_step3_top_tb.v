...
module trinity_block_step3_top_tb;

    reg clk;
    wire uart_tx, led;
    wire [1:0] debug_state;

    trinity_block_step3_top uut (
    ...
    initial begin
        $dumpfile("tb_step3.vcd");
        $dumpvars(0, trinity_block_step3_top_tb);
        clk = 0;
        $display("=== TRINITY BLOCK STEP 3: MatVec1 + ReLU + MatVec2 + Residual ===");
    end
    ...
    always @(posedge clk) begin
        if (uut.mv2_valid) begin
            if (result_count < 9)
                $display("  res: k=%0d mv2=%0d +x=%0d = %0d  expected=%0d match=%b",
                    uut.mv2_addr,
                    $signed(uut.mv2_data),
                    $signed(uut.res_rd_data),
                    $signed(uut.residual_data),
                    $signed(uut.expected_val),
                    ($signed(uut.residual_data) == $signed(uut.expected_val)));
            if (result_count == 9)
                $display("  ... (skipping) ...");
        ...
        if (uut.st_state == 3'd6 && uut.computation_done) begin
            $display("\n=== RESULT ===");
            ...
            $display("  Final results[0..7] (after residual):");
            ...
            $display("  Expected: -39365, 2, 39369, -39362, 5, 39372, -39359, 8");
