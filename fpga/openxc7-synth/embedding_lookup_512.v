...
// =============================================================================
// EMBEDDING LOOKUP 512 — Expanded Vocabulary Ternary Embedding
// =============================================================================
// Expands from 128 to 512 tokens using the 7 free BRAM36 on XC7A100T.
//
// Memory budget:
//   512 x 243 = 124,416 entries x 2 bits = 248,832 bits
//   BRAM allocation: ~7 BRAM36 (7 x 36,864 = 258,048 bits)
//   Power-of-2 depth: 2^17 = 131,072 entries
//
// Improvements over base embedding_lookup:
//   - 4x vocabulary: 128 → 512 tokens
//   - Compatible interface (just wider token_id)
//   - Base address multiply: 243 = (tok<<8) - (tok<<4) + (tok<<2) - tok
//   - Latency: DIM + 2 = 245 clocks = 4.9 us @ 50 MHz (unchanged)
//
// BRAM usage on XC7A100T:
//   Existing: 128 BRAM36 (4-block transformer)
//   This:     +7 BRAM36
//   Total:    135/135 BRAM36 (100% — fully utilized!)
//
...
module embedding_lookup_512 #(
    parameter VOCAB      = 512,
    parameter DIM        = 243,
    parameter DATA_WIDTH = 20,
    parameter ADDR_WIDTH = 17,   // 2^17 = 131072 >= 512*243 = 124416
    parameter TOK_WIDTH  = 9,    // ceil(log2(512))
    parameter DIM_WIDTH  = 8,    // ceil(log2(243))
    parameter MEM_FILE   = "fpga/weights/embedding_512_weights.mem"
)(
    input  wire                       clk,
    input  wire                       rst,
    input  wire                       start,
    ...
    // =========================================================================
    // 131,072 entries x 2 bits = 262,144 bits ≈ 7.1 BRAM36
    localparam MEM_DEPTH = 1 << ADDR_WIDTH;  // 131072
