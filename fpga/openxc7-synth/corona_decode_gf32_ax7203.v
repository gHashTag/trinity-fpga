...
`timescale 1ns / 1ps
// corona_decode_gf32_ax7203 — INT32 decode on AX7203 (32-bit decode frame).
// Same 32-bit decode frame as binary32 (4 code bytes). INT32 = identity (already 32-bit).
module corona_decode_gf32_ax7203 (
    ...
    wire [31:0] result;
    wire        is_zero;
    gf_decode_param #(.N(32), .E(12), .M(19), .BIAS(2047), .OUT_REG(0)) u_dec (.clk(1'b0), .rst_n(1'b1), .gf_in(code_r[31:0]), .fp32_out(result), .is_nan_o(), .is_inf_o(), .is_zero_o(), .is_subnormal_o());
    assign led[2] = |result;
