...
module gf_decode_param #(
    ...
    generate
        if (M <= FP32_MANT) begin : g_widen
            // BUGFIX #1 (widen-before-shift): the original
            //   assign norm_widen_result = { {..{1'b0}}, (pack_frac << (FP32_MANT-M)) };
            // shifted pack_frac ([M-1:0], M bits) INSIDE a self-determined
            // M-bit context, truncating all bits that shift above bit M-1
            // before the concatenation -- so mant23 came out 0 for any
            // pack_frac!=0. Symptom (iverilog witness): gf16 exhaustive
            // 1168/65536 bit-exact (fails=64368); only zero/inf/nan and
            // mant_in=0 cases passed. Python bit-model missed it (arbitrary-
            // precision ints don't truncate). Fix: widen pack_frac into a
            // [WIDE:0] container FIRST, then shift -- carry stays 0 on the
            // widen path since M+(FP32_MANT-M)=FP32_MANT <= WIDE.
    ...
    // BUGFIX #2 (found by independent iverilog witness №2, NOT by Python
    // bit-model): sub_shifted was declared [M:0] but read as [22:0] below
    // (`sub_mant_pre = {2'b0, sub_shifted[22:0]}`). For M<22 the bits
    // [22:(M+1)] are out-of-bounds -> X, which propagates through the round/
    // carry logic whenever the FP32-subnormal packer path is entered
    // (true_exp < -126, i.e. BIAS_GF > 127: gf24 BIAS=255, gf32 BIAS=2047).
    // gf4..gf16 (BIAS<=31) never enter this path, which is why they passed
    // exhaustive while gf24/gf32 emitted dut=00Xxxxxx on deep-underflow
    // normals. Fix: widen sub_shifted to [23:0] (RHS zero-extends, [22:0]
    // then reads valid zeros). Python bit-model could not catch this:
    // Python ints have no out-of-bounds read concept (урок 28.06 again).
