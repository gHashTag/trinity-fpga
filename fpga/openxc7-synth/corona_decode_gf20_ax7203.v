...
`timescale 1ns / 1ps
// corona_decode_gf20_ax7203 — INT32 decode on AX7203 (32-bit decode frame).
// Same 32-bit decode frame as binary32 (4 code bytes). INT32 = identity (already 32-bit).
module corona_decode_gf20_ax7203 (
    ...
    reg [2:0] frm; reg [7:0] fmt_r; reg [19:0] code_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
    ...
    wire [31:0] result;
    wire        is_zero;
    gf_decode_param #(.N(20), .E(7), .M(12), .BIAS(63), .OUT_REG(0)) u_dec (.clk(1'b0), .rst_n(1'b1), .gf_in(code_r[19:0]), .fp32_out(result), .is_nan_o(), .is_inf_o(), .is_zero_o(), .is_subnormal_o());
    assign led[2] = |result;
