...
`timescale 1ns / 1ps
// corona_decode_tf32_ax7203 — TF32 (TensorFloat-32, 1+8+10 = 19-bit) decode on AX7203.
// PROTOCOL VARIANT: 19-bit input does NOT fit the standard 16-bit code frame, so this
// build uses a 7-byte request frame:  AA 55 fmt code_lo code_mid code_hi trig
// where tf32_in[18:0] = {code_hi[2:0], code_mid[7:0], code_lo[7:0]}.
// Response is the standard 5-byte:  A5 r0 r1 r2 r3 (32-bit fp32 LE).
// Host must use the 3-byte exchange (corona_decode_host_ax7203.py::hw_exchange_tf32).
// Infra (CFGMCLK/STARTUPE2, BAUD_DIV=434, RX/TX FSMs) identical to corona_decode_fp8.
module corona_decode_tf32_ax7203 (
    ...
    // 7-byte frame: AA 55 fmt lo mid hi trig  (frm 0..6)
    reg [2:0] frm; reg [7:0] fmt_r; reg [23:0] code_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;code_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                3'd3: begin code_r[7:0]  <=rx_byte;frm<=3'd4; end
                3'd4: begin code_r[15:8] <=rx_byte;frm<=3'd5; end
                3'd5: begin code_r[23:16]<=rx_byte;frm<=3'd6; end
                3'd6: begin frame_valid<=1;frm<=3'd0; end
            endcase end
        end
    end
    assign led[1]=frame_valid;
    wire [18:0] tf32_code = code_r[18:0];
    wire [31:0] result;
    tf32_decode u_dec (.tf32_in(tf32_code), .fp32_out(result), .is_zero(), .is_inf(), .is_nan());
    assign led[2] = |result;
