...
module corona_compute_gf12_cmp_ax7203 (
    ...
    // ---- Frame FSM ----
    reg [2:0] frm; reg [7:0] fmt_r, op_r; reg [11:0] a_r, b_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;op_r<=0;a_r<=0;b_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                3'd5: begin a_r[11:8]<=rx_byte[3:0];frm<=3'd6; end
                3'd6: begin b_r[7:0]<=rx_byte;frm<=3'd7; end
                3'd7: begin b_r[11:8]<=rx_byte[3:0];frm<=3'd8; end
                3'd8: begin frame_valid<=1;frm<=0; end
                default: frm<=0;
            endcase end
        end
    end
    ...
    wire [10:0] abs_a = {ea, ma};
    wire [10:0] abs_b = {eb, mb};
    ...
    reg [11:0] result_reg;   // widened: the TX path reads up to bit 11
    reg result_ready;
    always @(posedge mclk or posedge rst) begin
