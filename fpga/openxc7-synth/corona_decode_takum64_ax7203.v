...
`timescale 1ns / 1ps
// corona_decode_takum64_ax7203 — takum64 (FP64) decode on AX7203.
// NEW 64-bit decode frame: AA 55 fmt code[0..7] trig (12 bytes) -> A5 FP32 LE.
module corona_decode_takum64_ax7203 (
    ...
    // 64-bit decode frame: AA 55 fmt code[0..7] trig (12 bytes, frm 0-11)
    reg [3:0] frm; reg [7:0] fmt_r; reg [63:0] code_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;code_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                4'd3:  begin code_r[7:0]<=rx_byte;frm<=4'd4; end
                4'd4:  begin code_r[15:8]<=rx_byte;frm<=4'd5; end
                4'd5:  begin code_r[23:16]<=rx_byte;frm<=4'd6; end
                4'd6:  begin code_r[31:24]<=rx_byte;frm<=4'd7; end
                4'd7:  begin code_r[39:32]<=rx_byte;frm<=4'd8; end
                4'd8:  begin code_r[47:40]<=rx_byte;frm<=4'd9; end
                4'd9:  begin code_r[55:48]<=rx_byte;frm<=4'd10; end
                4'd10: begin code_r[63:56]<=rx_byte;frm<=4'd11; end
                4'd11: begin frame_valid<=1;frm<=4'd0; end
            endcase end
        end
    end
    assign led[1]=frame_valid;
    takum64_decode_pipelined u_dec (.clk(mclk), .rst_n(~rst), .t64(code_r[63:0]), .fp32_out(result));
    assign led[2] = |result;
    reg responding; reg [2:0] tx_idx; reg [7:0] tx_buf0,tx_buf1,tx_buf2,tx_buf3,tx_buf4;
    reg [8:0] tcnt; reg [3:0] tbi; reg [9:0] tsr;
    // 6-cycle frame_valid delay for pipelined decoder
