#!/usr/bin/env python3
"""The specialised (MCM) lanes, same flow, same harness, same five seeds."""
...
A = [
 ("mn_mxfp4", 32,
  "  mcm_lane_mxfp4 #(.AW(8),.ACC(32)) u (.clk(clk),.rst_n(rst_n),"
  ".code(lf[3:0]),.a(lf[23:16]),.acc(y));", ["mcm_lane.v"], False, True),
 ("mn_cb10", 32,
  "  mcm_lane_cb10 #(.AW(8),.ACC(32)) u (.clk(clk),.rst_n(rst_n),"
  ".code(lf[3:0]),.a(lf[23:16]),.acc(y));", ["mcm_lane.v"], False, True),
]
res = {}
with cf.ThreadPoolExecutor(max_workers=2) as ex:
    for fu in cf.as_completed({ex.submit(run, *a): a[0] for a in A}):
        ...
        json.dump(res, open(os.path.join(HERE, "results_mcm.json"), "w"), indent=1)
