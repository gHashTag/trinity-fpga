...
module k_apot_requant (input wire clk, input wire rst_n, output wire [3:0] led);
  ...
  wire [31:0] keep = r0 ^ r1 ^ r2 ^ r3;   // держит все четыре живыми у ВСЕХ схем
  wire [5:0] p, q; wire sg;
  apot_requant #(.W(32)) u (.clk(clk), .v(r0), .p(p), .q(q), .sgn(sg));
  wire [31:0] acc = {19'b0, sg, p, q};
