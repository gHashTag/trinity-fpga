...
run() {  # $1 name  $2 width  $3 instance  $4 sources  [$5 x-expression]
cat > tw_$1.v <<V
\`timescale 1ns/1ps
module tw_$1;
  ...
  initial begin
    ...
    for (i = 0; i < 40000; i = i + 1) begin
      ...
      x = ${5:-r}; @(posedge clk); #1; \$display("%0d %08x", x, fp);
    end
    \$finish;
  end
endmodule
...
run tnf64s   65 "tnf64s_decode dec (.x(x), .fp32_out(fp));" "tnf_spec_decode.v" "{r[63], r}"
