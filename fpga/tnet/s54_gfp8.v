...
module s54_gfp8 (input wire clk, input wire rst_n, output wire [3:0] led);
  ...
  wire [31:0] fp; wire inv;
  gfplus8_a_decode dec (.word_in(lf[7:0]), .pocket(lf[9:8]), .fp32_out(fp), .is_zero()); assign inv=1'b0;
  ...
  assign led = all[3:0]^all[7:4]^all[11:8]^all[15:12]^all[19:16]^all[23:20]
             ^ all[27:24]^all[31:28]^{1'b0,all[34:32]}^{3'b0,inv};
endmodule
