...
V
yosys -q -p "read_verilog fpga/tnet/f_$1.v $3 fpga/tnet/gft_add_w.v; synth_xilinx -flatten -nodsp -top f_$1 -json fpga/tnet/f_$1.json" > fpga/tnet/fy_$1.log 2>&1
[ -f fpga/tnet/f_$1.json ] || { echo "$1|СИНТЕЗ_НЕ_ПРОШЁЛ"; return; }
FS=""
for S in 1 2 3 4 5; do
  $NP --chipdb $CDB --xdc bench.xdc --json fpga/tnet/f_$1.json --seed $S --write /dev/null > fpga/tnet/fs_$1_$S.log 2>&1
  FS="$FS $(grep -oE "Max frequency for clock .[^']*.: [0-9.]+" fpga/tnet/fs_$1_$S.log|tail -1|grep -oE '[0-9.]+$')"
done
L=$(grep -oE "SLICE_LUTX: *[0-9]+" fpga/tnet/fs_$1_1.log|tail -1|grep -oE '[0-9]+$')
echo "$1|$L|$FS"
}
cd "$(dirname "$0")/../.."   # takum16 reads its table by a repo-relative path
run takum16   "takum16_decode dec (.clk(clk), .takum16_in(lf[15:0]), .fp32_out(fp));" "fpga/tnet/takum16_decode.v"
