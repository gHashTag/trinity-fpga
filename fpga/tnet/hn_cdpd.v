...
module hn_cdpd (input wire clk, input wire rst_n, output wire [3:0] led);
  ...
  wire [31:0] fp; wire flag;
  tnf16c_dpd_decode dec (.x(lf[15:0]), .fp32_out(fp), .noncanonical(flag));
  ...
  assign led = all[3:0]^all[7:4]^all[11:8]^all[15:12]^all[19:16]^all[23:20]
             ^ all[27:24]^all[31:28]^{1'b0,all[34:32]}^{3'b0,flag};
endmodule
