...
run binary32  "binary32_decode dec (.b32_in(lf[31:0]), .fp32_out(fp));" "binary32_decode.v"
run binary16  "binary16_decode dec (.b16_in(lf[15:0]), .fp32_out(fp));" "binary16_decode.v"
...
run minifloat "minifloat_decode dec (.mf_in(lf[7:0]), .fp32_out(fp));" "minifloat_decode.v"
run takum16   "takum16_decode dec (.takum_in(lf[15:0]), .fp32_out(fp));" "takum16_decode.v"
run posit8    "posit8_es2_decode dec (.posit_in(lf[7:0]), .fp32_out(fp), .is_zero(), .is_nar());" "posit8_es2_decode.v"
