...
def self_test():
    cases = {0x00000000: 0x00000000, 0x3F800000: 0x3F800000,  # 0.0, 1.0
             0xBF800000: 0xBF800000, 0x40000000: 0x40000000,     # -1.0, 2.0
             0x7F7FFFFF: 0x7F7FFFFF, 0x00000001: 0x00000001}    # max, smallest subnormal
    bad = 0
    for code, exp in cases.items():
        if golden_binary32(code) != exp:
            bad += 1
...
def run_hw(port, baud, n):
    ...
    corners = [0x00000000, 0x3F800000, 0xBF800000, 0x40000000, 0x7F7FFFFF, 0x00000001,
               0x40490FDB, 0xC0490FDB, 0x7FC00000]
    sample = corners + [rnd.randint(0, 0xFFFFFFFF) for _ in range(max(0, n - len(corners)))]
    for code in sample:
        hw = hw_exchange(ser, code)
        gold = golden_binary32(code)
        checked += 1
