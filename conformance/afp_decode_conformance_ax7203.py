#!/usr/bin/env python3
"""afp decode conformance — 16-bit → FP32. 2-byte frame."""
import serial, struct, time, random, sys, argparse
N,E,M,BIAS = 16,8,7,127
EM = (1<<E)-1
def decode(raw):
    raw &= (1<<N)-1
    ...
    # bf16 / IEEE 754: subnormals are kept (not flushed). The RTL
    # (bf16_decode.v) performs a pure bit-pad {bf16_in, 16'b0} and returns
    # subnormal FP32 values as-is.
