...
T27 = {0x0:0x0, 0x4000000000000000:0x3F800000, 0xC000000000000000:0xBF800000,
       0x4080000000000000:0x40000000, 0x40C0000000000000:0x40400000}

def golden_vax_d(code):
...
def self_test():
    bad = sum(1 for c,e in T27.items() if golden_vax_d(c) != e)
...
def run_hw(port, baud, n):
    ...
    for code in sample:
        hw = hw_exchange(ser, code); gold = golden_vax_d(code); checked += 1
