#!/usr/bin/env python3
"""GF4 MUL compute-conformance on AX7203 (1S+1E+2M, bias=0, no Inf).

Exhaustive: all 16x16 = 256 input pairs. Golden = gf_ref.gf_mul (exact
...
Result is 4 bits, packed in the low nibble of res_lo by gf4_clean_ax7203.
  ...
  self-test:  python3 gf4_mul_conformance_ax7203.py --self-test
  on HW:      python3 gf4_mul_conformance_ax7203.py --port /dev/cu.usbserial-120 --baud 160000
"""
...
def self_test():
    """Offline golden gate (no HW): every result in-width + MUL commutativity + corners."""
    bad = 0
    for a in range(1 << WIDTH):
        for b in range(1 << WIDTH):
            ...
            if gf_mul(FMT, a, b) != gf_mul(FMT, b, a):   # commutativity
                bad += 1
                if bad <= 8:
