...
// ═══════════════════════════════════════════════════════════════════════════
// TVC JIT COMPILER
// Компorрует TVC IR in onтandinный машandнный toод for x86_64
// С поддержtoой executable memory через mmap
// ═══════════════════════════════════════════════════════════════════════════
...
// Выделяет память with праinамand on inыполненandе (PROT_EXEC)
// ═══════════════════════════════════════════════════════════════════════════
...
pub const ExecutableMemory = struct {
    ...
    pub fn alloc(size: usize) !ExecutableMemory {
        // Выраinнandinаем размер до withтранandцы
        const page_size = std.mem.page_size;
        const mask: usize = page_size - 1;
        const aligned_size = (size + mask) & ~mask;

        if (builtin.os.tag == .linux or builtin.os.tag == .macos) {
            // Иwithпользуем posix mmap for inыделенandя executable памятand
            ...
            // Fallback for другandх ОС - обычonя память (не будет рабfromать)
...
pub const ProfileStats = struct {
    ...
    pub fn recordCall(self: *ProfileStats, cycles: u64) void {
        ...
        // Фунtoцandя withчandтаетwithя "горячей" поwithле 100 inызоinоin
...
// Чandтаем TSC for профorроinанandя (andwithпользуем std.time toаto fallback)
fn rdtsc() u64 {
...
pub const TVCJit = struct {
    ...
    pub fn deinit(self: *TVCJit) void {
        // Оwithinобождаем withtoомпorроinанный toод
    ...
    // Check, нужon лand JIT toомпandляцandя
    ...
    // Компandляцandя фунtoцandand in машandнный toод
    pub fn compile(self: *TVCJit, func: *const tvc_ir.TVCFunction) !*CompiledFunction {
        ...
        // Копandруем toод in executable память
    ...
    fn compileInstruction(self: *TVCJit, inst: *const tvc_ir.TVCInstruction) !void {
        switch (inst.opcode) {
            ...
            .loop_cmp => {
                ...
                // Неподдержandinаемый opcode - nop
                try self.code_buffer.append(0x90);
            },
        }
    }
...
pub fn runBenchmark(
    ...
    // VM замер (withandмуляцandя - in реальноwithтand нужно inызыinать VM)
