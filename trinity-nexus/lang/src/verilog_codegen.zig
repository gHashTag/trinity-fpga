...
pub const VerilogCodeGen = struct {
    ...
    /// Generate Verilog file and write to output
    pub fn generate(self: *Self, spec: *const VibeeSpec) ![]const u8 {
        self.spec = spec;

        // Check if this is an FPGA-style spec (has signals defined)
        const is_fpga_style = spec.signals.items.len > 0;

        // Debug: Write to comment in output (write directly before any other output)
        try self.builder.writeFmt("// SIGNALS_COUNT: {d}\n", .{spec.signals.items.len});

        if (!is_fpga_style) {
            // Traditional VIBEE output with sacred constants, types, etc.
    ...
    fn writeHeader(self: *Self, spec: *const VibeeSpec) !void {
        ...
        try self.builder.writeLine("//");
        const latency = self.calculateLatency(spec);
        try self.builder.writeFmt("// Latency:  {d} cycles\n", .{latency});
        try self.builder.writeFmt("// Target:   {s} ({d} MHz)\n", .{ spec.fpga_target, spec.target_frequency });
    ...
    fn writeFPGAModule(self: *Self, spec: *const VibeeSpec) !void {
        ...
        for (spec.signals.items, 0..) |sig, i| {
            const comma = if (i < spec.signals.items.len - 1) "," else "";
            const width_str = if (sig.width > 1)
                try std.fmt.allocPrint(self.allocator, "[{d}:0] ", .{sig.width - 1})
            else
                "";
            defer {
                if (sig.width > 1) self.allocator.free(width_str);
            }
            try self.builder.writeFmt("{s}  wire {s}{s}{s}\n", .{ sig.direction, width_str, sig.name, comma });
        }
        ...
        try self.builder.writeFmt("endmodule\n", .{});
    }
    ...
    fn writeTritFullAdder(self: *Self) !void {
        ...
        const latency = self.calculateLatency(self.spec);
        try self.builder.writeFmt("// Latency:  {d} cycles\n", .{latency});
        try self.builder.writeFmt("// Target:   {s} ({d} MHz)\n", .{ self.spec.fpga_target, self.spec.target_frequency });
        try self.builder.writeFmt("// Pipeline: {s}\n", .{self.spec.pipeline});
        try self.builder.writeLine("// ═══════════════════════════════════════════════════════════════════════════════");
    ...
    fn calculateLatency(self: *Self, spec: *const VibeeSpec) u32 {
        _ = self;
        var base_latency: u32 = 0;

        // Modules with known latency in this architecture
        if (containsIgnoreCase(spec.name, "bitnet")) {
            base_latency = 5; // FSM states for BitNet processing
        } else if (containsIgnoreCase(spec.name, "sha256")) {
            base_latency = 64; // SHA rounds
        } else if (spec.behaviors.items.len > 0) {
            base_latency = 2; // Default FSM cycle (IDLE -> PROCESS -> DONE)
        }

        // Pipelining adds cycles but increases frequency
        if (std.mem.eql(u8, spec.pipeline, "auto") or std.mem.eql(u8, spec.pipeline, "stage1")) {
            base_latency += 1;
        } else if (std.mem.eql(u8, spec.pipeline, "stage2")) {
            base_latency += 2;
        }
