...
// When: Query each, classify by similarity threshold (0.10)
// Then: 30/30 — in-KG queries return symbolic match, out-of-KG routed to LLM fallback
// Test symbolicFirstPipeline: verify behavior is callable (compile-time check)
_ = symbolicFirstPipeline;
}
...
// When: Route each query based on similarity threshold
// Then: 20/20 — correct routing for all queries
// Test hybridRoutingAccuracy: verify behavior is callable (compile-time check)
_ = hybridRoutingAccuracy;
}
...
// When: Query each memory with its own keys
// Then: 25/25 — correct dispatch across all 5 memories
// Test multiMemoryDispatch: verify behavior is callable (compile-time check)
_ = multiMemoryDispatch;
}
...
test "igla_symbolic_hit" {
// Given: { query: "entity_0", memory: "bundled_10_pairs" }
...
test "igla_llm_fallback" {
// Given: { query: "unknown_entity", memory: "bundled_10_pairs" }
