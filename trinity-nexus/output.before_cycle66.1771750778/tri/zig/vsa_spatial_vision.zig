...
/// Bind patch vector with 2D spatial position (vision encoding)
/// Uses double permutation: permute(x) then permute(y*width) for 2D grid
pub fn realSpatialBind(patch: *vsa.HybridBigInt, position_vec: *vsa.HybridBigInt, x: usize, y: usize, width: usize) vsa.HybridBigInt {
    var pos_x = vsa.permute(position_vec, x);
    var pos_xy = vsa.permute(&pos_x, y * width);
    return vsa.bind(patch, &pos_xy);
}

/// Bundle spatially-bound patch vectors into image representation
pub fn realSpatialBundle(a: *vsa.HybridBigInt, b_vec: *vsa.HybridBigInt) vsa.HybridBigInt {
    return vsa.bundle2(a, b_vec);
}

/// Compare two spatially-encoded images
pub fn realSpatialSimilarity(img_a: *vsa.HybridBigInt, img_b: *vsa.HybridBigInt) f64 {
    return vsa.cosineSimilarity(img_a, img_b);
}

/// Hamming distance between spatially-encoded images
pub fn realSpatialDistance(img_a: *vsa.HybridBigInt, img_b: *vsa.HybridBigInt) usize {
    return vsa.hammingDistance(img_a, img_b);
}

/// Convert patch intensity to base hypervector
pub fn realPatchToVector(intensity: u8) vsa.HybridBigInt {
    return vsa.charToVector(intensity);
}
...
// Test realSpatialBind: verify behavior is callable (compile-time check)
_ = realSpatialBind;
}
...
// Test realSpatialBundle: verify behavior is callable (compile-time check)
_ = realSpatialBundle;
}
...
// Test realSpatialDistance: verify behavior is callable (compile-time check)
_ = realSpatialDistance;
}
...
// Test realPatchToVector: verify behavior is callable (compile-time check)
_ = realPatchToVector;
}
