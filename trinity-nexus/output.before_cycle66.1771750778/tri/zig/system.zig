...
// When: Update interval reached
// Then: Read system metrics (macOS sysctl or simulated)
// Test fetch_metrics: verify behavior is callable (compile-time check)
_ = fetch_metrics;
}
...
// When: Drawing CPU row
// Then: Draw label, usage bar, percentage, core info
// Test draw_cpu_section: verify behavior is callable (compile-time check)
_ = draw_cpu_section;
}
...
// When: Drawing memory row
// Then: Draw label, RAM bar, swap bar, values
// Test draw_memory_section: verify behavior is callable (compile-time check)
_ = draw_memory_section;
}
...
// When: Drawing temperature row
// Then: Draw CPU/GPU temp bars with color thresholds
// Test draw_temperature_section: verify behavior is callable (compile-time check)
_ = draw_temperature_section;
}
...
// When: Drawing disk row
// Then: Draw space bar, I/O rates
// Test draw_disk_section: verify behavior is callable (compile-time check)
_ = draw_disk_section;
}
...
// When: Drawing network row
// Then: Draw RX/TX rates with arrows
// Test draw_network_section: verify behavior is callable (compile-time check)
_ = draw_network_section;
}
...
// When: Drawing process row
// Then: Draw process/thread counts
// Test draw_process_section: verify behavior is callable (compile-time check)
_ = draw_process_section;
}
...
// When: Drawing uptime row
// Then: Format and draw "Xd Xh Xm Xs"
// Test draw_uptime: verify behavior is callable (compile-time check)
_ = draw_uptime;
}
...
// When: Displaying size
// Then: Return formatted string (KB, MB, GB)
// Test format_bytes: verify behavior is callable (compile-time check)
_ = format_bytes;
}
...
// When: Displaying uptime
// Then: Return formatted "Xd Xh Xm" string
// Test format_uptime: verify behavior is callable (compile-time check)
_ = format_uptime;
}
