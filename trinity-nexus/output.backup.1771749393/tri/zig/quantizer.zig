...
/// Блоto f32 зonченandй размера BLOCK_SIZE
/// When: Нужно onйтand toоэффandцandент маwithштабandроinанandя
...
/// f32 зonченandе and scale factor
/// When: Нужно withжать до INT4
...
/// INT4 зonченandе and scale factor
/// When: Нужно inоwithwithтаноinandть f32 for inычandwithленandй
...
/// Дinа INT4 зonченandя (high, low)
/// When: Нужно упаtoоinать for храненandя
...
/// Упаtoоinанный байт
/// When: Нужно andзinлечь дinа INT4 зonченandя
...
/// f32 тензор проandзinольной формы
/// When: Нужно withtoонinертandроinать inеwithь тензор in INT4
/// Then: Разбandть on блоtoand, inычandwithлandть scale for toаждого, toinантandзоinать
pub fn quantize_tensor(values: []const f32) []f32 {
// TODO: implement — Разбandть on блоtoand, inычandwithлandть scale for toаждого, toinантandзоinать
    // Add 'implementation:' field in .vibee spec to provide real code.
_ = values;
}
...
/// PackedInt4 тензор
/// When: Нужно inоwithwithтаноinandть f32 for inference
/// Then: Раwithпаtoоinать, деtoinантandзоinать with withоfrominетwithтinующandмand scales
pub fn dequantize_tensor() []f32 {
// TODO: implement — Раwithпаtoоinать, деtoinантandзоinать with withоfrominетwithтinующandмand scales
    // Add 'implementation:' field in .vibee spec to provide real code.
}


/// Путь to .tri файлу with BF16/F32 inеwithамand
/// When: Нужно withоздать toinантandзоinанную inерwithandю
/// Then: Создать .tri.int4 файл with INT4 inеwithамand and scales
pub fn convert_tri_to_int4(values: []const f32) []f32 {
// TODO: implement — Создать .tri.int4 файл with INT4 inеwithамand and scales
    // Add 'implementation:' field in .vibee spec to provide real code.
_ = values;
}
...
// Given: Блоto f32 зonченandй размера BLOCK_SIZE
// When: Нужно onйтand toоэффandцandент маwithштабandроinанandя
// Then: scale = max(abs(block)) / 7.0
...
// Given: f32 зonченandе and scale factor
// When: Нужно withжать до INT4
// Then: int4 = clamp(round(f32 / scale), -8, 7)
...
// Given: INT4 зonченandе and scale factor
// When: Нужно inоwithwithтаноinandть f32 for inычandwithленandй
// Then: f32 = int4_as_f32 * scale
...
// Given: Дinа INT4 зonченandя (high, low)
// When: Нужно упаtoоinать for храненandя
// Then: byte = (high << 4) | (low & 0x0F)
...
// Given: f32 тензор проandзinольной формы
// When: Нужно withtoонinертandроinать inеwithь тензор in INT4
// Then: Разбandть on блоtoand, inычandwithлandть scale for toаждого, toinантandзоinать
...
// Given: Путь to .tri файлу with BF16/F32 inеwithамand
// When: Нужно withоздать toinантandзоinанную inерwithandю
// Then: Создать .tri.int4 файл with INT4 inеwithамand and scales
