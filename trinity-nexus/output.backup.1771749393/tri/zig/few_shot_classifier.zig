...
// When: Bundle all K examples into a single prototype vector
// Then: Prototype preserves class concept signal above noise
// Test buildPrototype: verify behavior is callable (compile-time check)
_ = buildPrototype;
}
...
// Test classifyCosine: verify behavior is callable (compile-time check)
_ = classifyCosine;
}
...
// When: Build prototypes at each shot level, classify 20 test items
// Then: All shot levels achieve 100% (clean concept signals)
// Test fewShotCurve: verify behavior is callable (compile-time check)
_ = fewShotCurve;
}
...
// Test noBackpropClassification: verify behavior is callable (compile-time check)
_ = noBackpropClassification;
}
