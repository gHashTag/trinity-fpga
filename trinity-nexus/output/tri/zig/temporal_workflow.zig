...
pub fn start_workflow(input: []const u8) !void {
...
pub fn query_workflow(request: anytype) !void {
...
pub fn set_timer(config: anytype) !void {
...
pub fn get_workflow_metrics(self: *@This()) !void {
...
// When: Activity dispatched to worker
// Then: Activity executed, result recorded in history
// Test execute_activity: verify behavior is callable (compile-time check)
_ = execute_activity;
}
...
// When: Retry triggered after backoff
// Then: Activity rescheduled with incremented attempt
// Test retry_activity: verify behavior is callable (compile-time check)
_ = retry_activity;
}
...
// When: Signal delivery requested
// Then: Signal buffered and delivered to workflow handler
// Test send_signal: verify behavior is callable (compile-time check)
_ = send_signal;
}
...
// When: Synchronous query received
// Then: Query handler returns current workflow state
// Test query_workflow: verify behavior is callable (compile-time check)
_ = query_workflow;
}
...
// When: Child workflow spawned
// Then: Child created with parent tracking
// Test start_child_workflow: verify behavior is callable (compile-time check)
_ = start_child_workflow;
}
...
// When: Timer creation requested
// Then: Durable timer persisted, fires at scheduled time
// Test set_timer: verify behavior is callable (compile-time check)
_ = set_timer;
}
...
// When: Migration triggered
// Then: State transformed to new version format
// Test migrate_version: verify behavior is callable (compile-time check)
_ = migrate_version;
}
...
// When: Metrics requested
// Then: Returns WorkflowMetrics with execution stats
// Test get_workflow_metrics: verify behavior is callable (compile-time check)
_ = get_workflow_metrics;
}
