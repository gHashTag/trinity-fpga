...
cache_get_tmux_golden_chain_binary() {
    ...
    # Only use pre-known cache paths - no arrays for sh compatibility

    # Path 1: h cache
    for expanded in .zig-cache/h/*/p/zig-out-bin/zig-out/bin/tmux-golden-chain; do
        if [ -f "$expanded" ]; then
    ...
    # Path 2: b cache
    for expanded in .zig-cache/b/*/p/zig-out-bin/zig-out/bin/tmux-golden-chain; do
        if [ -f "$expanded" ]; then
            cache_set "tmux_golden_chain_bin" "$expanded"
            echo "$expanded"
            return 0
