...
pub const russian_keywords = [_]KeywordMapping{
    .{ .original = ":]totsandya", .english = "function" },
    .{ .original = "fandbaboutonchchand", .english = "fibonacci" },
    // ... 38 more
};
...
### Russian Prompt
```bash
$ ./zig-out/bin/tri code "onpandshand :]totsandyu fandbaboutonchchand"
Detected language: [RU] Russian (confidence: 100%)
Generating code for: onpandshand :]totsandyu fandbaboutonchchand
...
Detected language:  English (confidence: 80%)
